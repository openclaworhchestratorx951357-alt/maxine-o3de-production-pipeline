...
## Release Publication Execution Window Ticket QC Attachment Point

- `RELEASE_PUBLICATION_EXECUTION_WINDOW_TICKET_v1` report validation output is attachable to manifest QC.
- Current manifest attachment path: `qc.gates[]`.
- Future-compatible path: `qc.checks[]`.
- Suggested check id: `release_publication_execution_window_ticket_v1`.

## Release Publication Execution Window State QC Attachment Point

- `RELEASE_PUBLICATION_EXECUTION_WINDOW_STATE_v1` report validation output is attachable to manifest QC.
- Current manifest attachment path: `qc.gates[]`.
- Future-compatible path: `qc.checks[]`.
- Suggested check id: `release_publication_execution_window_state_v1`.

## Release Publication Gate Set QC Attachment Point

- `RELEASE_PUBLICATION_GATE_SET_v1` validates deterministic completeness/order of release publication gate evidence already attached to the manifest.
- Current manifest attachment path: `qc.gates[]`.
- Future-compatible path: `qc.checks[]`.
- Suggested check id: `release_publication_gate_set_v1`.
