...
The Editor layout/bootstrap/window lifecycle deep-dive diagnostic is invoked with:

```powershell
python tools/o3de/editor_smoke.py --manifest examples/manifests/release_rigged.pass.example.json --diagnose-editor-layout-bootstrap-window-lifecycle --strict-integration --editor-render-capture-rhi dx12 --editor-executable C:/src/o3de/build/windows/bin/profile/Editor.exe --project C:/Users/topgu/O3DE/Projects/MAXINE_GoldenCorpus --engine-root C:/src/o3de --apb-report artifacts/o3de-integration/apb/<run>/asset_processor_batch_live_report.json
```

The diagnostic mode is `editor-layout-bootstrap-window-lifecycle-deep-dive`. It preserves verified AP alignment and Editor/AP negotiation, selected non-null `dx12`, visible desktop/session and GPU/RHI readiness, and the safe temp visual scene/display context under `Levels/_maxine_visual_smoke/editor_safe_temp_visual_scene_display_context`.

This mode source-validates `CryEdit.cpp` launch and lifecycle behavior for `--runpython`, `--autotest_mode`, `--skipWelcomeScreenDialog`, `RunInitPythonScript`, `MainWindow::show/raise/update/setFocus`, `QtViewPaneManager::RestoreLayout`, `QTimer::singleShot`, `NotifyEditorInitialized`, and `app->exec`. It classifies the sanitized launch flags without emitting raw command lines or script paths, records whether automation timing or shell-suppression flags are present, and distinguishes source validation from runtime shell proof.

Layout mutation remains blocked. The diagnostic may read bounded, sanitized ViewPane registration/open-state signals such as `azlmbr.legacy.general.get_pane_class_names` and `is_pane_visible`, but it does not reset layouts, write user settings, force hidden windows visible, open screenshot capture, or mutate project/defaultlevel/production content. A run-scoped layout override remains blocked unless a later slice source-validates a reversible, non-destructive, run-owned path.

When lifecycle preconditions allow, the diagnostic reruns visible-shell classification after lifecycle waits, default viewport pane/widget readiness inventory, and readiness-only active/default viewport, Atom SwapChain, and FrameCapture target probes. No screenshot request is made. This diagnostic does not complete screenshot capture, does not verify rendered visual evidence, does not verify material correctness, does not verify character visual presence, does not satisfy `visual_material`, does not prove full runtime character behavior, and does not perform release packaging or publication. Expected blocked states include `blocked_by_editor_layout_bootstrap_lifecycle_source_validation_unavailable`, `blocked_by_editor_automation_script_before_shell_ready`, `blocked_by_editor_shell_ready_event_unavailable`, `blocked_by_editor_layout_restore_state_unavailable`, `blocked_by_viewpane_registration_unavailable`, `blocked_by_default_viewport_viewpane_registration_unavailable`, `blocked_by_visible_editor_shell_lifecycle_unavailable`, `blocked_by_editor_active_viewport_window_handle_unavailable`, `blocked_by_active_viewport_window_handle_unavailable`, `blocked_by_swapchain_probe_unavailable`, and `blocked_by_framecapture_target_unavailable`.

Current layout/bootstrap/window lifecycle evidence is `artifacts/o3de-integration/editor-smoke/editor-smoke-20260516T133830Z/editor_smoke_live_report.json` with progress log `artifacts/o3de-integration/editor-smoke/editor-smoke-20260516T133830Z/progress.jsonl`. AP alignment and Editor/AP negotiation remained verified with selected `dx12`, safe temp cleanup completed under `Levels/_maxine_visual_smoke/editor_safe_temp_visual_scene_display_context/editor_safe_temp_visual_scene_display_context_20260516t133830z`, and no defaultlevel/production mutation was detected. The live run classified sanitized launch flags as `--runpython`, `--autotest_mode`, `--skipWelcomeScreenDialog`, non-null `dx12`, and shell-suppression active, with no raw command line, script path, or environment dump emitted. Lifecycle readiness remains blocked by `blocked_by_editor_automation_script_before_shell_ready`; initialized and shell-ready waits remain unavailable, layout/user layout mutation was not attempted, and run-scoped layout override remains blocked by `blocked_by_run_scoped_layout_override_unavailable`. ViewPane registration and default viewport ViewPane registration verified through sanitized count-only evidence, but default viewport pane open/discovery remains blocked by `blocked_by_default_viewport_pane_unavailable`. Visible Editor shell after lifecycle wait remains blocked by `blocked_by_visible_editor_shell_unavailable`, visible shell lifecycle materialization by `blocked_by_visible_editor_shell_lifecycle_unavailable`, active/default viewport window-handle by `blocked_by_editor_active_viewport_window_handle_unavailable`, Atom SwapChain by `blocked_by_swapchain_probe_unavailable`, and FrameCapture target by `blocked_by_active_viewport_window_handle_unavailable`.

If this mode verifies a safe visible Editor shell and a viewport window handle, SwapChain, or FrameCapture target becomes available, the next slice is bounded screenshot request/completion proof in the safe temp visual scene. If it shows automation script execution occurs before shell readiness, the next slice is Editor bootstrap wait / shell-ready synchronization. If it shows layout suppression or missing view-pane registration, the next slice is Editor layout/view-pane registration repair.

## Synchronize Editor bootstrap wait with shell-ready lifecycle

The Editor bootstrap wait / shell-ready synchronization diagnostic is invoked with:

```powershell
python tools/o3de/editor_smoke.py --manifest examples/manifests/release_rigged.pass.example.json --diagnose-editor-bootstrap-wait-shell-ready-synchronization --strict-integration --editor-render-capture-rhi dx12 --editor-executable C:/src/o3de/build/windows/bin/profile/Editor.exe --project C:/Users/topgu/O3DE/Projects/MAXINE_GoldenCorpus --engine-root C:/src/o3de --apb-report artifacts/o3de-integration/apb/<run>/asset_processor_batch_live_report.json
```

The diagnostic mode is `editor-bootstrap-wait-shell-ready-synchronization`. It preserves verified AP alignment and Editor/AP negotiation, selected non-null `dx12`, visible desktop/session and GPU/RHI readiness, and the safe temp visual scene/display context under `Levels/_maxine_visual_smoke/editor_safe_temp_visual_scene_display_context`.

This mode source-validates the same `CryEdit.cpp` lifecycle surfaces from the layout/bootstrap slice and extends the investigation to deferred and late-runner candidates: `RunInitPythonScript`, `EditorPythonRunnerRequestBus::Broadcast`, `QTimer::singleShot`, `NotifyEditorInitialized`, `app->EnableOnIdle`, `app->exec`, and the read-only `azlmbr.legacy.general.idle_wait` / `idle_wait_frames` bindings. It records sanitized launch flag classification for `--runpython`, `--autotest_mode`, `--skipWelcomeScreenDialog`, and selected non-null RHI behavior without emitting raw command lines, script paths, environment dumps, window titles, raw object names, or native handles.

The shell-ready synchronization strategy is conservative. Source validation can identify candidate timing surfaces, but late diagnostic execution is verified only if a bounded diagnostic actually runs past the pre-shell `--runpython` boundary. QTimer, event-loop, NotifyEditorInitialized, app->exec, and shell-ready waits remain typed readiness checks; they are not visible shell proof, viewport proof, screenshot proof, rendered evidence, or material correctness proof.

No layout/user-setting mutation is attempted. A run-scoped layout override remains blocked unless a later slice source-validates a reversible, non-destructive, run-owned path. Hidden window show/raise/activate remains blocked unless the exact candidate is source-validated as the valid Editor shell and the action is bounded, non-destructive, reversible, and readiness only.

When source-validated preconditions allow, the diagnostic reruns visible-shell classification after the synchronization attempt, default viewport pane/widget readiness inventory, bounded idle/render wait only after safe shell and viewport-pane preconditions, and readiness-only active/default viewport, Atom SwapChain, and FrameCapture target probes. No screenshot request is made. This diagnostic does not complete screenshot capture, does not verify rendered visual evidence, does not verify material correctness, does not verify character visual presence, does not satisfy `visual_material`, does not prove full runtime character behavior, and does not perform release packaging or publication. Expected blocked states include `blocked_by_editor_bootstrap_wait_shell_ready_source_validation_unavailable`, `blocked_by_editor_shell_ready_synchronization_unavailable`, `blocked_by_editor_deferred_diagnostic_strategy_unavailable`, `blocked_by_editor_late_diagnostic_execution_unavailable`, `blocked_by_editor_qtimer_shell_ready_strategy_unavailable`, `blocked_by_editor_event_loop_posted_callback_unavailable`, `blocked_by_editor_notify_initialized_wait_unavailable`, `blocked_by_editor_app_exec_boundary_wait_unavailable`, `blocked_by_editor_shell_ready_event_unavailable`, `blocked_by_editor_automation_script_before_shell_ready`, `blocked_by_visible_editor_shell_unavailable`, `blocked_by_default_viewport_pane_unavailable`, `blocked_by_editor_active_viewport_window_handle_unavailable`, `blocked_by_active_viewport_window_handle_unavailable`, `blocked_by_swapchain_probe_unavailable`, and `blocked_by_framecapture_target_unavailable`.

Current shell-ready synchronization evidence is `artifacts/o3de-integration/editor-smoke/editor-smoke-20260516T171937Z/editor_smoke_live_report.json` with progress log `artifacts/o3de-integration/editor-smoke/editor-smoke-20260516T171937Z/progress.jsonl`. AP alignment and Editor/AP negotiation remained verified with selected `dx12`; safe temp cleanup completed under `Levels/_maxine_visual_smoke/editor_safe_temp_visual_scene_display_context/editor_safe_temp_visual_scene_display_context_20260516t171937z`; no defaultlevel, production-level, or production character mutation was detected; and no Asset Cache deletion or AP database/cache wipe was attempted. The live diagnostic source-validated the shell-ready synchronization surfaces, preserved PR #180's `verified_automation_script_executes_before_shell_ready` timing classification, and recorded `blocked_by_editor_shell_ready_synchronization_unavailable` with late diagnostic execution unverified as `blocked_by_editor_late_diagnostic_execution_unavailable`. Default viewport ViewPane registration preservation and default viewport widget discovery after shell-ready were verified, but default viewport pane discovery remained blocked by `blocked_by_default_viewport_pane_unavailable`, visible Editor shell after shell-ready by `blocked_by_visible_editor_shell_unavailable`, active/default viewport window-handle by `blocked_by_editor_active_viewport_window_handle_unavailable`, Atom SwapChain by `blocked_by_swapchain_probe_unavailable`, and FrameCapture target by `blocked_by_active_viewport_window_handle_unavailable`.

If shell-ready synchronization verifies and a viewport window handle, SwapChain, or FrameCapture target becomes available, the next slice is bounded screenshot request/completion proof in the safe temp visual scene. If synchronization verifies but the viewport handle remains blocked, the next slice is active/default viewport handle readiness deep-dive. If the late diagnostic strategy source-validates but does not execute, the next slice is Editor deferred diagnostic execution repair. If shell-ready synchronization remains blocked by `--runpython` / `--autotest_mode`, the next slice is Editor launch mode / automation-shell suppression repair.

## Repair Editor deferred diagnostic execution past shell-ready boundary

The Editor deferred diagnostic execution repair diagnostic is invoked with:

```powershell
python tools/o3de/editor_smoke.py --manifest examples/manifests/release_rigged.pass.example.json --diagnose-editor-deferred-diagnostic-execution-repair --strict-integration --editor-render-capture-rhi dx12 --editor-executable C:/src/o3de/build/windows/bin/profile/Editor.exe --project C:/Users/topgu/O3DE/Projects/MAXINE_GoldenCorpus --engine-root C:/src/o3de --apb-report artifacts/o3de-integration/apb/<run>/asset_processor_batch_live_report.json
```

The diagnostic mode is `editor-deferred-diagnostic-execution-repair`. It preserves verified AP alignment and Editor/AP negotiation, selected non-null `dx12`, visible desktop/session and GPU/RHI readiness, and the safe temp visual scene/display context under `Levels/_maxine_visual_smoke/editor_safe_temp_visual_scene_display_context`.

This mode source-validates the existing `CryEdit.cpp` and `CryEditPy.cpp` execution surfaces before it classifies a repair attempt: `RunInitPythonScript`, `EditorPythonRunnerRequestBus::Broadcast`, `QTimer::singleShot`, `NotifyEditorInitialized`, `app->EnableOnIdle`, `app->exec`, `QEventLoop`, and the read-only `azlmbr.legacy.general.idle_wait` / `idle_wait_frames` bindings. It preserves PR #180 and PR #181 findings that the prior automation path executes before shell-ready, then records whether a bounded late-runner/deferred/two-stage diagnostic execution path can actually emit sanitized run-owned progress and completion markers.

The repair strategy is readiness-only and marker-disciplined. Candidate QTimer/event-loop/idle/two-stage mechanisms are not counted as execution proof unless a late diagnostic completion marker is observed from a run-owned path. Timeout and cleanup are bounded; missing markers are typed blockers, not failures hidden as success. No raw command lines, script paths, environment dumps, window titles, raw object names, native handles, private logs, screenshots, generated products, or private live project content are committed.

No layout/user-setting mutation is attempted. Run-scoped layout override and hidden window show/raise/activate remain blocked unless a later slice source-validates a reversible, non-destructive, run-owned path and the exact target is proven safe. Any create/open/save/delete/cleanup action remains constrained to the approved temp root, and cleanup may remove only run-owned temp scene files created under `Levels/_maxine_visual_smoke`.

When late diagnostic execution and source-validated preconditions allow, the diagnostic reruns visible Editor shell, default viewport pane/widget readiness, and readiness-only active/default viewport, Atom SwapChain, and FrameCapture target probes after the late-execution boundary. No screenshot request is made. This diagnostic does not complete screenshot capture, does not verify rendered visual evidence, does not verify material correctness, does not verify character visual presence, does not satisfy `visual_material`, does not prove full runtime character behavior, and does not perform release packaging or publication. Expected blocked states include `blocked_by_editor_deferred_diagnostic_execution_repair_source_validation_unavailable`, `blocked_by_editor_deferred_diagnostic_strategy_unavailable`, `blocked_by_editor_late_diagnostic_scheduling_unavailable`, `blocked_by_editor_late_diagnostic_execution_unavailable`, `blocked_by_editor_late_diagnostic_completion_marker_missing`, `blocked_by_editor_late_diagnostic_progress_marker_missing`, `blocked_by_editor_qtimer_late_diagnostic_unavailable`, `blocked_by_editor_event_loop_late_callback_unavailable`, `blocked_by_editor_idle_callback_late_diagnostic_unavailable`, `blocked_by_editor_two_stage_diagnostic_unavailable`, `blocked_by_editor_stage2_late_execution_unavailable`, `blocked_by_editor_stage2_completion_marker_missing`, `blocked_by_editor_shell_ready_synchronization_unavailable`, `blocked_by_visible_editor_shell_unavailable`, `blocked_by_default_viewport_pane_unavailable`, `blocked_by_editor_active_viewport_window_handle_unavailable`, `blocked_by_active_viewport_window_handle_unavailable`, `blocked_by_swapchain_probe_unavailable`, and `blocked_by_framecapture_target_unavailable`.

Current deferred diagnostic execution repair evidence is `artifacts/o3de-integration/editor-smoke/editor-smoke-20260517T011956Z/editor_smoke_live_report.json` with progress log `artifacts/o3de-integration/editor-smoke/editor-smoke-20260517T011956Z/progress.jsonl`. AP alignment and Editor/AP negotiation remained verified with selected `dx12`; safe temp cleanup completed under `Levels/_maxine_visual_smoke/editor_safe_temp_visual_scene_display_context/editor_safe_temp_visual_scene_display_context_20260517t011956z`; no defaultlevel, production-level, or production character mutation was detected; and no Asset Cache deletion or AP database/cache wipe was attempted. Source validation passed, but repair remained blocked by `blocked_by_editor_late_diagnostic_execution_unavailable`: late scheduling was unverified with `blocked_by_editor_late_diagnostic_scheduling_unavailable`, the completion marker was missing with `blocked_by_editor_late_diagnostic_completion_marker_missing`, the progress marker was missing with `blocked_by_editor_late_diagnostic_progress_marker_missing`, timeout was bounded and reported, cleanup completed, and run-owned paths were sanitized. Because late diagnostic execution did not verify, after-late visible shell, default viewport pane/widget, active/default viewport, Atom SwapChain, and FrameCapture target probes were not selected beyond readiness blockers.

If deferred/late diagnostic execution verifies and a viewport window handle, SwapChain, or FrameCapture target becomes available, the next slice is bounded screenshot request/completion proof in the safe temp visual scene. If late execution verifies but the default viewport pane/widget remains blocked, the next slice is Editor default viewport pane/widget deep-dive. If the deferred strategy source-validates but live execution still does not occur, the next slice is Editor late-runner mechanism deep-dive v2.

## Deep-dive Editor late-runner mechanism and stage-two execution

The Editor late-runner mechanism deep-dive diagnostic is invoked with:

```powershell
python tools/o3de/editor_smoke.py --manifest examples/manifests/release_rigged.pass.example.json --diagnose-editor-late-runner-mechanism-deep-dive --strict-integration --editor-render-capture-rhi dx12 --editor-executable C:/src/o3de/build/windows/bin/profile/Editor.exe --project C:/Users/topgu/O3DE/Projects/MAXINE_GoldenCorpus --engine-root C:/src/o3de --apb-report artifacts/o3de-integration/apb/<run>/asset_processor_batch_live_report.json
```

The diagnostic mode is `editor-late-runner-mechanism-deep-dive`. It preserves verified AP alignment and Editor/AP negotiation, selected non-null `dx12`, visible desktop/session and GPU/RHI readiness, and the safe temp visual scene/display context under `Levels/_maxine_visual_smoke/editor_safe_temp_visual_scene_display_context`.

This mode source-validates the PR #182 deferred repair surfaces and then narrows the investigation to stage-two execution mechanics: QTimer callback candidates, event-loop callbacks, idle callbacks, module-global callback anchoring, Python object lifetime classification, stage-two script import/entrypoint checks, marker path writability/visibility, Editor alive wait classification, bounded timeout, and run-owned cleanup. Marker/progress/completion files are allowed only under the run-owned editor-smoke artifact directory and are reported through sanitized booleans and typed blockers, not raw private paths.

The deep-dive does not treat QTimer availability, module-global anchoring, marker path writability, stage-two scheduling, or stage-one pre-shell execution as stage-two execution proof. Stage-two late execution is verified only when sanitized stage-two progress/completion marker evidence proves the diagnostic actually ran past the pre-shell `--runpython` timing boundary. Expected blocked states include `blocked_by_editor_late_runner_mechanism_source_validation_unavailable`, `blocked_by_editor_stage2_scheduling_unavailable`, `blocked_by_editor_stage2_late_execution_unavailable`, `blocked_by_editor_stage2_progress_marker_missing`, `blocked_by_editor_stage2_completion_marker_missing`, `blocked_by_editor_late_runner_timeout`, `blocked_by_editor_late_runner_editor_alive_wait_unavailable`, `blocked_by_editor_shell_ready_synchronization_unavailable`, `blocked_by_visible_editor_shell_unavailable`, `blocked_by_default_viewport_pane_unavailable`, `blocked_by_editor_active_viewport_window_handle_unavailable`, `blocked_by_active_viewport_window_handle_unavailable`, `blocked_by_swapchain_probe_unavailable`, and `blocked_by_framecapture_target_unavailable`.

No layout/user-setting mutation is attempted. Run-scoped layout override and hidden window show/raise/activate remain blocked unless a later slice source-validates a reversible, non-destructive, run-owned path and the exact target is proven safe. Any create/open/save/delete/cleanup action remains constrained to the approved temp root, and cleanup may remove only run-owned temp scene files created under `Levels/_maxine_visual_smoke`.

When stage-two execution and source-validated preconditions allow, the diagnostic reruns visible Editor shell, default viewport pane/widget readiness, and readiness-only active/default viewport, Atom SwapChain, and FrameCapture target probes after the stage-two boundary. No screenshot request is made. This diagnostic does not complete screenshot capture, does not verify rendered visual evidence, does not verify material correctness, does not verify character visual presence, does not satisfy `visual_material`, does not prove full runtime character behavior, and does not perform release packaging or publication. No Asset Cache deletion or AP database/cache wipe is allowed.

If stage-two late diagnostic execution verifies and a viewport window handle, SwapChain, or FrameCapture target becomes available, the next slice is bounded screenshot request/completion proof in the safe temp visual scene. If stage-two execution verifies but viewport handle remains blocked, the next slice is active/default viewport handle readiness deep-dive. If stage-two scheduling verifies but stage-two execution still does not occur, the next slice is Editor stage-two execution repair. If callback object lifetime is the blocker, the next slice is Editor Python callback lifetime anchoring repair.

## Repair Editor stage-two late diagnostic execution

The Editor stage-two execution repair diagnostic is invoked with:

```powershell
python tools/o3de/editor_smoke.py --manifest examples/manifests/release_rigged.pass.example.json --diagnose-editor-stage-two-execution-repair --strict-integration --editor-render-capture-rhi dx12 --editor-executable C:/src/o3de/build/windows/bin/profile/Editor.exe --project C:/Users/topgu/O3DE/Projects/MAXINE_GoldenCorpus --engine-root C:/src/o3de --apb-report artifacts/o3de-integration/apb/<run>/asset_processor_batch_live_report.json
```

The diagnostic mode is `editor-stage-two-execution-repair`. It preserves verified AP alignment and Editor/AP negotiation, selected non-null `dx12`, visible desktop/session and GPU/RHI readiness, and the safe temp visual scene/display context under `Levels/_maxine_visual_smoke/editor_safe_temp_visual_scene_display_context`.

This mode extends the late-runner source boundary and records stage-two scheduler selection, callable reference, argument propagation, marker-path propagation, direct-control invocation, stage-two scheduling, late execution, progress/completion markers, Editor-alive wait, harness wait, bounded timeout, and cleanup as separate typed fields. Direct-control invocation is control proof only and is explicitly not late-execution proof.

Run-owned marker/progress paths remain sanitized under `artifacts/o3de-integration/editor-smoke/editor-smoke-<timestamp>Z/`; raw command lines, raw environment dumps, raw window titles, raw object names, screenshots, and private logs are not emitted. Layout/user-setting mutation remains blocked unless a later slice source-validates a reversible run-scoped path.

When stage-two execution and source-validated preconditions allow, the diagnostic reruns visible Editor shell, default viewport pane/widget readiness, and readiness-only active/default viewport, Atom SwapChain, and FrameCapture target probes after stage two. No screenshot request is made. This diagnostic does not complete screenshot capture, does not verify rendered visual evidence, does not verify material correctness, does not verify character visual presence, does not satisfy `visual_material`, does not prove full runtime character behavior, and does not perform release packaging or publication. No Asset Cache deletion or AP database/cache wipe is allowed.

Expected blocked states include `blocked_by_editor_stage_two_execution_repair_source_validation_unavailable`, `blocked_by_editor_stage_two_scheduler_selection_unavailable`, `blocked_by_editor_stage_two_callable_reference_unavailable`, `blocked_by_editor_stage_two_argument_propagation_unavailable`, `blocked_by_editor_stage_two_marker_path_propagation_unavailable`, `blocked_by_editor_stage_two_direct_control_invocation_unavailable`, `blocked_by_editor_stage_two_scheduling_unavailable`, `blocked_by_editor_stage_two_late_execution_unavailable`, `blocked_by_editor_stage_two_progress_marker_missing`, `blocked_by_editor_stage_two_completion_marker_missing`, `blocked_by_editor_stage_two_editor_alive_wait_unavailable`, `blocked_by_editor_stage_two_harness_wait_unavailable`, `blocked_by_editor_stage_two_timeout`, `blocked_by_visible_editor_shell_unavailable`, `blocked_by_default_viewport_pane_unavailable`, `blocked_by_editor_active_viewport_window_handle_unavailable`, `blocked_by_active_viewport_window_handle_unavailable`, `blocked_by_swapchain_probe_unavailable`, and `blocked_by_framecapture_target_unavailable`.

Current stage-two execution repair evidence is `artifacts/o3de-integration/editor-smoke/editor-smoke-20260517T135516Z/editor_smoke_live_report.json` with progress log `artifacts/o3de-integration/editor-smoke/editor-smoke-20260517T135516Z/progress.jsonl`. AP alignment and Editor/AP negotiation remained verified with selected `dx12`; desktop/session + GPU/driver + RHI readiness remained preserved; safe temp cleanup completed under `Levels/_maxine_visual_smoke/editor_safe_temp_visual_scene_display_context/editor_safe_temp_visual_scene_display_context_20260517t135516z`; no defaultlevel, production-level, or production character mutation was detected; and no Asset Cache deletion or AP database/cache wipe was attempted. Stage-two scheduler selection/scheduling remained blocked by `blocked_by_editor_stage_two_scheduler_selection_unavailable` / `blocked_by_editor_stage_two_scheduling_unavailable`, late execution by `blocked_by_editor_stage_two_late_execution_unavailable`, progress marker by `blocked_by_editor_stage_two_progress_marker_missing`, and completion marker by `blocked_by_editor_stage_two_completion_marker_missing`.
