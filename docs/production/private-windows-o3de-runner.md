...
This diagnostic may mutate only the approved source prefab path through source-backed Editor/prefab APIs. It focuses the owning prefab, switches to the parent focus/link context, applies Actor + Simple Motion component overrides, restores focus, saves the approved source prefab, and claims success only if parsed source-template marker evidence finds both `ActorAsset` and `MotionAsset`. It must not hand-author unknown JSON, run APB/runtime proof without persisted markers, claim runtime component wiring from Editor-only evidence, or claim animation/full-character proof.
