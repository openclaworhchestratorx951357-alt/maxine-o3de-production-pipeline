...
This diagnostic selects the source-backed `InstanceToTemplateInterface::GenerateEntityDomBySerializing` + `GeneratePatch` + `PatchEntityInTemplate` route when the parent-link override path is unavailable. It may claim source-template persistence only when the saved approved source prefab changes in the current run and parsed marker evidence finds both `ActorAsset` and `MotionAsset`. A positive source-template result still does not claim APB regeneration, runtime Actor/Simple Motion TypeIds, runtime asset assignments, animation playback, full runtime character behavior, publication, or release packaging.
