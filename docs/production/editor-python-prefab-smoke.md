...
This diagnostic implements the next bounded bridge route under `azlmbr.maxine.prefab_bridge.apply_approved_source_prefab_parent_link_component_overrides`. Before any focus or override call, the route verifies that the supplied entity is owned by the approved source prefab with `PrefabPublicInterface::GetOwningInstancePrefabPath` / `PrefabLoaderInterface::GetFullPath`, then verifies the Actor and Simple Motion component IDs are attached to that entity with `ComponentApplicationRequests::FindEntity` and `Entity::FindComponent`. After that ownership gate, it focuses the prefab instance owning the edited approved entity, focuses the parent of that instance so O3DE can derive a link-context path, calls `PrefabOverridePublicInterface::ApplyComponentOverrides` for the Actor and Simple Motion component references, and restores focus to the edited prefab instance. The route is still approved-source-only and must not hand-author unknown component JSON.

Verification remains marker-backed and freshness-gated. A callable route and successful override status only prove the parent/link route surface, and stale pre-existing markers are not proof. The diagnostic may claim source-template propagation only if the saved approved source prefab changed in the current run and parses with `ActorAsset` and `MotionAsset` marker evidence; APB/runtime TypeId proof, animation playback, and full runtime character proof remain false until those downstream gates actually run.
...
Approved source-prefab source-backed override-path/template-update diagnostic:
...
This diagnostic preserves the parent-link route and adds the narrower source-backed template update route `azlmbr.maxine.prefab_bridge.apply_approved_source_prefab_override_path_generation_template_update`. The route first verifies that the target entity belongs to the approved source prefab and that the Actor/Simple Motion component IDs belong to that entity. It then serializes the changed entity through `InstanceToTemplateInterface::GenerateEntityDomBySerializing`, generates a prefab patch with `GeneratePatch`, applies it to the owning source template with `PatchEntityInTemplate`, and saves through `PrefabPublicInterface::SavePrefab`.

The route is still approved-source-only and does not hand-author unknown component JSON. Success requires all of: ownership verification, source-backed patch application, route-specific rejection probes that call `apply_approved_source_prefab_override_path_generation_template_update` directly, `approved_source_prefab_changed_this_run=true` / before-after hash inequality, parsed `ActorAsset` and `MotionAsset` markers in the saved source template, defaultlevel/production mutation false, and no generated product/cache proof. Legacy `save_approved_source_prefab_wiring` rejection probes remain preservation evidence only and are not accepted as the new template-update route safety proof. The first positive live source-template result is `artifacts/o3de-integration/editor-smoke/editor-smoke-20260514T054159Z/editor_smoke_live_report.json`; it modifies only `examples/o3de-golden-project/source/Assets/Characters/MAXINE_GoldenCorpus/prefabs/release_rigged.prefab` with Editor Actor and Simple Motion components. APB/runtime TypeId proof, animation playback, and full runtime character proof remain false until downstream diagnostics actually run.
