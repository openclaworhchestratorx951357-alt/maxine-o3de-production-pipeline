...
The route is still approved-source-only and does not hand-author unknown component JSON. Success requires all of: ownership verification, source-backed patch application, `approved_source_prefab_changed_this_run=true` / before-after hash inequality, parsed `ActorAsset` and `MotionAsset` markers in the saved source template, defaultlevel/production mutation false, and no generated product/cache proof. The first positive live source-template result is `artifacts/o3de-integration/editor-smoke/editor-smoke-20260514T054159Z/editor_smoke_live_report.json`; it modifies only `examples/o3de-golden-project/source/Assets/Characters/MAXINE_GoldenCorpus/prefabs/release_rigged.prefab` with Editor Actor and Simple Motion components. APB/runtime TypeId proof, animation playback, and full runtime character proof remain false until downstream diagnostics actually run.
