...
Runtime approved character prefab-source diagnostics stay in that same runtime-harness lane. They inspect the repo-owned reviewed source `examples/o3de-golden-project/source/Assets/Characters/MAXINE_GoldenCorpus/prefabs/release_rigged.prefab`, verify that it is not defaultlevel, production, temp-only, or generic Transform-only content, and record the expected generated `.spawnable` product separately from runtime loading. A missing generated spawnable is an APB/source-surface blocker, not an Editor stall. A generated spawnable is still only runtime-equivalent surface evidence until a bounded runtime fixture load/readiness check passes.
