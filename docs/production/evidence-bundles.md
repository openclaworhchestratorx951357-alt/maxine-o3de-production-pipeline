...
Approved source-template propagation/apply-step evidence sits between Editor instance mutation and APB/runtime proof. Required fields include `approved_source_prefab_propagation_apply_step_attempted`, completed/verified/blocker, source-validation refs, candidate matrix, selected strategy, propagation API used, component override detection/application flags, entity-change commit flag, template-DOM update flag, source-prefab before/after hashes, persisted `ActorAsset` / `MotionAsset` marker booleans, and preserved runtime proof flags. Source validation alone may pin that `PrefabOverridePublicInterface::ApplyComponentOverrides` requires a focused parent/link context and may report `blocked_by_prefab_instance_to_template_propagation_requires_parent_link_context`; it must not claim source-prefab modification, persisted marker proof, APB regeneration, runtime component wiring, animation playback, or full runtime character proof until those actions actually run and pass.

Approved source-template parent-focus/link-context override apply evidence is the bounded implementation layer above the propagation/apply source-validation blocker. Required fields include `approved_source_prefab_parent_link_override_apply_route_attempted`, completed/verified/blocker, source-validation refs, candidate matrix, selected strategy, parent focus required/available/applied/restored booleans, link context required/available evidence, component override path detection, component override application, `PushOverridesToPrefab` attempt/verification, source-template DOM update, approved source-prefab before/after hashes, `ActorAsset` / `MotionAsset` marker booleans, and preserved APB/runtime proof flags. A bundle may set `approved_source_prefab_parent_link_override_apply_route_verified=true` only when the route is source-backed, focus is restored, the approved source prefab is saved, and parsed source-template marker evidence verifies both Actor and Motion asset serialization. Route callability or component override status alone must not claim APB regeneration, runtime component wiring, animation playback, or full runtime character proof.

If the route establishes and restores parent focus but no link/component override path is available, the bundle must keep source-template marker proof false and use a typed blocker such as `blocked_by_prefab_link_context_unavailable`. In that state the approved source prefab must remain unchanged or be restored to its original hash before reporting.
