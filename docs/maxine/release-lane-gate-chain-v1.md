...
| 6 | `screenshot_evidence_v1` | visual evidence completeness | not implemented on `main` | not implemented on `main` | not implemented on `main` | planned `qc.gates[]`/`qc.checks[]` | all | future extractor | no | yes | pending |
| 7 | `manual_hero_review_v1` | human decision gate for hero assets | not implemented on `main` | not implemented on `main` | not implemented on `main` | planned `qc.gates[]`/`qc.checks[]` | hero | manual review | no | yes | pending |
| 8 | `ci_artifact_retention_v1` | CI evidence retention policy | not implemented on `main` | not implemented on `main` | not implemented on `main` | planned `qc.gates[]`/`qc.checks[]` | all | future CI evidence | no | yes | pending |
| 9 | `release_package_bundle_v1` | release package bundle integrity | not implemented on `main` | not implemented on `main` | not implemented on `main` | planned `qc.gates[]`/`qc.checks[]` | all | fixture/manual report | no | yes | pending |
| 10 | `release_promotion_decision_v1` | promotion decision policy gate | not implemented on `main` | not implemented on `main` | not implemented on `main` | planned `qc.gates[]`/`qc.checks[]` | all | manual report | no | yes | pending |
...
| 16 | `release_publication_rollback_drill_v1` | rollback drill proof | not implemented on `main` | not implemented on `main` | not implemented on `main` | planned `qc.gates[]`/`qc.checks[]` | all | fixture/manual report | no | yes | pending |
...
| 19 | `release_publication_ready_for_execution_request_v1` | ready-for-execution request readiness | not implemented on `main` | not implemented on `main` | not implemented on `main` | planned `qc.gates[]`/`qc.checks[]` | all | fixture/manual report | no | yes | pending |
