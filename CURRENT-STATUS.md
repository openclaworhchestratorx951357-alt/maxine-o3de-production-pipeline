...
- Implement Parent-Focus / Link-Context Prefab Override Apply Route v1 is implemented on `codex/implement-parent-focus-link-context-prefab-override-apply-route-v1` and pending review:
  ...
  - source-template marker proof remains the authority: `approved_source_prefab_parent_link_override_apply_route_verified`, propagation/apply verification, and Actor + Simple Motion wiring verification may become true only when the saved source template contains parsed `ActorAsset` and `MotionAsset` marker evidence; route callability or override status alone is insufficient
