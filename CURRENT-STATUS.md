...
- Resolve Runtime Shutdown PoolAllocator Assertions v1 is implemented on `codex/resolve-runtime-shutdown-poolallocator-assertions-v1` and pending review:
  ...
  - this slice adds the runtime harness source-only diagnostic `--diagnose-runtime-shutdown-poolallocator-assertions` and the live after-APB fixture entry point `--enable-runtime-poolallocator-signal-classification-fixture`
  ...
  - the live rerun now records `runtime_character_animation_component_wiring_claimed=true` and `runtime_character_animation_component_wiring_verified=true` for bounded Actor + Simple Motion component wiring only; animation playback remains unattempted/unverified, and full runtime character proof remains false
  - the overall live fixture still exits fail because the playback-surface diagnostic is blocked by `blocked_by_runtime_animation_playback_probe_requires_playback_execution_api`; this is the next production slice candidate and is not animation playback proof
