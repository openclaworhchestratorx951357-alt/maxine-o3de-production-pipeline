...
- Add Approved Runtime Character Prefab Source v1 is in progress on `codex/add-approved-runtime-character-prefab-source-v1`:
  - PR #140 was merged into `main` at `8643a219eb0e8ca808c18bc495ca4f7e74c5ac49`, and this branch was created from updated `main`
  - this slice adds a repo-owned reviewed source prefab at `examples/o3de-golden-project/source/Assets/Characters/MAXINE_GoldenCorpus/prefabs/release_rigged.prefab`; it references the approved release `.procprefab` `assets/characters/maxine/release/maxine_idle_fbx.procprefab` and is not a generated product/cache artifact
  - `tools/o3de/runtime_harness.py` now records `--diagnose-runtime-character-prefab-source` as a source-contract diagnostic layered under the PR #140 approved spawnable-surface search; it validates repo ownership, non-defaultlevel/non-production/non-temp status, non-Transform-only character specificity, source refs for the Prefab Builder path, and the expected APB product `pc/assets/characters/maxine_goldencorpus/prefabs/release_rigged.spawnable`
  - `tools/o3de/asset_processor_batch.py` now stages that repo-owned prefab source into the live project `Assets` scanfolder only when `MAXINE_ALLOW_RUNTIME_CHARACTER_PREFAB_SOURCE_GENERATION=1`; it refuses to overwrite mismatched existing live project source and records reversible staging evidence in the APB report
  - `examples/o3de-golden-project/maxine-golden-project.fixture.json` now names the approved prefab source, project target path, approved dependency, expected spawnable product/catalog path, builder `Prefabs`, and spawnable AssetType `{855E3021-D305-4845-B284-20C3F7FDF16B}`
  - PR #140 remains preserved: no approved release-rigged character-specific `.spawnable` existed before this source was added; level/defaultlevel/temp/generic spawnables remain rejected and cannot satisfy the runtime-equivalent character surface contract
  - PR #139 remains preserved: direct `.procprefab` runtime AssetManager load is unsupported/builder-only through `AZ::Prefab::ProceduralPrefabAsset` and `AZ::Prefab::PrefabGroupAssetHandler` in `Gem::PrefabBuilder.Builders` / tools, with direct runtime `.procprefab` proof unclaimed
  - live APB evidence now stages the repo-owned source under the explicit `MAXINE_ALLOW_RUNTIME_CHARACTER_PREFAB_SOURCE_GENERATION=1` gate and reports the expected approved character `.spawnable` product at `pc/assets/characters/maxine_goldencorpus/prefabs/release_rigged.spawnable` / `assets/characters/maxine_goldencorpus/prefabs/release_rigged.spawnable`; the latest passing APB report is `artifacts/o3de-integration/apb/apb-live-20260513T052655Z/asset_processor_batch_live_report.json`, with `cache_heuristic_used=false`, missing products empty, and pending products empty
  - `--diagnose-runtime-character-prefab-source` and `--diagnose-runtime-character-spawnable-surface` now find the approved source-generated character spawnable while still rejecting level/defaultlevel/temp/generic non-character candidates
  - the bounded no-defaultlevel product-load fixture safely loaded the prior seven products plus the approved character spawnable ready, wrote `artifacts/o3de-integration/runtime-harness/runtime-character-product-load-live-report.json`, exited `0`, kept `runtime_cache_bootstrap_verified=true`, `runtime_signal_classification_verified=true`, `runtime_default_level_autoload_detected=false`, `runtime_production_level_loaded=false`, and now records `runtime_character_product_load_verified=true` under the updated contract
  - runtime instantiation/spawn, runtime animation, and full runtime character proof remain unclaimed despite the product-load proof; publication remains blocked, release packaging remains blocked, production/defaultlevel mutation remains forbidden, Asset Cache deletion remains forbidden, cache heuristic release proof remains forbidden, and no production-ready release status is claimed
