...
- Pin Effective Runtime LoadLevel Override v1 is implemented on `codex/pin-effective-runtime-loadlevel-override-v1`:
  ...
  - `tools/o3de/runtime_harness.py` now has `--diagnose-runtime-loadlevel-override` and `--enable-runtime-exit-fixture-loadlevel-override` in addition to the preserved launch-hygiene and runtime fixture modes
  - PR #132 defaultlevel diagnosis remains preserved: `C:/Users/topgu/O3DE/Projects/MAXINE_GoldenCorpus/Registry/load_level.setreg` sets `/O3DE/Autoexec/ConsoleCommands/LoadLevel=defaultlevel`, and the autoexec-only `--regremove=/O3DE/Autoexec/ConsoleCommands/LoadLevel` candidate remains recorded as failed historical evidence
  - source inspection now records the Settings Registry merge-order surface, command-line override order, console autoexec notification path, and `SpawnableLevelSystem` deferred load queue at `/O3DE/Runtime/SpawnableLevelSystem/DeferredLoadLevel`
  - the selected candidate is `settings_registry_regremove_autoexec_and_deferred_loadlevel`, with command args `--regremove=/O3DE/Autoexec/ConsoleCommands/LoadLevel` and `--regremove=/O3DE/Runtime/SpawnableLevelSystem/DeferredLoadLevel`; it is per-process and does not mutate project registry, `Levels/defaultlevel`, production levels, or shipping behavior
  - the bounded LoadLevel-override fixture command was attempted and wrote `artifacts/o3de-integration/runtime-harness/runtime-exit-fixture-loadlevel-override-command-report.json`; it launched, exited `0`, and observed `MAXINE_RUNTIME_EXIT_FIXTURE_REQUESTING_EXIT`, but still loaded `Levels/defaultlevel/defaultlevel.spawnable`
  - the attempted candidate records `runtime_loadlevel_override_status=runtime_loadlevel_override_candidate_attempted_failed_defaultlevel_autoload`; stdout reports both regremove targets as missing at parse time, so the candidate-specific blocker is `blocked_by_settings_registry_merge_order` while the launch-hygiene blocker remains `blocked_by_default_level_autoload`
  - AP negotiation and shader serializer signals remain present and disqualifying in the failed live fixture run; `runtime_exit_fixture_execution_verified=false`, `runtime_execution_verified=false`, `runtime_loadlevel_override_verified=false`, and runtime character proof remains unclaimed
  - live APB product evidence passed with report `artifacts/o3de-integration/apb/apb-live-20260512T123600Z/asset_processor_batch_live_report.json`; APB-only integration, product-evidence audit, targeted live Editor diagnostic modes, full Editor smoke, and include-editor-smoke integration suite passed
  - PR #131 fixture registration, enablement, scoped project mutation, scoped rebuild, and marker/exit-0 evidence remain preserved; PR #132 launch-hygiene/defaultlevel autoload evidence remains preserved
