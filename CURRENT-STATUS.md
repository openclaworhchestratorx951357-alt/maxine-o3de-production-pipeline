...
- Implement Parent-Focus / Link-Context Prefab Override Apply Route v1 is implemented on `codex/implement-parent-focus-link-context-prefab-override-apply-route-v1` and pending review:
  - PR #151 was merged into `main` at `7997da008c4aefd79502b147a8e4d941ea2b870a`; this branch was created from that updated `main`
  - this slice adds the Editor smoke diagnostic mode `approved-source-prefab-parent-link-override-apply-route`, plus CLI aliases `--diagnose-approved-source-prefab-parent-link-override-apply-route` and `--enable-approved-source-prefab-parent-link-override-apply-route-fixture`
  - the repo-owned `MaxineRuntimeExitFixture.Editor` bridge now reflects `azlmbr.maxine.prefab_bridge.apply_approved_source_prefab_parent_link_component_overrides`, which focuses the edited approved prefab instance, switches to the focused parent context, applies Actor + Simple Motion component overrides through `PrefabOverridePublicInterface::ApplyComponentOverrides`, then restores focus to the edited prefab instance
  - the route remains gated to `<active-project>/Assets/Characters/MAXINE_GoldenCorpus/prefabs/release_rigged.prefab`; it does not expose unrestricted `SavePrefab`, does not hand-author unknown component JSON, and rejects defaultlevel, production-level, generated product/cache, traversal, unapproved absolute, other-project, and non-prefab paths through the preserved approved-source path policy
  - review repair now verifies the submitted entity and Actor/Simple Motion component IDs belong to the same approved source prefab before any focus, override detection, override application, push, or save-adjacent route step; the check uses `PrefabPublicInterface::GetOwningInstancePrefabPath`, `PrefabLoaderInterface::GetFullPath`, `ComponentApplicationRequests::FindEntity`, and `Entity::FindComponent`
  - route verification now requires fresh source-template persistence in this run: parsed `ActorAsset` and `MotionAsset` markers are not enough unless `approved_source_prefab_changed_this_run=true` / before-after hash inequality also proves the approved source prefab changed during the diagnostic
  - the narrow Editor target rebuild passed with `cmake --build C:/src/o3de/build/windows --target MaxineRuntimeExitFixture.Editor --config profile --parallel 1 -- /m:1 /nodeReuse:false /v:m`
  - live Editor evidence confirms the new route is callable and source-validated; the route established and restored parent focus context, but O3DE still did not expose a link/component override path for the edited approved source instance, so the selected result is the typed blocker `blocked_by_prefab_link_context_unavailable`; report: `artifacts/o3de-integration/editor-smoke/editor-smoke-20260514T034511Z/editor_smoke_live_report.json`
  - live preservation diagnostics passed for save/update route (`artifacts/o3de-integration/editor-smoke/editor-smoke-20260514T034336Z/editor_smoke_live_report.json`), bridge-host (`artifacts/o3de-integration/editor-smoke/editor-smoke-20260514T034425Z/editor_smoke_live_report.json`), approved source-prefab Actor + Simple Motion wiring (`artifacts/o3de-integration/editor-smoke/editor-smoke-20260514T034440Z/editor_smoke_live_report.json`), and source-template propagation/apply step (`artifacts/o3de-integration/editor-smoke/editor-smoke-20260514T034456Z/editor_smoke_live_report.json`)
  - source-template marker proof remains the authority: `approved_source_prefab_parent_link_override_apply_route_verified`, propagation/apply verification, and Actor + Simple Motion wiring verification may become true only when the saved source template changed in this run and contains parsed `ActorAsset` and `MotionAsset` marker evidence; route callability, override status, or stale marker presence alone is insufficient
  - the approved repo-owned source prefab remains unchanged, Actor + Simple Motion were not persisted, `ActorAsset` / `MotionAsset` source-template markers were not verified, APB/runtime verification after source mutation did not run, and component-wiring, runtime animation, and full runtime character proof remain unclaimed
  - APB regeneration, runtime Actor/Simple Motion/Anim Graph TypeId inventory, runtime asset assignment proof, animation playback, and full runtime character proof remain unclaimed until source-template marker persistence is verified and downstream gates actually run
