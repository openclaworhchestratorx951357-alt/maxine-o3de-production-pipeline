...
- Pin Approved Runtime Animation Component Wiring Surface v1 is in progress on `codex/pin-approved-runtime-animation-component-wiring-surface-v1`:
  - PR #143 was repaired, validated, and merged into `main` at `6b0e8af062785c8836e069af528b57fd3d6d3acc`; this branch was created from that updated `main`
  - this slice adds `--diagnose-runtime-character-animation-component-wiring-surface` and `--enable-runtime-character-animation-component-wiring-surface-fixture` to `tools/o3de/runtime_harness.py`
  - O3DE source validation now pins the EMotionFX Editor component wiring surface without guessing: `EditorActorComponent` (`{A863EE1B-8CFD-4EDD-BA0D-1CEC2879AD44}`) with `Actor asset`, `EditorSimpleMotionComponent` (`{0CF1ADF7-DA51-4183-89EC-BDD7D2E17D36}`) with `Configuration|Motion`, `EditorAnimGraphComponent` (`{770F0A71-59EA-413B-8DAB-235FB0FF1384}`) with `Anim graph` and `Motion set asset`, `EditorComponentAPIBus` add/property APIs, `PrefabPublicRequestBus` instantiate/create APIs, `PrefabPublicHandler` save APIs, and Prefab-to-Spawnable conversion source
  ...
  - no approved source-prefab wiring is implemented in this slice unless it is generated through source-backed Editor/prefab APIs and subsequently verified through APB/spawned runtime EMotionFX component inventory
