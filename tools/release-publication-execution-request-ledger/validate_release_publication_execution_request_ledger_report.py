#!/usr/bin/env python3
"""Validate evidence-only release publication execution request ledger reports."""
...
EXPECTED_REPORT_TYPE = "RELEASE_PUBLICATION_EXECUTION_REQUEST_LEDGER_v1_REPORT"
EXPECTED_CONTRACT_ID = "RELEASE_PUBLICATION_EXECUTION_REQUEST_LEDGER_v1"
EXPECTED_REQUEST_ACTION = "request_publication_execution"
EXPECTED_READY_DECISION = "recorded_for_manual_execution_review"
TARGET_PATH = "qc.gates[]"
FUTURE_TARGET_PATH = "qc.checks[]"
CHECK_ID = "release_publication_execution_request_ledger_v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate MAXINE release publication execution request ledger report JSON."
    )
    parser.add_argument("report_path", help="Path to release publication execution request ledger report JSON")
    parser.add_argument(
        "--allow-warn",
        action="store_true",
...
def main() -> int:
    ...
    schema_path = repo_root / "schemas" / "maxine_release_publication_execution_request_ledger_report.schema.json"
    ...
    ledger_contract = report.get("ledger_contract", {}) if isinstance(report.get("ledger_contract"), dict) else {}
    contract_id = str(ledger_contract.get("contract_id", "")).strip()
    required_gate_ids = ledger_contract.get("required_gate_ids", [])
    required_approvals = ledger_contract.get("required_approvals")
    manual_review_required = ledger_contract.get("manual_review_required")

    if contract_id != EXPECTED_CONTRACT_ID:
        add_finding(findings, "contract_id_invalid", "error", "open", f"ledger_contract.contract_id must be {EXPECTED_CONTRACT_ID}.", {"actual": contract_id})
    if not _is_string_array(required_gate_ids):
        add_finding(findings, "required_gate_ids_invalid", "error", "open", "ledger_contract.required_gate_ids must be a non-empty string array.")
        required_gate_ids = []
    ...
    if not isinstance(required_approvals, int) or required_approvals < 1:
        add_finding(findings, "required_approvals_invalid", "error", "open", "ledger_contract.required_approvals must be integer >= 1.", {"actual": required_approvals})
        required_approvals = 1

    if not isinstance(manual_review_required, bool):
        add_finding(findings, "manual_review_required_invalid", "error", "open", "ledger_contract.manual_review_required must be boolean.", {"actual": manual_review_required})
    ...
    if ledger_contract.get("immutable_audit_bundle_required") is not True:
        add_finding(findings, "immutable_audit_bundle_required", "error", "open", "ledger_contract.immutable_audit_bundle_required must be true.", {"actual": ledger_contract.get("immutable_audit_bundle_required")})
    if ledger_contract.get("evidence_only") is not True:
        add_finding(findings, "evidence_only_required", "error", "open", "ledger_contract.evidence_only must be true.", {"actual": ledger_contract.get("evidence_only")})
    if ledger_contract.get("execution_admitted") is not False:
        add_finding(findings, "execution_admitted_must_be_false", "error", "open", "ledger_contract.execution_admitted must be false.", {"actual": ledger_contract.get("execution_admitted")})

    ledger = report.get("execution_request_ledger", {}) if isinstance(report.get("execution_request_ledger"), dict) else {}
    requested_action = str(ledger.get("requested_action", "")).strip()
    publish_command_display = str(ledger.get("publish_command_display", "")).strip()
    rollback_command_display = str(ledger.get("rollback_command_display", "")).strip()
    dry_run_bundle_path = str(ledger.get("evidence_bundle_path", "")).strip()
    dry_run_bundle_sha = str(ledger.get("evidence_bundle_sha256", "")).strip()
    dry_run_artifacts = ledger.get("evidence_bundle_artifacts", [])

    if requested_action != EXPECTED_REQUEST_ACTION:
        add_finding(findings, "requested_action_invalid", "error", "open", f"execution_request_ledger.requested_action must be {EXPECTED_REQUEST_ACTION}.", {"actual": requested_action})
    ...
    for field_name, command_display in (("publish_command_display", publish_command_display), ("rollback_command_display", rollback_command_display)):
        if not command_display:
            add_finding(findings, f"{field_name}_missing", "error", "open", f"execution_request_ledger.{field_name} is required.")
            continue
        if "displayonly" not in command_display.lower():
            add_finding(findings, f"{field_name}_must_be_display_only", "error", "open", f"execution_request_ledger.{field_name} must be display-only text.", {"value": command_display})
        if _contains_unsafe_command_tokens(command_display):
            add_finding(findings, f"{field_name}_unsafe", "error", "open", f"execution_request_ledger.{field_name} contains unsafe execution tokens.", {"value": command_display})

    if ledger.get("no_command_execution_recorded") is not True:
        add_finding(findings, "no_command_execution_recorded_required", "error", "open", "execution_request_ledger.no_command_execution_recorded must be true.", {"actual": ledger.get("no_command_execution_recorded")})

    if not dry_run_bundle_path:
        add_finding(findings, "evidence_bundle_path_missing", "error", "open", "execution_request_ledger.evidence_bundle_path is required.")
    elif _contains_unsafe_path_tokens(dry_run_bundle_path):
        add_finding(findings, "evidence_bundle_path_unsafe", "error", "open", "execution_request_ledger.evidence_bundle_path contains unsafe traversal or absolute-path tokens.", {"path": dry_run_bundle_path})

    if not _is_likely_sha256(dry_run_bundle_sha):
        add_finding(findings, "evidence_bundle_sha256_invalid", "error", "open", "execution_request_ledger.evidence_bundle_sha256 must look like sha256 hex.", {"actual": dry_run_bundle_sha})

    if not _is_string_array(dry_run_artifacts):
        add_finding(findings, "evidence_bundle_artifacts_invalid", "error", "open", "execution_request_ledger.evidence_bundle_artifacts must be a non-empty string array.")
        dry_run_artifacts = []
    dry_run_artifacts = _normalize_string_list(dry_run_artifacts)
    for artifact_path in dry_run_artifacts:
        if _contains_unsafe_path_tokens(artifact_path):
            add_finding(findings, "evidence_bundle_artifact_path_unsafe", "error", "open", "execution_request_ledger.evidence_bundle_artifacts contains unsafe path tokens.", {"path": artifact_path})
        ...
        if any(token in lowered for token in ("cache/", "cache\\", "assetdb.sqlite", ".fbx", ".gltf", ".glb", ".obj", ".png", ".jpg", ".jpeg", ".exe", ".dll", ".bin", ".sqlite")):
            add_finding(findings, "evidence_bundle_artifact_disallowed", "error", "open", "execution_request_ledger.evidence_bundle_artifacts includes disallowed binary/runtime/cache/database artifact.", {"path": artifact_path})

    if ledger.get("evidence_bundle_immutable") is not True:
        add_finding(findings, "evidence_bundle_immutable_required", "error", "open", "execution_request_ledger.evidence_bundle_immutable must be true.", {"actual": ledger.get("evidence_bundle_immutable")})
    ...
    if decision not in {EXPECTED_READY_DECISION, "pending_manual_review", "rejected"}:
        add_finding(findings, "approval_decision_invalid", "error", "open", "approval_state.decision must be recorded_for_manual_execution_review|pending_manual_review|rejected.", {"actual": decision})
    ...
    if required_gate_ids and readiness_required_gate_ids and required_gate_ids != readiness_required_gate_ids:
        add_finding(findings, "required_gate_ids_mismatch", "warning", "open", "ledger_contract.required_gate_ids does not match readiness.required_gate_ids.", {"contract_required_gate_ids": required_gate_ids, "readiness_required_gate_ids": readiness_required_gate_ids})
    ...
    if computed_missing_gate_ids:
        add_finding(findings, "required_gates_missing", "error", "open", "Required execution request ledger gates are missing.", {"missing_gate_ids": computed_missing_gate_ids})

    if release_readiness_state not in {"ready_for_manual_execution_request_ledger_review", "blocked_missing_evidence", "blocked_safety_boundary"}:
    ...
    if release_readiness_state == "ready_for_manual_execution_request_ledger_review" and computed_missing_gate_ids:
        add_finding(findings, "ready_state_with_missing_gates", "error", "open", "ready_for_manual_execution_request_ledger_review cannot be set when required gates are missing.")

    if decision == EXPECTED_READY_DECISION:
        if len(approver_ids) < required_approvals:
            add_finding(findings, "insufficient_approvals", "error", "open", "approval_state.approver_ids count is lower than ledger_contract.required_approvals.", {"approver_ids": approver_ids, "required_approvals": required_approvals})
        if blocked_reason_codes:
            add_finding(findings, "ready_with_blocked_reasons", "error", "open", "recorded_for_manual_execution_review must not include blocked_reason_codes.", {"blocked_reason_codes": blocked_reason_codes})
        if rollback_plan_verified is not True:
            add_finding(findings, "rollback_plan_not_verified", "error", "open", "recorded_for_manual_execution_review requires rollback_plan_verified=true.")
        if cleanup_plan_verified is not True:
            add_finding(findings, "cleanup_plan_not_verified", "error", "open", "recorded_for_manual_execution_review requires cleanup_plan_verified=true.")
        if release_readiness_state != "ready_for_manual_execution_request_ledger_review":
            add_finding(findings, "ready_without_ready_state", "error", "open", "recorded_for_manual_execution_review requires readiness.release_readiness_state=ready_for_manual_execution_request_ledger_review.")
    elif decision == "pending_manual_review":
        add_finding(findings, "request_ledger_pending_manual_review", "manual_review", "open", "Execution request ledger remains pending manual review.")
    elif decision == "rejected":
        add_finding(findings, "request_ledger_rejected", "error", "open", "Execution request ledger decision is rejected.")
