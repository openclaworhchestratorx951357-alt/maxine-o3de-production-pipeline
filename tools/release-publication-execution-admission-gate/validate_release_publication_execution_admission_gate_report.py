#!/usr/bin/env python3
"""Validate evidence-only release publication execution admission gate reports."""
...
EXPECTED_REPORT_TYPE = "RELEASE_PUBLICATION_EXECUTION_ADMISSION_GATE_v1_REPORT"
EXPECTED_CONTRACT_ID = "RELEASE_PUBLICATION_EXECUTION_ADMISSION_GATE_v1"
EXPECTED_REQUEST_ACTION = "admit_publication_execution"
EXPECTED_ADMISSION_DECISION_READY = "ready_for_future_admission_review"
TARGET_PATH = "qc.gates[]"
FUTURE_TARGET_PATH = "qc.checks[]"
CHECK_ID = "release_publication_execution_admission_gate_v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate MAXINE release publication execution admission gate report JSON."
    )
    parser.add_argument("report_path", help="Path to release publication execution admission gate report JSON")
    parser.add_argument(
        "--allow-warn",
        action="store_true",
...
def main() -> int:
    ...
    schema_path = repo_root / "schemas" / "maxine_release_publication_execution_admission_gate_report.schema.json"
    ...
    admission_contract = report.get("admission_contract", {}) if isinstance(report.get("admission_contract"), dict) else {}
    contract_id = str(admission_contract.get("contract_id", "")).strip()
    required_gate_ids = admission_contract.get("required_gate_ids", [])
    required_approvals = admission_contract.get("required_approvals")
    manual_review_required = admission_contract.get("manual_review_required")
    ...
    if contract_id != EXPECTED_CONTRACT_ID:
        add_finding(
            ...
            "error",
            "open",
            f"admission_contract.contract_id must be {EXPECTED_CONTRACT_ID}.",
            {"actual": contract_id},
        )
    if not _is_string_array(required_gate_ids):
        add_finding(
            ...
            "error",
            "open",
            "admission_contract.required_gate_ids must be a non-empty string array.",
        )
        required_gate_ids = []
    ...
    if not isinstance(required_approvals, int) or required_approvals < 1:
        add_finding(
            ...
            "error",
            "open",
            "admission_contract.required_approvals must be integer >= 1.",
    ...
    if not isinstance(manual_review_required, bool):
        add_finding(
            ...
            "error",
            "open",
            "admission_contract.manual_review_required must be boolean.",
    ...
    if admission_contract.get("explicit_execution_admission_required") is not True:
        add_finding(
            findings,
            "explicit_execution_admission_required",
            "error",
            "open",
            "admission_contract.explicit_execution_admission_required must be true.",
            {"actual": admission_contract.get("explicit_execution_admission_required")},
        )
    if admission_contract.get("evidence_only") is not True:
        add_finding(
            findings,
            "evidence_only_required",
            "error",
            "open",
            "admission_contract.evidence_only must be true.",
            {"actual": admission_contract.get("evidence_only")},
        )
    if admission_contract.get("execution_admitted") is not False:
        add_finding(
            findings,
            "execution_admitted_must_be_false",
            "error",
            "open",
            "admission_contract.execution_admitted must be false.",
            {"actual": admission_contract.get("execution_admitted")},
        )

    gate_request = report.get("execution_gate_request", {}) if isinstance(report.get("execution_gate_request"), dict) else {}
    requested_action = str(gate_request.get("requested_action", "")).strip()
    publish_command_display = str(gate_request.get("publish_command_display", "")).strip()
    rollback_command_display = str(gate_request.get("rollback_command_display", "")).strip()
    dry_run_evidence_paths = gate_request.get("dry_run_evidence_paths", [])
    ...
    if requested_action != EXPECTED_REQUEST_ACTION:
        add_finding(
            ...
            "error",
            "open",
            f"execution_gate_request.requested_action must be {EXPECTED_REQUEST_ACTION}.",
            {"actual": requested_action},
        )
    ...
    for field_name, command_display in (
        ...
        if not command_display:
            add_finding(
                ...
                "error",
                "open",
                f"execution_gate_request.{field_name} is required.",
            )
            continue
        if "displayonly" not in command_display.lower():
            add_finding(
                ...
                "error",
                "open",
                f"execution_gate_request.{field_name} must be display-only text.",
        ...
        if _contains_unsafe_command_tokens(command_display):
            add_finding(
                ...
                "error",
                "open",
                f"execution_gate_request.{field_name} contains unsafe execution tokens.",
                {"value": command_display},
            )

    if gate_request.get("no_command_execution_recorded") is not True:
        add_finding(
            findings,
            "no_command_execution_recorded_required",
            "error",
            "open",
            "execution_gate_request.no_command_execution_recorded must be true.",
            {"actual": gate_request.get("no_command_execution_recorded")},
        )
    ...
    if not _is_string_array(dry_run_evidence_paths):
        add_finding(
            ...
            "error",
            "open",
            "execution_gate_request.dry_run_evidence_paths must be a non-empty string array.",
        )
    ...
    for ev_path in dry_run_evidence_paths:
        if _contains_unsafe_path_tokens(ev_path):
            add_finding(
                ...
                "error",
                "open",
                "execution_gate_request.dry_run_evidence_paths contains unsafe traversal or absolute-path tokens.",
                {"path": ev_path},
            )

    assessment = report.get("admission_assessment", {}) if isinstance(report.get("admission_assessment"), dict) else {}
    admission_decision = str(assessment.get("admission_decision", "")).strip()
    approver_ids = assessment.get("approver_ids", [])
    blocked_reason_codes = assessment.get("blocked_reason_codes", [])
    rollback_plan_verified = assessment.get("rollback_plan_verified")
    cleanup_plan_verified = assessment.get("cleanup_plan_verified")

    if admission_decision not in {EXPECTED_ADMISSION_DECISION_READY, "pending_manual_review", "rejected"}:
        add_finding(
            findings,
            "admission_decision_invalid",
            "error",
            "open",
            "admission_assessment.admission_decision must be ready_for_future_admission_review|pending_manual_review|rejected.",
            {"actual": admission_decision},
        )
        admission_decision = "pending_manual_review"
    ...
    if not _is_string_array(approver_ids) and approver_ids != []:
        add_finding(
            ...
            "error",
            "open",
            "admission_assessment.approver_ids must be a string array.",
        )
        approver_ids = []
    if not _is_string_array(blocked_reason_codes) and blocked_reason_codes != []:
        add_finding(
            ...
            "error",
            "open",
            "admission_assessment.blocked_reason_codes must be a string array.",
        )
    ...
    for flag_name in (
        ...
        if assessment.get(flag_name) is not False:
            add_finding(
                findings,
                f"{flag_name}_must_be_false",
                "error",
                "open",
                f"admission_assessment.{flag_name} must be false.",
                {"actual": assessment.get(flag_name)},
            )
    ...
    if not isinstance(rollback_plan_verified, bool):
        add_finding(
            ...
            "error",
            "open",
            "admission_assessment.rollback_plan_verified must be boolean.",
    ...
    if not isinstance(cleanup_plan_verified, bool):
        add_finding(
            ...
            "error",
            "open",
            "admission_assessment.cleanup_plan_verified must be boolean.",
    ...
    if required_gate_ids and readiness_required_gate_ids and required_gate_ids != readiness_required_gate_ids:
        add_finding(
            ...
            "warning",
            "open",
            "admission_contract.required_gate_ids does not match readiness.required_gate_ids.",
    ...
    if computed_missing_gate_ids:
        add_finding(
            ...
            "error",
            "open",
            "Required release publication execution admission gates are missing.",
            {"missing_gate_ids": computed_missing_gate_ids},
        )

    if release_readiness_state not in {
        "ready_for_manual_execution_admission_review",
    ...
    if release_readiness_state == "ready_for_manual_execution_admission_review" and computed_missing_gate_ids:
        add_finding(
            findings,
            "ready_state_with_missing_gates",
            "error",
            "open",
            "ready_for_manual_execution_admission_review cannot be set when required gates are missing.",
        )

    if admission_decision == EXPECTED_ADMISSION_DECISION_READY:
        if len(approver_ids) < required_approvals:
            add_finding(
                ...
                "error",
                "open",
                "admission_assessment.approver_ids count is lower than admission_contract.required_approvals.",
        ...
        if blocked_reason_codes:
            add_finding(
                ...
                "error",
                "open",
                "ready_for_future_admission_review must not include blocked_reason_codes.",
        ...
        if rollback_plan_verified is not True:
            add_finding(
                ...
                "error",
                "open",
                "ready_for_future_admission_review requires rollback_plan_verified=true.",
            )
        if cleanup_plan_verified is not True:
            add_finding(
                ...
                "error",
                "open",
                "ready_for_future_admission_review requires cleanup_plan_verified=true.",
            )
        if release_readiness_state != "ready_for_manual_execution_admission_review":
            add_finding(
                findings,
                "ready_without_ready_state",
                "error",
                "open",
                "ready_for_future_admission_review requires readiness.release_readiness_state=ready_for_manual_execution_admission_review.",
            )
    elif admission_decision == "pending_manual_review":
        add_finding(
            findings,
            "admission_pending_manual_review",
            "manual_review",
            "open",
            "Execution admission gate remains pending manual review.",
        )
    elif admission_decision == "rejected":
        add_finding(
            findings,
            "admission_rejected",
            "error",
            "open",
            "Execution admission gate decision is rejected.",
        )
