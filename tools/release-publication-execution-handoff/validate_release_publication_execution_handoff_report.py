#!/usr/bin/env python3
"""Validate evidence-only release publication execution handoff reports."""
...
EXPECTED_REPORT_TYPE = "RELEASE_PUBLICATION_EXECUTION_HANDOFF_v1_REPORT"
EXPECTED_CONTRACT_ID = "RELEASE_PUBLICATION_EXECUTION_HANDOFF_v1"
EXPECTED_REQUEST_ACTION = "record_release_publication_execution_handoff"
EXPECTED_HANDOFF_SCOPE = "release_publication_chain"
EXPECTED_READY_DECISION = "execution_handoff_recorded_for_future_execution_admission_request"
TARGET_PATH = "qc.gates[]"
FUTURE_TARGET_PATH = "qc.checks[]"
CHECK_ID = "release_publication_execution_handoff_v1"
...
REQUIRED_ARTIFACT_HINTS = {
    ...
    "release_publication_ready_for_execution_request": "release-publication-ready-for-execution-request",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate MAXINE release publication execution handoff report JSON."
    )
    parser.add_argument("report_path", help="Path to release publication execution handoff report JSON")
    parser.add_argument(
        "--allow-warn",
        action="store_true",
...
def main() -> int:
    ...
    schema_path = repo_root / "schemas" / "maxine_release_publication_execution_handoff_report.schema.json"
    ...
    required_handoff_artifact_ids = readiness_contract.get("required_handoff_artifact_ids", [])
    ...
    if not _is_string_array(required_gate_ids):
        ...
        required_gate_ids = []
    if not _is_string_array(required_handoff_artifact_ids):
        add_finding(
            findings,
            "required_handoff_artifact_ids_invalid",
            "error",
            "open",
            "readiness_contract.required_handoff_artifact_ids must be a non-empty string array.",
        )
        required_handoff_artifact_ids = []

    required_gate_ids = _normalize_string_list(required_gate_ids)
    required_handoff_artifact_ids = _normalize_string_list(required_handoff_artifact_ids)
    ...
    if readiness_contract.get("immutable_handoff_packet_required") is not True:
        add_finding(
            findings,
            "immutable_handoff_packet_required",
            "error",
            "open",
            "readiness_contract.immutable_handoff_packet_required must be true.",
            {"actual": readiness_contract.get("immutable_handoff_packet_required")},
        )
    ...
    handoff_packet_data = report.get("execution_handoff", {}) if isinstance(report.get("execution_handoff"), dict) else {}
    handoff_action = str(handoff_packet_data.get("handoff_action", "")).strip()
    handoff_scope = str(handoff_packet_data.get("handoff_scope", "")).strip()
    hash_algorithm = str(handoff_packet_data.get("hash_algorithm", "")).strip()
    verification_status = str(handoff_packet_data.get("handoff_verification_status", "")).strip()
    chain_manifest_path = str(handoff_packet_data.get("chain_manifest_path", "")).strip()
    handoff_packet_path = str(handoff_packet_data.get("handoff_packet_path", "")).strip()
    handoff_packet_sha = str(handoff_packet_data.get("handoff_packet_sha256", "")).strip()
    source_ready_packet_path = str(
        handoff_packet_data.get("source_ready_for_execution_request_packet_path", "")
    ).strip()
    source_ready_packet_sha = str(
        handoff_packet_data.get("source_ready_for_execution_request_packet_sha256", "")
    ).strip()
    handoff_packet_artifacts = handoff_packet_data.get("handoff_packet_artifacts", [])

    if handoff_action != EXPECTED_REQUEST_ACTION:
        add_finding(
            findings,
            "handoff_action_invalid",
            "error",
            "open",
            f"execution_handoff.handoff_action must be {EXPECTED_REQUEST_ACTION}.",
            {"actual": handoff_action},
        )
    if handoff_scope != EXPECTED_HANDOFF_SCOPE:
        add_finding(
            findings,
            "handoff_scope_invalid",
            "error",
            "open",
            f"execution_handoff.handoff_scope must be {EXPECTED_HANDOFF_SCOPE}.",
            {"actual": handoff_scope},
        )

    source_ready_request_id = str(
        handoff_packet_data.get("source_ready_for_execution_request_id", "")
    ).strip()
    if not source_ready_request_id:
        add_finding(
            findings,
            "source_ready_for_execution_request_id_missing",
            "error",
            "open",
            "execution_handoff.source_ready_for_execution_request_id is required.",
        )
    ...
    if hash_algorithm != "sha256":
        add_finding(
            ...
            "error",
            "open",
            "execution_handoff.hash_algorithm must be sha256.",
    ...
    if verification_status not in ALLOWED_STATUS:
        add_finding(
            findings,
            "handoff_verification_status_invalid",
            "error",
            "open",
            "execution_handoff.handoff_verification_status must be pass|warn|fail|pending_manual.",
    ...
    for label, path_text in (
        ...
        ("source_ready_for_execution_request_packet_path", source_ready_packet_path),
    ):
        if not path_text:
            add_finding(findings, f"{label}_missing", "error", "open", f"execution_handoff.{label} is required.")
            continue
        if _contains_unsafe_path_tokens(path_text):
            add_finding(
                ...
                "error",
                "open",
                f"execution_handoff.{label} contains unsafe traversal or absolute-path tokens.",
                {"path": path_text},
            )

    if not _is_likely_sha256(handoff_packet_sha):
        add_finding(
            findings,
            "handoff_packet_sha256_invalid",
            "error",
            "open",
            "execution_handoff.handoff_packet_sha256 must look like sha256 hex.",
            {"actual": handoff_packet_sha},
        )

    if not _is_likely_sha256(source_ready_packet_sha):
        add_finding(
            findings,
            "source_ready_for_execution_request_packet_sha256_invalid",
            "error",
            "open",
            "execution_handoff.source_ready_for_execution_request_packet_sha256 must look like sha256 hex.",
            {"actual": source_ready_packet_sha},
        )

    if handoff_packet_data.get("no_command_execution_recorded") is not True:
        add_finding(
            findings,
            "no_command_execution_recorded_required",
            "error",
            "open",
            "execution_handoff.no_command_execution_recorded must be true.",
            {"actual": handoff_packet_data.get("no_command_execution_recorded")},
        )

    artifact_hashes = handoff_packet_data.get("handoff_packet_artifact_hashes", {})
    if not isinstance(artifact_hashes, dict) or not artifact_hashes:
        add_finding(
            findings,
            "handoff_packet_artifact_hashes_invalid",
            "error",
            "open",
            "execution_handoff.handoff_packet_artifact_hashes must be a non-empty object.",
        )
        artifact_hashes = {}
    ...
    for key, value in artifact_hashes.items():
        ...
        if not key_text:
            add_finding(findings, "artifact_hash_key_missing", "error", "open", "handoff_packet_artifact_hashes keys must be non-empty.")
            continue
        ...
        if not _is_likely_sha256(value_text):
            add_finding(
                ...
                "error",
                "open",
                "handoff_packet_artifact_hashes values must look like sha256 hex.",
                {"artifact_id": key_text, "value": value_text},
            )

    artifact_count = handoff_packet_data.get("artifact_count")
    if not isinstance(artifact_count, int) or artifact_count < 1:
        add_finding(
            ...
            "error",
            "open",
            "execution_handoff.artifact_count must be integer >= 1.",
        ...
        add_finding(
            ...
            "error",
            "open",
            "artifact_count must equal the number of handoff_packet_artifact_hashes entries.",
            {"declared": artifact_count, "computed": len(normalized_hashes)},
        )

    missing_ids = handoff_packet_data.get("missing_handoff_artifact_ids", [])
    mismatch_ids = handoff_packet_data.get("hash_mismatch_handoff_artifact_ids", [])
    if not _is_string_array(missing_ids) and missing_ids != []:
        add_finding(findings, "missing_handoff_artifact_ids_invalid", "error", "open", "missing_handoff_artifact_ids must be a string array.")
        missing_ids = []
    if not _is_string_array(mismatch_ids) and mismatch_ids != []:
        add_finding(findings, "hash_mismatch_handoff_artifact_ids_invalid", "error", "open", "hash_mismatch_handoff_artifact_ids must be a string array.")
        mismatch_ids = []

    missing_ids = _normalize_string_list(missing_ids)
    mismatch_ids = _normalize_string_list(mismatch_ids)

    computed_missing_ids = sorted(set(required_handoff_artifact_ids) - set(normalized_hashes.keys()))
    if computed_missing_ids != missing_ids:
        add_finding(
            findings,
            "missing_handoff_artifact_ids_mismatch",
            "warning",
            "open",
            "missing_handoff_artifact_ids does not match required-vs-present hash-index computation.",
    ...
    if computed_missing_ids:
        add_finding(
            findings,
            "required_handoff_artifacts_missing",
            "error",
            "open",
            "Required handoff artifacts are missing from handoff_packet_artifact_hashes.",
            {"missing_handoff_artifact_ids": computed_missing_ids},
        )

    unexpected_mismatch_ids = [item for item in mismatch_ids if item not in required_handoff_artifact_ids]
    if unexpected_mismatch_ids:
        add_finding(
            ...
            "error",
            "open",
            "hash_mismatch_handoff_artifact_ids includes values not present in required_handoff_artifact_ids.",
            {"unexpected_ids": unexpected_mismatch_ids},
        )

    if verification_status == "pass":
        if computed_missing_ids:
            add_finding(findings, "pass_with_missing_handoff_artifacts", "error", "open", "handoff_verification_status=pass is invalid when required handoff artifacts are missing.")
        if mismatch_ids:
            add_finding(findings, "pass_with_hash_mismatch", "error", "open", "handoff_verification_status=pass is invalid when hash_mismatch_handoff_artifact_ids is not empty.")
    ...
    if verification_status in {"fail", "pending_manual"} and not (computed_missing_ids or mismatch_ids):
        add_finding(
            ...
            "warning",
            "open",
            "handoff_verification_status is non-pass but no missing/mismatch evidence is recorded.",
        )

    if not _is_string_array(handoff_packet_artifacts):
        add_finding(
            findings,
            "handoff_packet_artifacts_invalid",
            "error",
            "open",
            "execution_handoff.handoff_packet_artifacts must be a non-empty string array.",
        )
        handoff_packet_artifacts = []
    handoff_packet_artifacts = _normalize_string_list(handoff_packet_artifacts)

    if isinstance(artifact_count, int) and artifact_count > 0 and len(handoff_packet_artifacts) != artifact_count:
        add_finding(
            findings,
            "handoff_packet_artifacts_count_mismatch",
            "warning",
            "open",
            "handoff_packet_artifacts count differs from artifact_count.",
            {"artifact_count": artifact_count, "artifact_path_count": len(handoff_packet_artifacts)},
        )

    for artifact_path in handoff_packet_artifacts:
        if _contains_unsafe_path_tokens(artifact_path):
            add_finding(
                findings,
                "handoff_packet_artifact_path_unsafe",
                "error",
                "open",
                "handoff_packet_artifacts contains unsafe path tokens.",
                {"path": artifact_path},
            )
        ...
        if any(token in lowered for token in DISALLOWED_ARTIFACT_TOKENS):
            add_finding(
                findings,
                "handoff_packet_artifact_disallowed",
                "error",
                "open",
                "handoff_packet_artifacts includes disallowed binary/runtime/cache/database artifact.",
                {"path": artifact_path},
            )

    for hint_id, hint_token in REQUIRED_ARTIFACT_HINTS.items():
        if hint_id in required_handoff_artifact_ids and not any(hint_token in item.lower() for item in handoff_packet_artifacts):
            add_finding(
                findings,
                f"required_artifact_missing_{hint_id}",
                "error",
                "open",
                "handoff_packet_artifacts is missing required execution-handoff evidence artifact.",
                {"required_hint": hint_token},
            )

    if handoff_packet_data.get("handoff_packet_immutable") is not True:
        add_finding(
            findings,
            "handoff_packet_immutable_required",
            "error",
            "open",
            "execution_handoff.handoff_packet_immutable must be true.",
            {"actual": handoff_packet_data.get("handoff_packet_immutable")},
        )
    ...
    if decision not in {EXPECTED_READY_DECISION, "pending_manual_review", "rejected"}:
        add_finding(
            ...
            "error",
            "open",
            "approval_state.decision must be execution_handoff_recorded_for_future_execution_admission_request|pending_manual_review|rejected.",
            {"actual": decision},
        )
    ...
    readiness_state = str(readiness.get("execution_handoff_readiness_state", "")).strip()
    ...
    if readiness_state not in {
        "execution_handoff_recorded_for_future_execution_admission_request",
        "blocked_missing_evidence",
        "blocked_safety_boundary",
    }:
        add_finding(
            findings,
            "execution_handoff_readiness_state_invalid",
            "error",
            "open",
            "readiness.execution_handoff_readiness_state is invalid.",
            {"actual": readiness_state},
        )

    if readiness_state == "execution_handoff_recorded_for_future_execution_admission_request" and computed_missing_gate_ids:
        add_finding(
            findings,
            "ready_state_with_missing_gates",
            "error",
            "open",
            "execution_handoff_recorded_for_future_execution_admission_request cannot be set when required gates are missing.",
        )
    ...
    if decision == EXPECTED_READY_DECISION:
        ...
        if blocked_reason_codes:
            add_finding(
                ...
                "error",
                "open",
                "execution_handoff_recorded_for_future_execution_admission_request must not include blocked_reason_codes.",
        ...
        if rollback_plan_verified is not True:
            add_finding(
                ...
                "error",
                "open",
                "execution_handoff_recorded_for_future_execution_admission_request requires rollback_plan_verified=true.",
            )
        if cleanup_plan_verified is not True:
            add_finding(
                ...
                "error",
                "open",
                "execution_handoff_recorded_for_future_execution_admission_request requires cleanup_plan_verified=true.",
            )
        if readiness_state != "execution_handoff_recorded_for_future_execution_admission_request":
            add_finding(
                findings,
                "ready_without_ready_state",
                "error",
                "open",
                "execution_handoff_recorded_for_future_execution_admission_request requires readiness.execution_handoff_readiness_state=execution_handoff_recorded_for_future_execution_admission_request.",
            )
    elif decision == "pending_manual_review":
        add_finding(
            findings,
            "execution_handoff_pending_manual_review",
            "manual_review",
            "open",
            "Release publication execution handoff remains pending manual review.",
        )
        ...
        add_finding(
            ...
            "error",
            "open",
            "Release publication execution handoff decision is rejected.",
        )

    if verification_status == "pass" and declared_status != "pass":
        add_finding(
            findings,
            "status_should_be_pass_for_handoff_packet_pass",
            "error",
            "open",
            "handoff_verification_status=pass requires report.status=pass.",
        )
    elif verification_status == "warn" and declared_status != "warn":
        add_finding(
            findings,
            "status_should_be_warn_for_handoff_packet_warn",
            "error",
            "open",
            "handoff_verification_status=warn requires report.status=warn.",
        )
    elif verification_status == "fail" and declared_status != "fail":
        add_finding(
            findings,
            "status_should_be_fail_for_handoff_packet_fail",
            "error",
            "open",
            "handoff_verification_status=fail requires report.status=fail.",
        )
    elif verification_status == "pending_manual" and declared_status != "pending_manual":
        add_finding(
            findings,
            "status_should_be_pending_for_handoff_packet_pending",
            "error",
            "open",
            "handoff_verification_status=pending_manual requires report.status=pending_manual.",
        )
