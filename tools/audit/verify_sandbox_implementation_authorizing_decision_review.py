#!/usr/bin/env python3
"""Verify sandbox implementation authorizing decision review artifacts."""
...
REQUIRED_DOC = "docs/reviews/SANDBOX-IMPLEMENTATION-AUTHORIZING-DECISION-REVIEW.md"
REQUIRED_TEMPLATE = "docs/reviews/sandbox_implementation_authorizing_decision_template.json"
REQUIRED_PACKAGE = (
    "docs/reviews/sandbox_implementation_authorizing_decision_review_package.json"
)
REQUIRED_REVIEW_NEEDLES = [
    "future execution implementation decision package",
...
REQUIRED_SAFETY = {
    ...
    "implementation_authorizing_progression_allowed_by_default": False,
...
def main() -> int:
    ...
    if package.get("default_decision_status") != "implementation_authorizing_decision_review_hold":
        failures.append(
            "default_decision_status must be implementation_authorizing_decision_review_hold"
        )
    ...
    if prior_decision_path.exists():
        try:
            ...
            if (
                prior_decision.get("decision_status")
                != "explicit_execution_authorization_review_pending_implementation_authorization_decision"
            ):
                failures.append(
                    "prior decision status must be explicit_execution_authorization_review_pending_implementation_authorization_decision"
                )
        except Exception as exc:
            failures.append(f"unable to parse prior decision JSON: {exc}")
    else:
        failures.append(
            "required prior decision missing: docs/reviews/sandbox_implementation_explicit_execution_authorization_decision.json"
        )
    ...
    if template_path.exists():
        try:
            ...
            if (
                ...
                failures.append(
                    "decision template decision_status must be implementation_authorizing_decision_review_hold"
                )
            ...
            if template.get("implementation_authorizing_progression_allowed") is not False:
                failures.append(
                    "decision template implementation_authorizing_progression_allowed must be false"
                )
        except Exception as exc:
    ...
    if failures:
        print("FAIL: sandbox implementation authorizing decision review verification failed.")
        for item in failures:
            print(f" - {item}")
        return 1

    print("PASS: sandbox implementation authorizing decision review verification passed.")
    return 0
