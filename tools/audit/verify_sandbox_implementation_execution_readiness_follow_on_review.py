#!/usr/bin/env python3
"""Verify sandbox implementation execution-readiness follow-on review artifacts."""
...
REQUIRED_REVIEW_NEEDLES = [
    "progress beyond `execution_readiness_review_pending`",
...
REQUIRED_SAFETY = {
    ...
    "execution_readiness_progression_allowed_by_default": False,
...
def main() -> int:
    ...
    if package.get("default_decision_status") != "execution_readiness_review_pending_hold":
        failures.append(
            "default_decision_status must be execution_readiness_review_pending_hold"
        )
    ...
    if readiness_decision_path.exists():
        try:
            readiness_decision = load_json(readiness_decision_path)
            if (
                readiness_decision.get("decision_status")
                != "execution_readiness_review_pending"
            ):
                failures.append(
                    "execution-readiness decision status must be execution_readiness_review_pending"
                )
            if readiness_decision.get("execution_hold_current_status") != "hold_active":
                failures.append(
                    "execution-readiness decision execution_hold_current_status must be hold_active"
                )
            if readiness_decision.get("execution_hold_next_status") != "hold_active":
                failures.append(
                    "execution-readiness decision execution_hold_next_status must be hold_active"
                )
        except Exception as exc:
            failures.append(f"unable to parse execution-readiness decision JSON: {exc}")
    else:
        failures.append(
            "required execution-readiness decision missing: docs/reviews/sandbox_implementation_execution_readiness_decision.json"
        )
    ...
    if template_path.exists():
        try:
            ...
            if (
                ...
                failures.append(
                    "decision template decision_status must be execution_readiness_review_pending_hold"
                )
            ...
            if (
                template.get("execution_readiness_decision_preparation_allowed")
                is not False
            ):
                failures.append(
                    "decision template execution_readiness_decision_preparation_allowed must be false"
                )
        except Exception as exc:
    ...
    if failures:
        print(
            "FAIL: sandbox implementation execution-readiness follow-on review verification failed."
        )
        for item in failures:
            print(f" - {item}")
        return 1

    print("PASS: sandbox implementation execution-readiness follow-on review verification passed.")
    return 0
