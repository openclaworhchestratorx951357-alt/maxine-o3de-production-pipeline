#!/usr/bin/env python3
"""Verify sandbox implementation execution-readiness follow-on decision artifacts."""
...
REQUIRED_DOC_NEEDLES = [
    "decision: execution_readiness_review_pending_accepted_for_decision_preparation_only",
...
def main() -> int:
    ...
    review_doc = (
        root / "docs/reviews/SANDBOX-IMPLEMENTATION-EXECUTION-READINESS-FOLLOW-ON-REVIEW.md"
    )
    ...
    if status not in ALLOWED_DECISION_STATUS:
        failures.append(
            "decision_status must be one of: execution_readiness_review_pending_hold, execution_readiness_review_pending_rejected, execution_readiness_review_pending_accepted_for_decision_preparation_only"
        )
    ...
    if accepted:
        if decision.get("execution_hold_transition_approved") is not True:
            failures.append(
                "execution_hold_transition_approved must be true when decision_status is execution_readiness_review_pending_accepted_for_decision_preparation_only"
            )
        if decision.get("execution_readiness_decision_preparation_allowed") is not True:
            failures.append(
                "execution_readiness_decision_preparation_allowed must be true when decision_status is execution_readiness_review_pending_accepted_for_decision_preparation_only"
            )
    else:
        ...
        if decision.get("execution_readiness_decision_preparation_allowed") is not False:
            failures.append(
                "execution_readiness_decision_preparation_allowed must be false for hold/rejected statuses"
            )
    ...
    if (
        ...
        failures.append(
            "review_package must reference docs/reviews/sandbox_implementation_execution_readiness_follow_on_review_package.json"
        )
    ...
    if (
        ...
        failures.append(
            "review_document must reference docs/reviews/SANDBOX-IMPLEMENTATION-EXECUTION-READINESS-FOLLOW-ON-REVIEW.md"
        )
    ...
    if (
        ...
        failures.append(
            "follow-on review package default_decision_status must be execution_readiness_review_pending_hold"
        )

    prior_decision = (
        root / "docs/reviews/sandbox_implementation_execution_readiness_decision.json"
    )
    if prior_decision.exists():
        try:
            prev = load_json(prior_decision)
            if prev.get("decision_status") != "execution_readiness_review_pending":
                failures.append(
                    "prior execution-readiness decision status must be execution_readiness_review_pending"
                )
        except Exception as exc:
            failures.append(f"unable to parse prior execution-readiness decision: {exc}")
    else:
        failures.append(
            "required prior execution-readiness decision missing: docs/reviews/sandbox_implementation_execution_readiness_decision.json"
        )
    ...
    if failures:
        print(
            "FAIL: sandbox implementation execution-readiness follow-on decision verification failed."
        )
        for item in failures:
            print(f" - {item}")
        return 1

    print("PASS: sandbox implementation execution-readiness follow-on decision verification passed.")
    return 0
