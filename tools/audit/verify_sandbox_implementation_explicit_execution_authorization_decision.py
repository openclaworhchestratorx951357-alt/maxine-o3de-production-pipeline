#!/usr/bin/env python3
"""Verify sandbox implementation explicit execution-authorization decision artifacts."""
...
REQUIRED_DOC_NEEDLES = [
    "decision: explicit_execution_authorization_review_pending_implementation_authorization_decision",
...
def main() -> int:
    ...
    decision_doc = (
        root / "docs/reviews/SANDBOX-IMPLEMENTATION-EXPLICIT-EXECUTION-AUTHORIZATION-DECISION.md"
    )
    decision_json = (
        root / "docs/reviews/sandbox_implementation_explicit_execution_authorization_decision.json"
    )
    review_package = (
        root
    ...
    if status not in ALLOWED_DECISION_STATUS:
        failures.append(
            "decision_status must be one of: explicit_execution_authorization_review_hold, explicit_execution_authorization_review_rejected, explicit_execution_authorization_review_pending_implementation_authorization_decision"
        )
    ...
    if accepted:
        if decision.get("execution_hold_transition_approved") is not True:
            failures.append(
                "execution_hold_transition_approved must be true when decision_status is explicit_execution_authorization_review_pending_implementation_authorization_decision"
            )
        if (
            decision.get("implementation_authorization_decision_preparation_allowed")
            is not True
        ):
            failures.append(
                "implementation_authorization_decision_preparation_allowed must be true when decision_status is explicit_execution_authorization_review_pending_implementation_authorization_decision"
            )
    else:
        ...
        if (
            ...
            failures.append(
                "implementation_authorization_decision_preparation_allowed must be false for hold/rejected statuses"
            )
    ...
    if (
        ...
        failures.append(
            "review_package must reference docs/reviews/sandbox_implementation_explicit_execution_authorization_decision_review_package.json"
        )
    ...
    if (
        ...
        failures.append(
            "review_document must reference docs/reviews/SANDBOX-IMPLEMENTATION-EXPLICIT-EXECUTION-AUTHORIZATION-DECISION-REVIEW.md"
        )
    ...
    if (
        review_pkg.get("default_decision_status")
        != "explicit_execution_authorization_review_hold"
    ):
        failures.append(
            "review package default_decision_status must be explicit_execution_authorization_review_hold"
        )

    prior_decision = (
        root / "docs/reviews/sandbox_implementation_execution_authorization_decision.json"
    )
    if prior_decision.exists():
        try:
            ...
            if (
                ...
                failures.append(
                    "prior decision status must be execution_authorization_decision_review_pending_explicit_authorization_decision"
                )
        except Exception as exc:
            failures.append(f"unable to parse prior decision: {exc}")
    else:
        failures.append(
            "required prior decision missing: docs/reviews/sandbox_implementation_execution_authorization_decision.json"
        )
    ...
    if failures:
        print(
            "FAIL: sandbox implementation explicit execution-authorization decision verification failed."
        )
        for item in failures:
            print(f" - {item}")
        return 1

    print("PASS: sandbox implementation explicit execution-authorization decision verification passed.")
    return 0
