...
RELEASE_CANDIDATE_PUBLICATION_DRY_RUN_SANDBOX_BOUNDARY_REL = (
...
EXPECTED_DRY_RUN_SANDBOX_BOUNDARY_STATUS = "static_boundary_valid_blocked"
EXPECTED_DRY_RUN_RUNNER_INTERFACE_STATUS = "static_interface_valid_blocked"
...
def collect_sandbox_writer_invariant_failures(root: Path) -> List[str]:
    ...
    release_candidate_publication_dry_run_sandbox_boundary = (
        root / RELEASE_CANDIDATE_PUBLICATION_DRY_RUN_SANDBOX_BOUNDARY_REL
    )
    ...
    if release_candidate_publication_dry_run_approval_request_readiness.exists():
        try:
            approval_request_readiness = json.loads(
                release_candidate_publication_dry_run_approval_request_readiness.read_text(
                    encoding="utf-8-sig"
                )
            )
            if approval_request_readiness.get("schema_version") != "1.0.0":
                failures.append(
                    "release candidate publication dry-run approval request readiness schema_version must be 1.0.0."
                )
            if (
                approval_request_readiness.get("record_type")
                != "RELEASE_CANDIDATE_PUBLICATION_DRY_RUN_APPROVAL_REQUEST_READINESS_v1"
            ):
                failures.append(
                    "release candidate publication dry-run approval request readiness record_type must be RELEASE_CANDIDATE_PUBLICATION_DRY_RUN_APPROVAL_REQUEST_READINESS_v1."
                )
            if (
                str(approval_request_readiness.get("candidate_id", "")).strip()
                != EXPECTED_ROLLUP_NEXT_SLICE_CANDIDATE_ID
            ):
                failures.append(
                    "release candidate publication dry-run approval request readiness candidate_id must be release_candidate_package_publish_dry_run_v1."
                )
            if str(approval_request_readiness.get("candidate_type", "")).strip() != "dry_run":
                failures.append(
                    "release candidate publication dry-run approval request readiness candidate_type must be dry_run."
                )
            if (
                str(
                    approval_request_readiness.get(
                ...
                failures.append(
                    "release candidate publication dry-run approval request readiness must keep approval_request_readiness_status=static_request_readiness_valid_blocked."
                )
            if str(approval_request_readiness.get("admission_status", "")).strip() != "unadmitted":
                failures.append(
                    "release candidate publication dry-run approval request readiness admission_status must remain unadmitted."
                )
            for field in (
                "packet_structurally_complete",
                "packet_complete_for_future_review_template",
            ):
                if approval_request_readiness.get(field) is not True:
                    failures.append(
                        "release candidate publication dry-run approval request readiness must keep field true: "
                        f"{field}"
                    )
            for field in (
                "approval_request_ready",
                ...
                if bool(approval_request_readiness.get(field, False)):
                    failures.append(
                        "release candidate publication dry-run approval request readiness must keep field false: "
                        f"{field}"
                    )
            if (
                str(approval_request_readiness.get("approval_phrase_required", "")).strip()
                != EXPECTED_DRY_RUN_PLAN_APPROVAL_PHRASE
            ):
                failures.append(
                    "release candidate publication dry-run approval request readiness must keep exact approval phrase for release_candidate_package_publish_dry_run_v1."
                )
            if (
                str(approval_request_readiness.get("final_recommendation", "")).strip()
                != "do_not_request_approval_yet"
            ):
                failures.append(
                    "release candidate publication dry-run approval request readiness must keep final_recommendation=do_not_request_approval_yet."
                )

            approval_decision_reference = approval_request_readiness.get(
                "approval_decision_reference"
            )
            if isinstance(approval_decision_reference, str) and approval_decision_reference.strip():
                failures.append(
                    "release candidate publication dry-run approval request readiness approval_decision_reference must be null/empty while unadmitted."
                )

            for key in (
                "readiness_summary",
                "structural_completeness_summary",
            ):
                value = str(approval_request_readiness.get(key, "")).strip()
                if not value:
                    failures.append(
                        "release candidate publication dry-run approval request readiness field must be non-empty: "
                        f"{key}"
                    )
            ...
            for key in (
                ...
                values = {
                    str(item).strip()
                    for item in (approval_request_readiness.get(key) or [])
                    if str(item).strip()
                }
                if not values:
                    failures.append(
                        "release candidate publication dry-run approval request readiness list must be non-empty: "
                        f"{key}"
                    )

            request_readiness_blockers = {
                str(item).strip()
                for item in (approval_request_readiness.get("request_readiness_blockers") or [])
                if str(item).strip()
            }
            for token in (
                "approval_request_not_ready",
                ...
                if token not in request_readiness_blockers:
                    failures.append(
                        "release candidate publication dry-run approval request readiness request_readiness_blockers must include: "
                        f"{token}"
                    )

            approval_blockers = {
                str(item).strip()
                for item in (approval_request_readiness.get("approval_blockers") or [])
                if str(item).strip()
            }
            for token in ("missing_approval_decision", "operator_approval_not_granted"):
                if token not in approval_blockers:
                    failures.append(
                        "release candidate publication dry-run approval request readiness approval_blockers must include: "
                        f"{token}"
                    )

            execution_blockers = {
                str(item).strip()
                for item in (approval_request_readiness.get("execution_blockers") or [])
                if str(item).strip()
            }
            for token in ("dry_run_not_admitted", "dry_run_not_executed"):
                if token not in execution_blockers:
                    failures.append(
                        "release candidate publication dry-run approval request readiness execution_blockers must include: "
                        f"{token}"
                    )

            receipt_blockers = {
                str(item).strip()
                for item in (approval_request_readiness.get("receipt_blockers") or [])
                if str(item).strip()
            }
            for token in ("receipt_not_issued", "rollback_cleanup_evidence_missing"):
                if token not in receipt_blockers:
                    failures.append(
                        "release candidate publication dry-run approval request readiness receipt_blockers must include: "
                        f"{token}"
                    )

            publication_blockers = {
                str(item).strip()
                for item in (approval_request_readiness.get("publication_blockers") or [])
                if str(item).strip()
            }
            for token in (
                "publication_surfaces_blocked_by_policy",
                ...
                if token not in publication_blockers:
                    failures.append(
                        "release candidate publication dry-run approval request readiness publication_blockers must include: "
                        f"{token}"
                    )

            production_readiness_blockers = {
                str(item).strip()
                for item in (approval_request_readiness.get("production_readiness_blockers") or [])
                if str(item).strip()
            }
            for token in (
                ...
                if token not in production_readiness_blockers:
                    failures.append(
                        "release candidate publication dry-run approval request readiness production_readiness_blockers must include: "
                        f"{token}"
                    )

            required_validation_commands = {
                str(item).strip()
                for item in (approval_request_readiness.get("required_validation_commands") or [])
                if str(item).strip()
            }
            for command_path in (
                "tools/execution-admission/validate_execution_admission_candidate_matrix.py",
                ...
                if command_path not in required_validation_commands:
                    failures.append(
                        "release candidate publication dry-run approval request readiness required_validation_commands must include: "
                        f"{command_path}"
                    )

            source_artifacts = (
                approval_request_readiness.get("source_artifacts")
                if isinstance(approval_request_readiness.get("source_artifacts"), dict)
                else {}
            )
            expected_source_refs = {
                "candidate_matrix_ref": EXECUTION_ADMISSION_CANDIDATE_MATRIX_REL,
            ...
            for field, expected_path in expected_source_refs.items():
                ...
                if not actual_ref:
                    failures.append(
                        "release candidate publication dry-run approval request readiness source_artifacts is missing field: "
                        f"{field}"
                    )
                    continue
                if actual_ref != expected_path:
                    failures.append(
                        "release candidate publication dry-run approval request readiness source_artifacts field has unexpected path: "
                        f"{field} -> {actual_ref}"
                    )

            source_status = (
                approval_request_readiness.get("source_artifact_validation_status")
                if isinstance(approval_request_readiness.get("source_artifact_validation_status"), dict)
                else {}
            )
            for field in (
                "candidate_matrix_status",
                ...
                if str(source_status.get(field, "")).strip() != "pass":
                    failures.append(
                        "release candidate publication dry-run approval request readiness source_artifact_validation_status must keep field pass: "
                        f"{field}"
                    )

            text = json.dumps(approval_request_readiness, sort_keys=True).lower()
            for forbidden in (
                "publish allowed",
                "spawn allowed",
                ...
                if forbidden in text:
                    failures.append(
                        "release candidate publication dry-run approval request readiness contains forbidden allowance/claim phrase: "
                        f"{forbidden}"
                    )

            if bool(approval_request_readiness.get("unsafe_claims_detected", False)):
                failures.append(
                    "release candidate publication dry-run approval request readiness must keep unsafe_claims_detected=false."
                )
        except Exception as exc:  # pragma: no cover - defensive failure surface
            failures.append(
    ...
    if release_candidate_publication_dry_run_sandbox_boundary.exists():
        try:
            sandbox_boundary = json.loads(
                release_candidate_publication_dry_run_sandbox_boundary.read_text(
                    encoding="utf-8-sig"
                )
            )
            if sandbox_boundary.get("schema_version") != "1.0.0":
                failures.append(
                    "release candidate publication dry-run sandbox boundary schema_version must be 1.0.0."
                )
            if (
                sandbox_boundary.get("record_type")
                != "RELEASE_CANDIDATE_PUBLICATION_DRY_RUN_SANDBOX_BOUNDARY_v1"
            ):
                failures.append(
                    "release candidate publication dry-run sandbox boundary record_type must be RELEASE_CANDIDATE_PUBLICATION_DRY_RUN_SANDBOX_BOUNDARY_v1."
                )
            if (
                str(sandbox_boundary.get("candidate_id", "")).strip()
                != EXPECTED_ROLLUP_NEXT_SLICE_CANDIDATE_ID
            ):
                failures.append(
                    "release candidate publication dry-run sandbox boundary candidate_id must be release_candidate_package_publish_dry_run_v1."
                )
            if str(sandbox_boundary.get("candidate_type", "")).strip() != "dry_run":
                failures.append(
                    "release candidate publication dry-run sandbox boundary candidate_type must be dry_run."
                )
            if (
                str(sandbox_boundary.get("sandbox_boundary_status", "")).strip()
                != EXPECTED_DRY_RUN_SANDBOX_BOUNDARY_STATUS
            ):
                failures.append(
                    "release candidate publication dry-run sandbox boundary must keep sandbox_boundary_status=static_boundary_valid_blocked."
                )
            if str(sandbox_boundary.get("admission_status", "")).strip() != "unadmitted":
                failures.append(
                    "release candidate publication dry-run sandbox boundary must keep admission_status=unadmitted."
                )
            if bool(sandbox_boundary.get("runner_implemented", False)):
                failures.append(
                    "release candidate publication dry-run sandbox boundary must keep runner_implemented=false."
                )
            for field in (
                "approval_request_ready",
                "operator_approval_granted",
                ...
                if bool(sandbox_boundary.get(field, False)):
                    failures.append(
                        "release candidate publication dry-run sandbox boundary must keep field false: "
                        f"{field}"
                    )
            ...
            for key in (
                ...
                values = {
                    str(item).strip()
                    for item in (sandbox_boundary.get(key) or [])
                    if str(item).strip()
                }
                if not values:
                    failures.append(
                        "release candidate publication dry-run sandbox boundary list must be non-empty: "
                        f"{key}"
                    )
            if not isinstance(sandbox_boundary.get("required_output_index"), dict) or not sandbox_boundary.get("required_output_index"):
                failures.append(
                    "release candidate publication dry-run sandbox boundary must keep required_output_index present."
                )
            if not isinstance(sandbox_boundary.get("required_hashing"), dict) or not sandbox_boundary.get("required_hashing"):
                failures.append(
                    "release candidate publication dry-run sandbox boundary must keep required_hashing present."
                )
            runner_constraints = (
                sandbox_boundary.get("runner_interface_constraints")
                if isinstance(sandbox_boundary.get("runner_interface_constraints"), dict)
                else {}
            )
            if not runner_constraints:
                failures.append(
                    "release candidate publication dry-run sandbox boundary must keep runner_interface_constraints present."
                )
            if bool(runner_constraints.get("runner_implemented", False)):
                failures.append(
                    "release candidate publication dry-run sandbox boundary runner_interface_constraints must keep runner_implemented=false."
                )
            if runner_constraints.get("future_runner_requires_separate_proposal") is not True:
                failures.append(
                    "release candidate publication dry-run sandbox boundary runner_interface_constraints must require a separate future runner proposal."
                )

            source_artifacts = (
                sandbox_boundary.get("source_artifacts")
                if isinstance(sandbox_boundary.get("source_artifacts"), dict)
                else {}
            )
            expected_source_refs = {
                "candidate_matrix_ref": EXECUTION_ADMISSION_CANDIDATE_MATRIX_REL,
            ...
            for field, expected_path in expected_source_refs.items():
                ...
                if not actual_ref:
                    failures.append(
                        "release candidate publication dry-run sandbox boundary source_artifacts is missing field: "
                        f"{field}"
                    )
                    continue
                if actual_ref != expected_path:
                    failures.append(
                        "release candidate publication dry-run sandbox boundary source_artifacts field has unexpected path: "
                        f"{field} -> {actual_ref}"
                    )

            source_status = (
                sandbox_boundary.get("source_artifact_validation_status")
                if isinstance(sandbox_boundary.get("source_artifact_validation_status"), dict)
                else {}
            )
            for field in (
                "candidate_matrix_status",
                ...
                if str(source_status.get(field, "")).strip() != "pass":
                    failures.append(
                        "release candidate publication dry-run sandbox boundary source_artifact_validation_status must keep field pass: "
                        f"{field}"
                    )

            text = json.dumps(sandbox_boundary, sort_keys=True).lower()
            for forbidden in (
                "publish allowed",
                "spawn allowed",
                "production path writes allowed",
                ...
                "authoritative asset id claims allowed",
                "authoritative product id claims allowed",
                "sandbox boundary contract equals approval",
                "sandbox boundary contract equals approval-ready",
                "sandbox boundary contract equals dry-run admission",
                "sandbox boundary contract equals runner implementation",
                "sandbox boundary contract equals receipt issuance",
                "sandbox boundary contract equals execution admission",
                "sandbox boundary contract equals publication admission",
                "sandbox boundary contract equals production_ready",
                "runner implemented",
                "dry-run executed",
            ):
                if forbidden in text:
                    failures.append(
                        "release candidate publication dry-run sandbox boundary contains forbidden allowance/claim phrase: "
                        f"{forbidden}"
                    )

            if bool(sandbox_boundary.get("unsafe_claims_detected", False)):
                failures.append(
                    "release candidate publication dry-run sandbox boundary must keep unsafe_claims_detected=false."
                )
        except Exception as exc:  # pragma: no cover - defensive failure surface
            failures.append(
                "release candidate publication dry-run sandbox boundary is not valid JSON: "
                f"{exc}"
            )

    if release_candidate_publication_dry_run_runner_interface.exists():
        try:
            runner_interface = json.loads(
                release_candidate_publication_dry_run_runner_interface.read_text(
                    encoding="utf-8-sig"
                )
            )
            if runner_interface.get("schema_version") != "1.0.0":
                failures.append(
                    "release candidate publication dry-run runner interface schema_version must be 1.0.0."
                )
            if (
                runner_interface.get("record_type")
                != "RELEASE_CANDIDATE_PUBLICATION_DRY_RUN_RUNNER_INTERFACE_v1"
            ):
                failures.append(
                    "release candidate publication dry-run runner interface record_type must be RELEASE_CANDIDATE_PUBLICATION_DRY_RUN_RUNNER_INTERFACE_v1."
                )
            if (
                str(runner_interface.get("candidate_id", "")).strip()
                != EXPECTED_ROLLUP_NEXT_SLICE_CANDIDATE_ID
            ):
                failures.append(
                    "release candidate publication dry-run runner interface candidate_id must be release_candidate_package_publish_dry_run_v1."
                )
            if str(runner_interface.get("candidate_type", "")).strip() != "dry_run":
                failures.append(
                    "release candidate publication dry-run runner interface candidate_type must be dry_run."
                )
            if (
                str(runner_interface.get("runner_interface_status", "")).strip()
                != EXPECTED_DRY_RUN_RUNNER_INTERFACE_STATUS
            ):
                failures.append(
                    "release candidate publication dry-run runner interface must keep runner_interface_status=static_interface_valid_blocked."
                )
            if str(runner_interface.get("admission_status", "")).strip() != "unadmitted":
                failures.append(
                    "release candidate publication dry-run runner interface must keep admission_status=unadmitted."
                )
            if bool(runner_interface.get("runner_implemented", False)):
                failures.append(
                    "release candidate publication dry-run runner interface must keep runner_implemented=false."
                )
            if bool(runner_interface.get("runner_admitted", False)):
                failures.append(
                    "release candidate publication dry-run runner interface must keep runner_admitted=false."
                )
            if bool(runner_interface.get("runner_executed", False)):
                failures.append(
                    "release candidate publication dry-run runner interface must keep runner_executed=false."
                )
            for field in (
                "approval_request_ready",
                "operator_approval_granted",
                ...
                if bool(runner_interface.get(field, False)):
                    failures.append(
                        "release candidate publication dry-run runner interface must keep field false: "
                        f"{field}"
                    )

            if str(runner_interface.get("current_lifecycle_state", "")).strip() not in {
                "not_started",
                "blocked_failed_safe",
                "invalidated_failed_safe",
            }:
                failures.append(
                    "release candidate publication dry-run runner interface current_lifecycle_state must remain safe and blocked."
                )

            current_outputs = runner_interface.get("current_emitted_outputs")
            if not isinstance(current_outputs, list) or current_outputs:
                failures.append(
                    "release candidate publication dry-run runner interface current_emitted_outputs must remain an empty list."
                )
            ...
            for key in (
                ...
                values = {
                    str(item).strip()
                    for item in (runner_interface.get(key) or [])
                    if str(item).strip()
                }
                if not values:
                    failures.append(
                        "release candidate publication dry-run runner interface list must be non-empty: "
                        f"{key}"
                    )

            source_artifacts = (
                runner_interface.get("source_artifacts")
                if isinstance(runner_interface.get("source_artifacts"), dict)
                else {}
            )
            expected_source_refs = {
                "candidate_matrix_ref": EXECUTION_ADMISSION_CANDIDATE_MATRIX_REL,
            ...
            for field, expected_path in expected_source_refs.items():
                ...
                if not actual_ref:
                    failures.append(
                        "release candidate publication dry-run runner interface source_artifacts is missing field: "
                        f"{field}"
                    )
                    continue
                if actual_ref != expected_path:
                    failures.append(
                        "release candidate publication dry-run runner interface source_artifacts field has unexpected path: "
                        f"{field} -> {actual_ref}"
                    )

            source_status = (
                runner_interface.get("source_artifact_validation_status")
                if isinstance(runner_interface.get("source_artifact_validation_status"), dict)
                else {}
            )
            for field in (
                "candidate_matrix_status",
                ...
                if str(source_status.get(field, "")).strip() != "pass":
                    failures.append(
                        "release candidate publication dry-run runner interface source_artifact_validation_status must keep field pass: "
                        f"{field}"
                    )

            text = json.dumps(runner_interface, sort_keys=True).lower()
            for forbidden in (
                "publish allowed",
                "spawn allowed",
                "production path writes allowed",
                ...
                "runner interface contract equals approval",
                ...
                if forbidden in text:
                    failures.append(
                        "release candidate publication dry-run runner interface contains forbidden allowance/claim phrase: "
                        f"{forbidden}"
                    )

            if bool(runner_interface.get("unsafe_claims_detected", False)):
                failures.append(
                    "release candidate publication dry-run runner interface must keep unsafe_claims_detected=false."
                )
        except Exception as exc:  # pragma: no cover - defensive failure surface
            failures.append(
