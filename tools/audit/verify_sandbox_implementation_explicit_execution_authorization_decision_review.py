#!/usr/bin/env python3
"""Verify sandbox implementation explicit execution-authorization decision review artifacts."""
...
REQUIRED_REVIEW_NEEDLES = [
    "future implementation-authorizing decision",
...
REQUIRED_SAFETY = {
    ...
    "explicit_execution_authorization_progression_allowed_by_default": False,
...
def main() -> int:
    ...
    if package.get("default_decision_status") != "explicit_execution_authorization_review_hold":
        failures.append(
            "default_decision_status must be explicit_execution_authorization_review_hold"
        )
    ...
    if template_path.exists():
        try:
            ...
            if (
                ...
                failures.append(
                    "decision template decision_status must be explicit_execution_authorization_review_hold"
                )
            ...
            if template.get("explicit_execution_authorization_progression_allowed") is not False:
                failures.append(
                    "decision template explicit_execution_authorization_progression_allowed must be false"
                )
        except Exception as exc:
    ...
    if failures:
        print(
            "FAIL: sandbox implementation explicit execution-authorization decision review verification failed."
        )
    ...
    print(
        "PASS: sandbox implementation explicit execution-authorization decision review verification passed."
    )
    return 0
