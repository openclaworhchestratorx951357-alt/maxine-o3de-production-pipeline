#!/usr/bin/env python3
"""Verify sandbox implementation execution-authorization decision artifacts."""
...
REQUIRED_DOC_NEEDLES = [
    "decision: execution_authorization_decision_review_pending_explicit_authorization_decision",
...
def main() -> int:
    ...
    decision_doc = (
        root / "docs/reviews/SANDBOX-IMPLEMENTATION-EXECUTION-AUTHORIZATION-DECISION.md"
    )
    decision_json = (
    ...
    if status not in ALLOWED_DECISION_STATUS:
        failures.append(
            "decision_status must be one of: execution_authorization_decision_review_hold, execution_authorization_decision_review_rejected, execution_authorization_decision_review_pending_explicit_authorization_decision"
        )
    ...
    if accepted:
        if decision.get("execution_hold_transition_approved") is not True:
            failures.append(
                "execution_hold_transition_approved must be true when decision_status is execution_authorization_decision_review_pending_explicit_authorization_decision"
            )
        if (
            decision.get("explicit_execution_authorization_decision_preparation_allowed")
            is not True
        ):
            failures.append(
                "explicit_execution_authorization_decision_preparation_allowed must be true when decision_status is execution_authorization_decision_review_pending_explicit_authorization_decision"
            )
    else:
        ...
        if (
            ...
            failures.append(
                "explicit_execution_authorization_decision_preparation_allowed must be false for hold/rejected statuses"
            )
    ...
    if (
        ...
        failures.append(
            "review_package must reference docs/reviews/sandbox_implementation_execution_authorization_decision_review_package.json"
        )
    ...
    if (
        ...
        failures.append(
            "review_document must reference docs/reviews/SANDBOX-IMPLEMENTATION-EXECUTION-AUTHORIZATION-DECISION-REVIEW.md"
        )
    ...
    if (
        ...
        failures.append(
            "review package default_decision_status must be execution_authorization_decision_review_hold"
        )
    ...
    if prior_decision.exists():
        try:
            ...
            if (
                ...
                failures.append(
                    "prior authorization decision status must be execution_authorization_review_pending_decision_preparation_only"
                )
        except Exception as exc:
            failures.append(f"unable to parse prior authorization decision: {exc}")
    else:
        failures.append(
            "required prior authorization decision missing: docs/reviews/sandbox_implementation_execution_readiness_authorization_decision.json"
        )
    ...
    if failures:
        print(
            "FAIL: sandbox implementation execution-authorization decision verification failed."
        )
        for item in failures:
            print(f" - {item}")
        return 1

    print("PASS: sandbox implementation execution-authorization decision verification passed.")
    return 0
