#!/usr/bin/env python3
"""Verify sandbox implementation execution-authorization decision review artifacts."""
...
REQUIRED_REVIEW_NEEDLES = [
    "future explicit execution-authorization decision",
...
REQUIRED_SAFETY = {
    ...
    "execution_authorization_decision_preparation_status_required": True,
    "explicit_execution_authorization_decision_allowed_by_default": False,
...
def main() -> int:
    ...
    if (
        package.get("default_decision_status")
        != "execution_authorization_decision_review_hold"
    ):
        failures.append(
            "default_decision_status must be execution_authorization_decision_review_hold"
        )
    ...
    if authorization_decision_path.exists():
        try:
            authorization_decision = load_json(authorization_decision_path)
            if (
                authorization_decision.get("decision_status")
                != "execution_authorization_review_pending_decision_preparation_only"
            ):
                failures.append(
                    "authorization decision status must be execution_authorization_review_pending_decision_preparation_only"
                )
            if (
                authorization_decision.get("execution_hold_current_status")
                != "hold_active"
            ):
                failures.append(
                    "authorization decision execution_hold_current_status must be hold_active"
                )
            if (
                authorization_decision.get("execution_hold_next_status")
                != "hold_active"
            ):
                failures.append(
                    "authorization decision execution_hold_next_status must be hold_active"
                )
        except Exception as exc:
            failures.append(f"unable to parse authorization decision JSON: {exc}")
    else:
        failures.append(
            "required authorization decision missing: docs/reviews/sandbox_implementation_execution_readiness_authorization_decision.json"
        )
    ...
    if template_path.exists():
        try:
            ...
            if (
                template.get("decision_status")
                != "execution_authorization_decision_review_hold"
            ):
                failures.append(
                    "decision template decision_status must be execution_authorization_decision_review_hold"
                )
            ...
            if (
                template.get("execution_authorization_decision_preparation_status_required")
                is not True
            ):
                failures.append(
                    "decision template execution_authorization_decision_preparation_status_required must be true"
                )
            if (
                template.get("explicit_execution_authorization_decision_preparation_allowed")
                is not False
            ):
                failures.append(
                    "decision template explicit_execution_authorization_decision_preparation_allowed must be false"
                )
        except Exception as exc:
    ...
    if failures:
        print(
            "FAIL: sandbox implementation execution-authorization decision review verification failed."
        )
    ...
    print(
        "PASS: sandbox implementation execution-authorization decision review verification passed."
    )
    return 0
