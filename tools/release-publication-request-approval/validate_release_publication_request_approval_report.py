#!/usr/bin/env python3
"""Validate evidence-only release publication request approval reports."""
...
EXPECTED_REPORT_TYPE = "RELEASE_PUBLICATION_REQUEST_APPROVAL_v1_REPORT"
EXPECTED_CONTRACT_ID = "RELEASE_PUBLICATION_REQUEST_APPROVAL_v1"
EXPECTED_REQUEST_ACTION = "publish_release_bundle"
TARGET_PATH = "qc.gates[]"
FUTURE_TARGET_PATH = "qc.checks[]"
CHECK_ID = "release_publication_request_approval_v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate MAXINE release publication request approval report JSON."
    )
    parser.add_argument("report_path", help="Path to release publication request approval report JSON")
    parser.add_argument(
        "--allow-warn",
        action="store_true",
...
def main() -> int:
    ...
    schema_path = repo_root / "schemas" / "maxine_release_publication_request_approval_report.schema.json"
    ...
    request_contract = report.get("request_contract", {}) if isinstance(report.get("request_contract"), dict) else {}
    contract_id = str(request_contract.get("contract_id", "")).strip()
    required_gate_ids = request_contract.get("required_gate_ids", [])
    required_approvals = request_contract.get("required_approvals")
    manual_review_required = request_contract.get("manual_review_required")
    ...
    if contract_id != EXPECTED_CONTRACT_ID:
        add_finding(
            ...
            "error",
            "open",
            f"request_contract.contract_id must be {EXPECTED_CONTRACT_ID}.",
            {"actual": contract_id},
        )
    if not _is_string_array(required_gate_ids):
        add_finding(
            ...
            "error",
            "open",
            "request_contract.required_gate_ids must be a non-empty string array.",
        )
        required_gate_ids = []
    ...
    if not isinstance(required_approvals, int) or required_approvals < 1:
        add_finding(
            ...
            "error",
            "open",
            "request_contract.required_approvals must be integer >= 1.",
    ...
    if not isinstance(manual_review_required, bool):
        add_finding(
            ...
            "error",
            "open",
            "request_contract.manual_review_required must be boolean.",
    ...
    if request_contract.get("explicit_publish_execution_request_required") is not True:
        add_finding(
            findings,
            "explicit_publish_execution_request_required",
            "error",
            "open",
            "request_contract.explicit_publish_execution_request_required must be true.",
            {"actual": request_contract.get("explicit_publish_execution_request_required")},
        )
    if request_contract.get("evidence_only") is not True:
        add_finding(
            findings,
            "evidence_only_required",
            "error",
            "open",
            "request_contract.evidence_only must be true.",
            {"actual": request_contract.get("evidence_only")},
        )
    if request_contract.get("execution_admitted") is not False:
        add_finding(
            findings,
            "execution_admitted_must_be_false",
            "error",
            "open",
            "request_contract.execution_admitted must be false.",
            {"actual": request_contract.get("execution_admitted")},
        )

    publication_request = report.get("publication_request", {}) if isinstance(report.get("publication_request"), dict) else {}
    requested_action = str(publication_request.get("requested_action", "")).strip()
    publish_request_command_display = str(publication_request.get("publish_request_command_display", "")).strip()
    rollback_request_command_display = str(publication_request.get("rollback_request_command_display", "")).strip()
    dry_run_evidence_paths = publication_request.get("dry_run_evidence_paths", [])
    ...
    if requested_action != EXPECTED_REQUEST_ACTION:
        add_finding(
            ...
            "error",
            "open",
            f"publication_request.requested_action must be {EXPECTED_REQUEST_ACTION}.",
            {"actual": requested_action},
        )

    for field_name, command_display in (
        ("publish_request_command_display", publish_request_command_display),
        ("rollback_request_command_display", rollback_request_command_display),
    ):
        if not command_display:
            add_finding(
                ...
                "error",
                "open",
                f"publication_request.{field_name} is required.",
            )
            continue
        if "displayonly" not in command_display.lower():
            add_finding(
                ...
                "error",
                "open",
                f"publication_request.{field_name} must be display-only text.",
        ...
        if _contains_unsafe_command_tokens(command_display):
            add_finding(
                ...
                "error",
                "open",
                f"publication_request.{field_name} contains unsafe execution tokens.",
                {"value": command_display},
            )

    if publication_request.get("no_command_execution_recorded") is not True:
        add_finding(
            findings,
            "no_command_execution_recorded_required",
            "error",
            "open",
            "publication_request.no_command_execution_recorded must be true.",
            {"actual": publication_request.get("no_command_execution_recorded")},
        )
    ...
    if not _is_string_array(dry_run_evidence_paths):
        add_finding(
            ...
            "error",
            "open",
            "publication_request.dry_run_evidence_paths must be a non-empty string array.",
        )
    ...
    for ev_path in dry_run_evidence_paths:
        if _contains_unsafe_path_tokens(ev_path):
            add_finding(
                ...
                "error",
                "open",
                "publication_request.dry_run_evidence_paths contains unsafe traversal or absolute-path tokens.",
                {"path": ev_path},
            )
    ...
    if decision not in {"approved", "pending", "rejected"}:
        add_finding(
            ...
            "error",
            "open",
            "approval_state.decision must be approved|pending|rejected.",
            {"actual": decision},
        )
        decision = "pending"
    ...
    if not _is_string_array(approver_ids) and approver_ids != []:
        add_finding(
            ...
            "error",
            "open",
            "approval_state.approver_ids must be a string array.",
        )
        approver_ids = []
    if not _is_string_array(blocked_reason_codes) and blocked_reason_codes != []:
        add_finding(
            ...
            "error",
            "open",
            "approval_state.blocked_reason_codes must be a string array.",
        )
    ...
    if required_gate_ids and readiness_required_gate_ids and required_gate_ids != readiness_required_gate_ids:
        add_finding(
            ...
            "warning",
            "open",
            "request_contract.required_gate_ids does not match readiness.required_gate_ids.",
    ...
    if computed_missing_gate_ids:
        add_finding(
            ...
            "error",
            "open",
            "Required release publication request gates are missing.",
            {"missing_gate_ids": computed_missing_gate_ids},
        )

    if release_readiness_state not in {
        "ready_for_manual_publication_execution_request",
    ...
    if release_readiness_state == "ready_for_manual_publication_execution_request" and computed_missing_gate_ids:
        add_finding(
            findings,
            "ready_state_with_missing_gates",
            "error",
            "open",
            "ready_for_manual_publication_execution_request cannot be set when required gates are missing.",
        )
    ...
    if decision == "approved":
        if len(approver_ids) < required_approvals:
            add_finding(
                ...
                "error",
                "open",
                "approval_state.approver_ids count is lower than request_contract.required_approvals.",
        ...
        if blocked_reason_codes:
            add_finding(
                ...
                "error",
                "open",
                "approved decision must not include blocked_reason_codes.",
        ...
        if rollback_plan_verified is not True:
            add_finding(
                ...
                "error",
                "open",
                "approved decision requires rollback_plan_verified=true.",
            )
        if cleanup_plan_verified is not True:
            add_finding(
                ...
                "error",
                "open",
                "approved decision requires cleanup_plan_verified=true.",
            )
        if release_readiness_state != "ready_for_manual_publication_execution_request":
            add_finding(
                findings,
                "approved_without_ready_state",
                "error",
                "open",
                "approved decision requires readiness.release_readiness_state=ready_for_manual_publication_execution_request.",
            )
    elif decision == "pending":
        add_finding(
            findings,
            "publication_request_pending_manual_review",
            "manual_review",
            "open",
            "Publication request remains pending manual approval.",
        )
        ...
        add_finding(
            ...
            "error",
            "open",
            "Publication request decision is rejected.",
        )
