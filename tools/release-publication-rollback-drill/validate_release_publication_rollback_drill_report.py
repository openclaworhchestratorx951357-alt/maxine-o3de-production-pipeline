#!/usr/bin/env python3
"""Validate evidence-only release publication rollback drill reports."""
...
EXPECTED_REPORT_TYPE = "RELEASE_PUBLICATION_ROLLBACK_DRILL_v1_REPORT"
EXPECTED_CONTRACT_ID = "RELEASE_PUBLICATION_ROLLBACK_DRILL_v1"
EXPECTED_DRILL_ACTION = "record_manual_publication_rollback_drill"
EXPECTED_READY_DECISION = "rollback_drill_recorded_for_audit"
TARGET_PATH = "qc.gates[]"
FUTURE_TARGET_PATH = "qc.checks[]"
CHECK_ID = "release_publication_rollback_drill_v1"
...
REQUIRED_ARTIFACT_HINTS = {
    ...
    "rollback_log": "rollback-drill-manual.log",
    "post_rollback_validation": "post-rollback-validation.log",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate MAXINE release publication rollback drill report JSON."
    )
    parser.add_argument("report_path", help="Path to release publication rollback drill report JSON")
    parser.add_argument(
        "--allow-warn",
        action="store_true",
...
def main() -> int:
    ...
    schema_path = repo_root / "schemas" / "maxine_release_publication_rollback_drill_report.schema.json"
    ...
    rollback_contract = report.get("rollback_contract", {}) if isinstance(report.get("rollback_contract"), dict) else {}
    contract_id = str(rollback_contract.get("contract_id", "")).strip()
    required_gate_ids = rollback_contract.get("required_gate_ids", [])
    required_approvals = rollback_contract.get("required_approvals")
    manual_review_required = rollback_contract.get("manual_review_required")

    if contract_id != EXPECTED_CONTRACT_ID:
        add_finding(findings, "contract_id_invalid", "error", "open", f"rollback_contract.contract_id must be {EXPECTED_CONTRACT_ID}.", {"actual": contract_id})
    if not _is_string_array(required_gate_ids):
        add_finding(findings, "required_gate_ids_invalid", "error", "open", "rollback_contract.required_gate_ids must be a non-empty string array.")
        required_gate_ids = []
    ...
    if not isinstance(required_approvals, int) or required_approvals < 1:
        add_finding(findings, "required_approvals_invalid", "error", "open", "rollback_contract.required_approvals must be integer >= 1.", {"actual": required_approvals})
        required_approvals = 1

    if not isinstance(manual_review_required, bool):
        add_finding(findings, "manual_review_required_invalid", "error", "open", "rollback_contract.manual_review_required must be boolean.", {"actual": manual_review_required})
    ...
    if rollback_contract.get("immutable_audit_bundle_required") is not True:
        add_finding(findings, "immutable_audit_bundle_required", "error", "open", "rollback_contract.immutable_audit_bundle_required must be true.", {"actual": rollback_contract.get("immutable_audit_bundle_required")})
    if rollback_contract.get("external_execution_evidence_required") is not True:
        add_finding(findings, "external_execution_evidence_required", "error", "open", "rollback_contract.external_execution_evidence_required must be true.", {"actual": rollback_contract.get("external_execution_evidence_required")})
    if rollback_contract.get("evidence_only") is not True:
        add_finding(findings, "evidence_only_required", "error", "open", "rollback_contract.evidence_only must be true.", {"actual": rollback_contract.get("evidence_only")})
    if rollback_contract.get("execution_admitted") is not False:
        add_finding(findings, "execution_admitted_must_be_false", "error", "open", "rollback_contract.execution_admitted must be false.", {"actual": rollback_contract.get("execution_admitted")})

    rollback_drill = report.get("rollback_drill", {}) if isinstance(report.get("rollback_drill"), dict) else {}
    drill_action = str(rollback_drill.get("drill_action", "")).strip()
    rollback_result = str(rollback_drill.get("rollback_result", "")).strip()
    publish_command_display = str(rollback_drill.get("publish_command_display", "")).strip()
    rollback_command_display = str(rollback_drill.get("rollback_command_display", "")).strip()
    rollback_drill_log_path = str(rollback_drill.get("rollback_drill_log_path", "")).strip()
    post_rollback_validation_log_path = str(rollback_drill.get("post_rollback_validation_log_path", "")).strip()
    rollback_drill_log_sha = str(rollback_drill.get("rollback_drill_log_sha256", "")).strip()
    post_rollback_validation_sha = str(rollback_drill.get("post_rollback_validation_log_sha256", "")).strip()
    evidence_bundle_path = str(rollback_drill.get("evidence_bundle_path", "")).strip()
    evidence_bundle_sha = str(rollback_drill.get("evidence_bundle_sha256", "")).strip()
    evidence_bundle_artifacts = rollback_drill.get("evidence_bundle_artifacts", [])

    if drill_action != EXPECTED_DRILL_ACTION:
        add_finding(findings, "drill_action_invalid", "error", "open", f"rollback_drill.drill_action must be {EXPECTED_DRILL_ACTION}.", {"actual": drill_action})

    if rollback_result not in {
        "manual_rollback_reported_success",
        "manual_rollback_reported_warn",
        "manual_rollback_reported_fail",
        "pending_manual_rollback",
        "manual_rollback_reported_blocked",
    }:
        add_finding(findings, "rollback_result_invalid", "error", "open", "rollback_drill.rollback_result is invalid.", {"actual": rollback_result})
    ...
    for field_name, command_display in (("publish_command_display", publish_command_display), ("rollback_command_display", rollback_command_display)):
        if not command_display:
            add_finding(findings, f"{field_name}_missing", "error", "open", f"rollback_drill.{field_name} is required.")
            continue
        if "displayonly" not in command_display.lower():
            add_finding(findings, f"{field_name}_must_be_display_only", "error", "open", f"rollback_drill.{field_name} must be display-only text.", {"value": command_display})
        if _contains_unsafe_command_tokens(command_display):
            add_finding(findings, f"{field_name}_unsafe", "error", "open", f"rollback_drill.{field_name} contains unsafe execution tokens.", {"value": command_display})

    if rollback_drill.get("no_command_execution_recorded") is not True:
        add_finding(findings, "no_command_execution_recorded_required", "error", "open", "rollback_drill.no_command_execution_recorded must be true.", {"actual": rollback_drill.get("no_command_execution_recorded")})

    for label, path_text in (
        ("rollback_drill_log_path", rollback_drill_log_path),
        ("post_rollback_validation_log_path", post_rollback_validation_log_path),
        ("evidence_bundle_path", evidence_bundle_path),
    ):
        if not path_text:
            add_finding(findings, f"{label}_missing", "error", "open", f"rollback_drill.{label} is required.")
            continue
        if _contains_unsafe_path_tokens(path_text):
            add_finding(findings, f"{label}_unsafe", "error", "open", f"rollback_drill.{label} contains unsafe traversal or absolute-path tokens.", {"path": path_text})

    for label, sha_text in (
        ("rollback_drill_log_sha256", rollback_drill_log_sha),
        ("post_rollback_validation_log_sha256", post_rollback_validation_sha),
        ("evidence_bundle_sha256", evidence_bundle_sha),
    ):
        if not _is_likely_sha256(sha_text):
            add_finding(findings, f"{label}_invalid", "error", "open", f"rollback_drill.{label} must look like sha256 hex.", {"actual": sha_text})

    if not _is_string_array(evidence_bundle_artifacts):
        add_finding(findings, "evidence_bundle_artifacts_invalid", "error", "open", "rollback_drill.evidence_bundle_artifacts must be a non-empty string array.")
    ...
    for artifact_path in evidence_bundle_artifacts:
        if _contains_unsafe_path_tokens(artifact_path):
            add_finding(findings, "evidence_bundle_artifact_path_unsafe", "error", "open", "rollback_drill.evidence_bundle_artifacts contains unsafe path tokens.", {"path": artifact_path})
        ...
        if any(token in lowered for token in ("cache/", "cache\\", "assetdb.sqlite", ".fbx", ".gltf", ".glb", ".obj", ".png", ".jpg", ".jpeg", ".exe", ".dll", ".bin", ".sqlite")):
            add_finding(findings, "evidence_bundle_artifact_disallowed", "error", "open", "rollback_drill.evidence_bundle_artifacts includes disallowed binary/runtime/cache/database artifact.", {"path": artifact_path})

    for hint_id, hint_token in REQUIRED_ARTIFACT_HINTS.items():
        if not any(hint_token in item.lower() for item in evidence_bundle_artifacts):
            add_finding(
                findings,
                f"required_artifact_missing_{hint_id}",
                "error",
                "open",
                "rollback_drill.evidence_bundle_artifacts is missing required rollback evidence artifact.",
                {"required_hint": hint_token},
            )

    if rollback_drill.get("evidence_bundle_immutable") is not True:
        add_finding(findings, "evidence_bundle_immutable_required", "error", "open", "rollback_drill.evidence_bundle_immutable must be true.", {"actual": rollback_drill.get("evidence_bundle_immutable")})
    ...
    if decision not in {EXPECTED_READY_DECISION, "pending_manual_review", "rejected"}:
        add_finding(findings, "approval_decision_invalid", "error", "open", "approval_state.decision must be rollback_drill_recorded_for_audit|pending_manual_review|rejected.", {"actual": decision})
    ...
    rollback_readiness_state = str(readiness.get("rollback_readiness_state", "")).strip()
    ...
    if required_gate_ids and readiness_required_gate_ids and required_gate_ids != readiness_required_gate_ids:
        add_finding(findings, "required_gate_ids_mismatch", "warning", "open", "rollback_contract.required_gate_ids does not match readiness.required_gate_ids.", {"contract_required_gate_ids": required_gate_ids, "readiness_required_gate_ids": readiness_required_gate_ids})
    ...
    if computed_missing_gate_ids:
        add_finding(findings, "required_gates_missing", "error", "open", "Required rollback drill gates are missing.", {"missing_gate_ids": computed_missing_gate_ids})

    if rollback_readiness_state not in {"rollback_drill_recorded_for_audit", "blocked_missing_evidence", "blocked_safety_boundary"}:
        add_finding(findings, "rollback_readiness_state_invalid", "error", "open", "readiness.rollback_readiness_state is invalid.", {"actual": rollback_readiness_state})

    if rollback_readiness_state == "rollback_drill_recorded_for_audit" and computed_missing_gate_ids:
        add_finding(findings, "ready_state_with_missing_gates", "error", "open", "rollback_drill_recorded_for_audit cannot be set when required gates are missing.")

    if decision == EXPECTED_READY_DECISION:
        if len(approver_ids) < required_approvals:
            add_finding(findings, "insufficient_approvals", "error", "open", "approval_state.approver_ids count is lower than rollback_contract.required_approvals.", {"approver_ids": approver_ids, "required_approvals": required_approvals})
        if blocked_reason_codes:
            add_finding(findings, "ready_with_blocked_reasons", "error", "open", "rollback_drill_recorded_for_audit must not include blocked_reason_codes.", {"blocked_reason_codes": blocked_reason_codes})
        if rollback_plan_verified is not True:
            add_finding(findings, "rollback_plan_not_verified", "error", "open", "rollback_drill_recorded_for_audit requires rollback_plan_verified=true.")
        if cleanup_plan_verified is not True:
            add_finding(findings, "cleanup_plan_not_verified", "error", "open", "rollback_drill_recorded_for_audit requires cleanup_plan_verified=true.")
        if rollback_readiness_state != "rollback_drill_recorded_for_audit":
            add_finding(findings, "ready_without_ready_state", "error", "open", "rollback_drill_recorded_for_audit requires readiness.rollback_readiness_state=rollback_drill_recorded_for_audit.")
    elif decision == "pending_manual_review":
        add_finding(findings, "rollback_drill_pending_manual_review", "manual_review", "open", "Rollback drill remains pending manual review.")
    elif decision == "rejected":
        add_finding(findings, "rollback_drill_rejected", "error", "open", "Rollback drill decision is rejected.")

    if rollback_result == "manual_rollback_reported_success":
        if declared_status != "pass":
            add_finding(findings, "status_should_be_pass_for_success", "error", "open", "manual_rollback_reported_success requires report.status=pass.")
        if decision != EXPECTED_READY_DECISION:
            add_finding(findings, "success_requires_ready_decision", "error", "open", "manual_rollback_reported_success requires decision=rollback_drill_recorded_for_audit.")
    elif rollback_result == "manual_rollback_reported_warn":
        if declared_status != "warn":
            add_finding(findings, "status_should_be_warn_for_warn_result", "error", "open", "manual_rollback_reported_warn requires report.status=warn.")
        if decision != EXPECTED_READY_DECISION:
            add_finding(findings, "warn_requires_ready_decision", "error", "open", "manual_rollback_reported_warn requires decision=rollback_drill_recorded_for_audit.")
    elif rollback_result in {"manual_rollback_reported_fail", "manual_rollback_reported_blocked"}:
        if declared_status != "fail":
            add_finding(findings, "status_should_be_fail_for_fail_result", "error", "open", "manual_rollback_reported_fail|manual_rollback_reported_blocked requires report.status=fail.")
    elif rollback_result == "pending_manual_rollback":
        if declared_status != "pending_manual":
            add_finding(findings, "status_should_be_pending_manual", "error", "open", "pending_manual_rollback requires report.status=pending_manual.")
        if decision != "pending_manual_review":
            add_finding(findings, "pending_result_requires_pending_decision", "error", "open", "pending_manual_rollback requires decision=pending_manual_review.")
