#!/usr/bin/env python3
"""Validate evidence-only release publication execution window ticket reports."""
...
EXPECTED_REPORT_TYPE = "RELEASE_PUBLICATION_EXECUTION_WINDOW_TICKET_v1_REPORT"
EXPECTED_CONTRACT_ID = "RELEASE_PUBLICATION_EXECUTION_WINDOW_TICKET_v1"
EXPECTED_REQUEST_ACTION = "record_release_publication_execution_window_ticket"
EXPECTED_REVIEW_SCOPE = "release_publication_chain"
EXPECTED_READY_DECISION = "execution_window_ticket_recorded_for_future_manual_execution_window"
TARGET_PATH = "qc.gates[]"
FUTURE_TARGET_PATH = "qc.checks[]"
CHECK_ID = "release_publication_execution_window_ticket_v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate MAXINE release publication execution window ticket report JSON."
    )
    parser.add_argument("report_path", help="Path to release publication execution window ticket report JSON")
    parser.add_argument(
        "--allow-warn",
        action="store_true",
...
def main() -> int:
    ...
    schema_path = repo_root / "schemas" / "maxine_release_publication_execution_window_ticket_report.schema.json"
    ...
    required_window_ticket_artifact_ids = readiness_contract.get("required_window_ticket_artifact_ids", [])
    ...
    if not _is_string_array(required_gate_ids):
        ...
        required_gate_ids = []
    if not _is_string_array(required_window_ticket_artifact_ids):
        add_finding(
            findings,
            "required_window_ticket_artifact_ids_invalid",
            "error",
            "open",
            "readiness_contract.required_window_ticket_artifact_ids must be a non-empty string array.",
        )
        required_window_ticket_artifact_ids = []

    required_gate_ids = _normalize_string_list(required_gate_ids)
    required_window_ticket_artifact_ids = _normalize_string_list(required_window_ticket_artifact_ids)
    ...
    if "release_publication_execution_authorization_record" not in required_window_ticket_artifact_ids:
        add_finding(
            findings,
            "required_execution_authorization_record_artifact_missing",
            "error",
            "open",
            "readiness_contract.required_window_ticket_artifact_ids must include release_publication_execution_authorization_record.",
        )
    ...
    if readiness_contract.get("immutable_window_ticket_required") is not True:
        add_finding(
            findings,
            "immutable_window_ticket_required",
            "error",
            "open",
            "readiness_contract.immutable_window_ticket_required must be true.",
            {"actual": readiness_contract.get("immutable_window_ticket_required")},
        )
    ...
    window_ticket_data = report.get("execution_window_ticket", {}) if isinstance(report.get("execution_window_ticket"), dict) else {}
    window_ticket_action = str(window_ticket_data.get("window_ticket_action", "")).strip()
    window_ticket_scope = str(window_ticket_data.get("window_ticket_scope", "")).strip()
    hash_algorithm = str(window_ticket_data.get("hash_algorithm", "")).strip()
    verification_status = str(window_ticket_data.get("window_ticket_verification_status", "")).strip()
    chain_manifest_path = str(window_ticket_data.get("chain_manifest_path", "")).strip()
    window_ticket_path = str(window_ticket_data.get("window_ticket_path", "")).strip()
    window_ticket_sha = str(window_ticket_data.get("window_ticket_sha256", "")).strip()
    source_ready_packet_path = str(
        window_ticket_data.get("source_execution_authorization_record_path", "")
    ).strip()
    source_ready_packet_sha = str(
        window_ticket_data.get("source_execution_authorization_record_sha256", "")
    ).strip()
    window_ticket_artifacts = window_ticket_data.get("window_ticket_artifacts", [])

    if window_ticket_action != EXPECTED_REQUEST_ACTION:
        add_finding(
            findings,
            "window_ticket_action_invalid",
            "error",
            "open",
            f"execution_window_ticket.window_ticket_action must be {EXPECTED_REQUEST_ACTION}.",
            {"actual": window_ticket_action},
        )
    if window_ticket_scope != EXPECTED_REVIEW_SCOPE:
        add_finding(
            findings,
            "window_ticket_scope_invalid",
            "error",
            "open",
            f"execution_window_ticket.window_ticket_scope must be {EXPECTED_REVIEW_SCOPE}.",
            {"actual": window_ticket_scope},
        )

    source_ready_request_id = str(
        window_ticket_data.get("source_execution_authorization_record_id", "")
    ).strip()
    if not source_ready_request_id:
        add_finding(
            ...
            "error",
            "open",
            "execution_window_ticket.source_execution_authorization_record_id is required.",
        )
        ...
        add_finding(
            ...
            "error",
            "open",
            "execution_window_ticket.source_execution_authorization_record_id must reference an execution-authorization-record id.",
    ...
    if hash_algorithm != "sha256":
        add_finding(
            ...
            "error",
            "open",
            "execution_window_ticket.hash_algorithm must be sha256.",
    ...
    if verification_status not in ALLOWED_STATUS:
        add_finding(
            findings,
            "window_ticket_verification_status_invalid",
            "error",
            "open",
            "execution_window_ticket.window_ticket_verification_status must be pass|warn|fail|pending_manual.",
            {"actual": verification_status},
        )

    for label, path_text in (
        ("chain_manifest_path", chain_manifest_path),
        ("window_ticket_path", window_ticket_path),
        ("source_execution_authorization_record_path", source_ready_packet_path),
    ):
        if not path_text:
            add_finding(findings, f"{label}_missing", "error", "open", f"execution_window_ticket.{label} is required.")
            continue
        if _contains_unsafe_path_tokens(path_text):
            add_finding(
                ...
                "error",
                "open",
                f"execution_window_ticket.{label} contains unsafe traversal or absolute-path tokens.",
                {"path": path_text},
            )

    if not _is_likely_sha256(window_ticket_sha):
        add_finding(
            findings,
            "window_ticket_sha256_invalid",
            "error",
            "open",
            "execution_window_ticket.window_ticket_sha256 must look like sha256 hex.",
            {"actual": window_ticket_sha},
        )
    ...
    if not _is_likely_sha256(source_ready_packet_sha):
        add_finding(
            ...
            "error",
            "open",
            "execution_window_ticket.source_execution_authorization_record_sha256 must look like sha256 hex.",
    ...
    if source_ready_packet_path and "release-publication-execution-authorization-record" not in source_ready_packet_path.lower():
        add_finding(
            ...
            "error",
            "open",
            "execution_window_ticket.source_execution_authorization_record_path must reference execution-authorization-record evidence.",
            {"actual": source_ready_packet_path},
        )

    if window_ticket_data.get("no_command_execution_recorded") is not True:
        add_finding(
            findings,
            "no_command_execution_recorded_required",
            "error",
            "open",
            "execution_window_ticket.no_command_execution_recorded must be true.",
            {"actual": window_ticket_data.get("no_command_execution_recorded")},
        )

    artifact_hashes = window_ticket_data.get("window_ticket_artifact_hashes", {})
    if not isinstance(artifact_hashes, dict) or not artifact_hashes:
        add_finding(
            findings,
            "window_ticket_artifact_hashes_invalid",
            "error",
            "open",
            "execution_window_ticket.window_ticket_artifact_hashes must be a non-empty object.",
        )
        artifact_hashes = {}
    ...
    for key, value in artifact_hashes.items():
        ...
        if not key_text:
            add_finding(findings, "artifact_hash_key_missing", "error", "open", "window_ticket_artifact_hashes keys must be non-empty.")
            continue
        ...
        if not _is_likely_sha256(value_text):
            add_finding(
                ...
                "error",
                "open",
                "window_ticket_artifact_hashes values must look like sha256 hex.",
                {"artifact_id": key_text, "value": value_text},
            )

    artifact_count = window_ticket_data.get("artifact_count")
    if not isinstance(artifact_count, int) or artifact_count < 1:
        add_finding(
            ...
            "error",
            "open",
            "execution_window_ticket.artifact_count must be integer >= 1.",
        ...
        add_finding(
            ...
            "error",
            "open",
            "artifact_count must equal the number of window_ticket_artifact_hashes entries.",
            {"declared": artifact_count, "computed": len(normalized_hashes)},
        )

    missing_ids = window_ticket_data.get("missing_window_ticket_artifact_ids", [])
    mismatch_ids = window_ticket_data.get("hash_mismatch_window_ticket_artifact_ids", [])
    if not _is_string_array(missing_ids) and missing_ids != []:
        add_finding(findings, "missing_window_ticket_artifact_ids_invalid", "error", "open", "missing_window_ticket_artifact_ids must be a string array.")
        missing_ids = []
    if not _is_string_array(mismatch_ids) and mismatch_ids != []:
        add_finding(findings, "hash_mismatch_window_ticket_artifact_ids_invalid", "error", "open", "hash_mismatch_window_ticket_artifact_ids must be a string array.")
        mismatch_ids = []

    missing_ids = _normalize_string_list(missing_ids)
    mismatch_ids = _normalize_string_list(mismatch_ids)

    computed_missing_ids = sorted(set(required_window_ticket_artifact_ids) - set(normalized_hashes.keys()))
    if computed_missing_ids != missing_ids:
        add_finding(
            findings,
            "missing_window_ticket_artifact_ids_mismatch",
            "warning",
            "open",
            "missing_window_ticket_artifact_ids does not match required-vs-present hash-index computation.",
    ...
    if computed_missing_ids:
        add_finding(
            findings,
            "required_window_ticket_artifacts_missing",
            "error",
            "open",
            "Required window ticket source artifacts are missing from window_ticket_artifact_hashes.",
            {"missing_window_ticket_artifact_ids": computed_missing_ids},
        )

    unexpected_mismatch_ids = [item for item in mismatch_ids if item not in required_window_ticket_artifact_ids]
    if unexpected_mismatch_ids:
        add_finding(
            ...
            "error",
            "open",
            "hash_mismatch_window_ticket_artifact_ids includes values not present in required_window_ticket_artifact_ids.",
            {"unexpected_ids": unexpected_mismatch_ids},
        )

    if verification_status == "pass":
        if computed_missing_ids:
            add_finding(findings, "pass_with_missing_window_ticket_artifacts", "error", "open", "window_ticket_verification_status=pass is invalid when required window ticket source artifacts are missing.")
        if mismatch_ids:
            add_finding(findings, "pass_with_hash_mismatch", "error", "open", "window_ticket_verification_status=pass is invalid when hash_mismatch_window_ticket_artifact_ids is not empty.")
    ...
    if verification_status in {"fail", "pending_manual"} and not (computed_missing_ids or mismatch_ids):
        add_finding(
            ...
            "warning",
            "open",
            "window_ticket_verification_status is non-pass but no missing/mismatch evidence is recorded.",
        )

    if not _is_string_array(window_ticket_artifacts):
        add_finding(
            findings,
            "window_ticket_artifacts_invalid",
            "error",
            "open",
            "execution_window_ticket.window_ticket_artifacts must be a non-empty string array.",
        )
        window_ticket_artifacts = []
    window_ticket_artifacts = _normalize_string_list(window_ticket_artifacts)

    if isinstance(artifact_count, int) and artifact_count > 0 and len(window_ticket_artifacts) != artifact_count:
        add_finding(
            findings,
            "window_ticket_artifacts_count_mismatch",
            "warning",
            "open",
            "window_ticket_artifacts count differs from artifact_count.",
            {"artifact_count": artifact_count, "artifact_path_count": len(window_ticket_artifacts)},
        )

    for artifact_path in window_ticket_artifacts:
        if _contains_unsafe_path_tokens(artifact_path):
            add_finding(
                findings,
                "window_ticket_artifact_path_unsafe",
                "error",
                "open",
                "window_ticket_artifacts contains unsafe path tokens.",
                {"path": artifact_path},
            )
        ...
        if any(token in lowered for token in DISALLOWED_ARTIFACT_TOKENS):
            add_finding(
                findings,
                "window_ticket_artifact_disallowed",
                "error",
                "open",
                "window_ticket_artifacts includes disallowed binary/runtime/cache/database artifact.",
                {"path": artifact_path},
            )

    for hint_id, hint_token in REQUIRED_ARTIFACT_HINTS.items():
        if hint_id in required_window_ticket_artifact_ids and not any(hint_token in item.lower() for item in window_ticket_artifacts):
            add_finding(
                findings,
                f"required_artifact_missing_{hint_id}",
                "error",
                "open",
                "window_ticket_artifacts is missing a required authorization-record evidence artifact.",
                {"required_hint": hint_token},
            )

    if window_ticket_data.get("window_ticket_immutable") is not True:
        add_finding(
            findings,
            "window_ticket_immutable_required",
            "error",
            "open",
            "execution_window_ticket.window_ticket_immutable must be true.",
            {"actual": window_ticket_data.get("window_ticket_immutable")},
        )
    ...
    if decision not in {EXPECTED_READY_DECISION, "pending_manual_review", "rejected"}:
        add_finding(
            ...
            "error",
            "open",
            "approval_state.decision must be execution_window_ticket_recorded_for_future_manual_execution_window|pending_manual_review|rejected.",
            {"actual": decision},
        )
    ...
    readiness_state = str(readiness.get("execution_window_ticket_readiness_state", "")).strip()
    ...
    if readiness_state not in {
        "execution_window_ticket_recorded_for_future_manual_execution_window",
        "blocked_missing_evidence",
        "blocked_safety_boundary",
    }:
        add_finding(
            findings,
            "execution_window_ticket_readiness_state_invalid",
            "error",
            "open",
            "readiness.execution_window_ticket_readiness_state is invalid.",
            {"actual": readiness_state},
        )

    if readiness_state == "execution_window_ticket_recorded_for_future_manual_execution_window" and computed_missing_gate_ids:
        add_finding(
            findings,
            "ready_state_with_missing_gates",
            "error",
            "open",
            "execution_window_ticket_recorded_for_future_manual_execution_window cannot be set when required gates are missing.",
        )
    ...
    if decision == EXPECTED_READY_DECISION:
        ...
        if blocked_reason_codes:
            add_finding(
                ...
                "error",
                "open",
                "execution_window_ticket_recorded_for_future_manual_execution_window must not include blocked_reason_codes.",
        ...
        if rollback_plan_verified is not True:
            add_finding(
                ...
                "error",
                "open",
                "execution_window_ticket_recorded_for_future_manual_execution_window requires rollback_plan_verified=true.",
            )
        if cleanup_plan_verified is not True:
            add_finding(
                ...
                "error",
                "open",
                "execution_window_ticket_recorded_for_future_manual_execution_window requires cleanup_plan_verified=true.",
            )
        if readiness_state != "execution_window_ticket_recorded_for_future_manual_execution_window":
            add_finding(
                findings,
                "ready_without_ready_state",
                "error",
                "open",
                "execution_window_ticket_recorded_for_future_manual_execution_window requires readiness.execution_window_ticket_readiness_state=execution_window_ticket_recorded_for_future_manual_execution_window.",
            )
    elif decision == "pending_manual_review":
        add_finding(
            findings,
            "execution_window_ticket_pending_manual_review",
            "manual_review",
            "open",
            "Release publication execution window ticket remains pending manual review.",
        )
    elif decision == "rejected":
        add_finding(
            findings,
            "execution_window_ticket_rejected",
            "error",
            "open",
            "Release publication execution window ticket decision is rejected.",
        )

    if verification_status == "pass" and declared_status != "pass":
        add_finding(
            findings,
            "status_should_be_pass_for_window_ticket_pass",
            "error",
            "open",
            "window_ticket_verification_status=pass requires report.status=pass.",
        )
    elif verification_status == "warn" and declared_status != "warn":
        add_finding(
            findings,
            "status_should_be_warn_for_window_ticket_warn",
            "error",
            "open",
            "window_ticket_verification_status=warn requires report.status=warn.",
        )
    elif verification_status == "fail" and declared_status != "fail":
        add_finding(
            findings,
            "status_should_be_fail_for_window_ticket_fail",
            "error",
            "open",
            "window_ticket_verification_status=fail requires report.status=fail.",
        )
    elif verification_status == "pending_manual" and declared_status != "pending_manual":
        add_finding(
            findings,
            "status_should_be_pending_for_window_ticket_pending",
            "error",
            "open",
            "window_ticket_verification_status=pending_manual requires report.status=pending_manual.",
        )
