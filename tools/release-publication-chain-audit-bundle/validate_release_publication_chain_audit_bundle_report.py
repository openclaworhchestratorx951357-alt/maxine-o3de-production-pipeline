#!/usr/bin/env python3
"""Validate evidence-only release publication chain audit bundle reports."""
...
EXPECTED_REPORT_TYPE = "RELEASE_PUBLICATION_CHAIN_AUDIT_BUNDLE_v1_REPORT"
EXPECTED_CONTRACT_ID = "RELEASE_PUBLICATION_CHAIN_AUDIT_BUNDLE_v1"
EXPECTED_BUNDLE_ACTION = "record_release_publication_chain_audit_bundle"
EXPECTED_READY_DECISION = "chain_audit_bundle_recorded_for_audit"
TARGET_PATH = "qc.gates[]"
FUTURE_TARGET_PATH = "qc.checks[]"
CHECK_ID = "release_publication_chain_audit_bundle_v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate MAXINE release publication chain audit bundle report JSON."
    )
    parser.add_argument("report_path", help="Path to release publication chain audit bundle report JSON")
    parser.add_argument(
        "--allow-warn",
        action="store_true",
...
def main() -> int:
    ...
    schema_path = repo_root / "schemas" / "maxine_release_publication_chain_audit_bundle_report.schema.json"
    ...
    required_gate_ids = bundle_contract.get("required_gate_ids", [])
    required_report_artifact_ids = bundle_contract.get("required_report_artifact_ids", [])
    required_approvals = bundle_contract.get("required_approvals")
    manual_review_required = bundle_contract.get("manual_review_required")
    ...
    if not _is_string_array(required_gate_ids):
        add_finding(
            ...
            "error",
            "open",
            "bundle_contract.required_gate_ids must be a non-empty string array.",
        )
        required_gate_ids = []
    if not _is_string_array(required_report_artifact_ids):
        add_finding(
            ...
            "error",
            "open",
            "bundle_contract.required_report_artifact_ids must be a non-empty string array.",
        )
    ...
    if not isinstance(required_approvals, int) or required_approvals < 1:
        add_finding(
            ...
            "error",
            "open",
            "bundle_contract.required_approvals must be integer >= 1.",
    ...
    if not isinstance(manual_review_required, bool):
        add_finding(
            ...
            "error",
            "open",
            "bundle_contract.manual_review_required must be boolean.",
    ...
    if bundle_contract.get("immutable_bundle_required") is not True:
        add_finding(
            findings,
            "immutable_bundle_required",
            "error",
            "open",
            "bundle_contract.immutable_bundle_required must be true.",
            {"actual": bundle_contract.get("immutable_bundle_required")},
        )
    if bundle_contract.get("hash_verification_required") is not True:
        add_finding(
            findings,
            "hash_verification_required",
            "error",
            "open",
            "bundle_contract.hash_verification_required must be true.",
            {"actual": bundle_contract.get("hash_verification_required")},
        )
    ...
    if bundle_contract.get("execution_admitted") is not False:
        add_finding(
            findings,
            "execution_admitted_must_be_false",
            "error",
            "open",
            "bundle_contract.execution_admitted must be false.",
            {"actual": bundle_contract.get("execution_admitted")},
        )

    chain_bundle = report.get("chain_audit_bundle", {}) if isinstance(report.get("chain_audit_bundle"), dict) else {}
    bundle_action = str(chain_bundle.get("bundle_action", "")).strip()
    hash_algorithm = str(chain_bundle.get("hash_algorithm", "")).strip()
    verification_status = str(chain_bundle.get("bundle_verification_status", "")).strip()
    chain_manifest_path = str(chain_bundle.get("chain_manifest_path", "")).strip()
    audit_bundle_path = str(chain_bundle.get("audit_bundle_path", "")).strip()
    audit_bundle_sha = str(chain_bundle.get("audit_bundle_sha256", "")).strip()
    audit_bundle_artifacts = chain_bundle.get("audit_bundle_artifacts", [])

    if bundle_action != EXPECTED_BUNDLE_ACTION:
        add_finding(
            findings,
            "bundle_action_invalid",
            "error",
            "open",
            f"chain_audit_bundle.bundle_action must be {EXPECTED_BUNDLE_ACTION}.",
            {"actual": bundle_action},
        )
    ...
    if hash_algorithm != "sha256":
        add_finding(
            ...
            "error",
            "open",
            "chain_audit_bundle.hash_algorithm must be sha256.",
    ...
    if verification_status not in ALLOWED_STATUS:
        add_finding(
            findings,
            "bundle_verification_status_invalid",
            "error",
            "open",
            "chain_audit_bundle.bundle_verification_status must be pass|warn|fail|pending_manual.",
            {"actual": verification_status},
        )

    for label, path_text in (("chain_manifest_path", chain_manifest_path), ("audit_bundle_path", audit_bundle_path)):
        if not path_text:
            add_finding(findings, f"{label}_missing", "error", "open", f"chain_audit_bundle.{label} is required.")
            continue
        if _contains_unsafe_path_tokens(path_text):
            add_finding(
                ...
                "error",
                "open",
                f"chain_audit_bundle.{label} contains unsafe traversal or absolute-path tokens.",
                {"path": path_text},
            )

    if not _is_likely_sha256(audit_bundle_sha):
        add_finding(
            findings,
            "audit_bundle_sha256_invalid",
            "error",
            "open",
            "chain_audit_bundle.audit_bundle_sha256 must look like sha256 hex.",
            {"actual": audit_bundle_sha},
        )

    if chain_bundle.get("no_command_execution_recorded") is not True:
        add_finding(
            findings,
            "no_command_execution_recorded_required",
            "error",
            "open",
            "chain_audit_bundle.no_command_execution_recorded must be true.",
            {"actual": chain_bundle.get("no_command_execution_recorded")},
        )

    artifact_hashes = chain_bundle.get("audit_bundle_artifact_hashes", {})
    if not isinstance(artifact_hashes, dict) or not artifact_hashes:
        add_finding(
            findings,
            "audit_bundle_artifact_hashes_invalid",
            "error",
            "open",
            "chain_audit_bundle.audit_bundle_artifact_hashes must be a non-empty object.",
        )
        artifact_hashes = {}
    ...
    for key, value in artifact_hashes.items():
        ...
        if not key_text:
            add_finding(findings, "artifact_hash_key_missing", "error", "open", "audit_bundle_artifact_hashes keys must be non-empty.")
            continue
        ...
        if not _is_likely_sha256(value_text):
            add_finding(
                ...
                "error",
                "open",
                "audit_bundle_artifact_hashes values must look like sha256 hex.",
                {"artifact_id": key_text, "value": value_text},
            )

    artifact_count = chain_bundle.get("artifact_count")
    if not isinstance(artifact_count, int) or artifact_count < 1:
        add_finding(
            ...
            "error",
            "open",
            "chain_audit_bundle.artifact_count must be integer >= 1.",
        ...
        add_finding(
            ...
            "error",
            "open",
            "artifact_count must equal the number of audit_bundle_artifact_hashes entries.",
            {"declared": artifact_count, "computed": len(normalized_hashes)},
        )

    missing_ids = chain_bundle.get("missing_report_artifact_ids", [])
    mismatch_ids = chain_bundle.get("hash_mismatch_report_artifact_ids", [])
    ...
    if computed_missing_ids:
        add_finding(
            ...
            "error",
            "open",
            "Required report artifacts are missing from audit_bundle_artifact_hashes.",
    ...
    if verification_status == "pass":
        if computed_missing_ids:
            add_finding(findings, "pass_with_missing_report_artifacts", "error", "open", "bundle_verification_status=pass is invalid when required report artifacts are missing.")
        if mismatch_ids:
            add_finding(findings, "pass_with_hash_mismatch", "error", "open", "bundle_verification_status=pass is invalid when hash_mismatch_report_artifact_ids is not empty.")
    ...
    if verification_status in {"fail", "pending_manual"} and not (computed_missing_ids or mismatch_ids):
        add_finding(
            ...
            "warning",
            "open",
            "bundle_verification_status is non-pass but no missing/mismatch evidence is recorded.",
        )

    if not _is_string_array(audit_bundle_artifacts):
        add_finding(
            findings,
            "audit_bundle_artifacts_invalid",
            "error",
            "open",
            "chain_audit_bundle.audit_bundle_artifacts must be a non-empty string array.",
        )
        audit_bundle_artifacts = []
    audit_bundle_artifacts = _normalize_string_list(audit_bundle_artifacts)

    if isinstance(artifact_count, int) and artifact_count > 0 and len(audit_bundle_artifacts) != artifact_count:
        add_finding(
            findings,
            "audit_bundle_artifacts_count_mismatch",
            "warning",
            "open",
            "audit_bundle_artifacts count differs from artifact_count.",
            {"artifact_count": artifact_count, "artifact_path_count": len(audit_bundle_artifacts)},
        )

    for artifact_path in audit_bundle_artifacts:
        if _contains_unsafe_path_tokens(artifact_path):
            add_finding(
                findings,
                "audit_bundle_artifact_path_unsafe",
                "error",
                "open",
                "audit_bundle_artifacts contains unsafe path tokens.",
                {"path": artifact_path},
            )
        ...
        if any(token in lowered for token in DISALLOWED_ARTIFACT_TOKENS):
            add_finding(
                findings,
                "audit_bundle_artifact_disallowed",
                "error",
                "open",
                "audit_bundle_artifacts includes disallowed binary/runtime/cache/database artifact.",
                {"path": artifact_path},
            )

    for hint_id, hint_token in REQUIRED_ARTIFACT_HINTS.items():
        if hint_id in required_report_artifact_ids and not any(hint_token in item.lower() for item in audit_bundle_artifacts):
            add_finding(
                findings,
                f"required_artifact_missing_{hint_id}",
                "error",
                "open",
                "audit_bundle_artifacts is missing required chain-audit evidence artifact.",
                {"required_hint": hint_token},
            )

    if chain_bundle.get("audit_bundle_immutable") is not True:
        add_finding(
            findings,
            "audit_bundle_immutable_required",
            "error",
            "open",
            "chain_audit_bundle.audit_bundle_immutable must be true.",
            {"actual": chain_bundle.get("audit_bundle_immutable")},
        )
    ...
    if decision not in {EXPECTED_READY_DECISION, "pending_manual_review", "rejected"}:
        add_finding(
            ...
            "error",
            "open",
            "approval_state.decision must be chain_audit_bundle_recorded_for_audit|pending_manual_review|rejected.",
            {"actual": decision},
        )
    ...
    readiness_state = str(readiness.get("chain_audit_bundle_readiness_state", "")).strip()
    ...
    if required_gate_ids and readiness_required_gate_ids and required_gate_ids != readiness_required_gate_ids:
        add_finding(
            ...
            "warning",
            "open",
            "bundle_contract.required_gate_ids does not match readiness.required_gate_ids.",
            {
    ...
    if computed_missing_gate_ids:
        add_finding(
            ...
            "error",
            "open",
            "Required chain-audit gates are missing.",
            {"missing_gate_ids": computed_missing_gate_ids},
        )

    if readiness_state not in {
        "chain_audit_bundle_recorded_for_audit",
        "blocked_missing_evidence",
        "blocked_safety_boundary",
    }:
        add_finding(
            findings,
            "chain_audit_bundle_readiness_state_invalid",
            "error",
            "open",
            "readiness.chain_audit_bundle_readiness_state is invalid.",
            {"actual": readiness_state},
        )

    if readiness_state == "chain_audit_bundle_recorded_for_audit" and computed_missing_gate_ids:
        add_finding(
            findings,
            "ready_state_with_missing_gates",
            "error",
            "open",
            "chain_audit_bundle_recorded_for_audit cannot be set when required gates are missing.",
        )
    ...
    if decision == EXPECTED_READY_DECISION:
        if len(approver_ids) < required_approvals:
            add_finding(
                ...
                "error",
                "open",
                "approval_state.approver_ids count is lower than bundle_contract.required_approvals.",
        ...
        if blocked_reason_codes:
            add_finding(
                ...
                "error",
                "open",
                "chain_audit_bundle_recorded_for_audit must not include blocked_reason_codes.",
        ...
        if rollback_plan_verified is not True:
            add_finding(
                ...
                "error",
                "open",
                "chain_audit_bundle_recorded_for_audit requires rollback_plan_verified=true.",
            )
        if cleanup_plan_verified is not True:
            add_finding(
                ...
                "error",
                "open",
                "chain_audit_bundle_recorded_for_audit requires cleanup_plan_verified=true.",
            )
        if readiness_state != "chain_audit_bundle_recorded_for_audit":
            add_finding(
                findings,
                "ready_without_ready_state",
                "error",
                "open",
                "chain_audit_bundle_recorded_for_audit requires readiness.chain_audit_bundle_readiness_state=chain_audit_bundle_recorded_for_audit.",
            )
    elif decision == "pending_manual_review":
        add_finding(
            findings,
            "chain_audit_bundle_pending_manual_review",
            "manual_review",
            "open",
            "Release publication chain audit bundle remains pending manual review.",
        )
    elif decision == "rejected":
        add_finding(
            findings,
            "chain_audit_bundle_rejected",
            "error",
            "open",
            "Release publication chain audit bundle decision is rejected.",
        )

    if verification_status == "pass" and declared_status != "pass":
        add_finding(
            findings,
            "status_should_be_pass_for_bundle_pass",
            "error",
            "open",
            "bundle_verification_status=pass requires report.status=pass.",
        )
    elif verification_status == "warn" and declared_status != "warn":
        add_finding(
            findings,
            "status_should_be_warn_for_bundle_warn",
            "error",
            "open",
            "bundle_verification_status=warn requires report.status=warn.",
        )
    elif verification_status == "fail" and declared_status != "fail":
        add_finding(
            findings,
            "status_should_be_fail_for_bundle_fail",
            "error",
            "open",
            "bundle_verification_status=fail requires report.status=fail.",
        )
    elif verification_status == "pending_manual" and declared_status != "pending_manual":
        add_finding(
            findings,
            "status_should_be_pending_for_bundle_pending",
            "error",
            "open",
            "bundle_verification_status=pending_manual requires report.status=pending_manual.",
        )
