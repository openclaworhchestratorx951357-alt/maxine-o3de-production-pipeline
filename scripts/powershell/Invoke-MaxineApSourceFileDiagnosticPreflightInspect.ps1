...
function Get-Summary {
    ...
    return [ordered]@{
        source_file_diagnostic_preflight_id = [string]$Item.source_file_diagnostic_preflight_id
        source_review_packet_id = [string]$Item.source_review_packet_id
        source_proposal_id = [string]$Item.source_proposal_id
        source_ap_binary_preflight_id = [string]$Item.source_ap_binary_preflight_id
        source_real_binary_diagnostic_execution_id = [string]$Item.source_real_binary_diagnostic_execution_id
        source_project_inventory_id = [string]$Item.source_project_inventory_id
        candidate_relative_path = [string]$Item.candidate_relative_path
        ...
        required_manual_confirmation = [bool]$Item.required_manual_confirmation
...
if (-not (Test-Path -LiteralPath $preflightsRootAbs -PathType Container)) {
    throw "source file diagnostic preflights root not found: $SourceFileDiagnosticPreflightsRoot"
}
...
if ($List) {
    [ordered]@{
        ...
        source_file_diagnostic_preflights = @($refs | ForEach-Object { Get-Summary -Item $_.item })
...
if (-not [string]::IsNullOrWhiteSpace($PreflightPath)) {
    ...
    $match = $refs | Where-Object { [string]$_.item.source_file_diagnostic_preflight_id -eq $PreflightId } | Select-Object -First 1
    if (-not $match) {
