...
def _validator_script() -> Path:
    return _repo_root() / "tools" / "release-publication-execution-receipt" / "validate_release_publication_execution_receipt_report.py"


def _report_path(name: str) -> Path:
    return _repo_root() / "examples" / "release-publication-execution-receipt" / name
...
def test_pass_report_returns_pass_and_exit_zero():
    result = _run_validator(_report_path("max_biped_v1_release_publication_execution_receipt_pass.json"))
...
def test_warn_report_returns_warn_and_exit_zero_with_allow_warn():
    result = _run_validator(_report_path("max_biped_v1_release_publication_execution_receipt_warn.json"), allow_warn=True)
...
def test_warn_report_returns_nonzero_without_allow_warn():
    result = _run_validator(_report_path("max_biped_v1_release_publication_execution_receipt_warn.json"), allow_warn=False)
...
def test_fail_report_returns_fail_and_nonzero():
    result = _run_validator(_report_path("max_biped_v1_release_publication_execution_receipt_fail.json"))
...
def test_missing_required_gates_fails(tmp_path: Path):
    report = json.loads(_report_path("max_biped_v1_release_publication_execution_receipt_pass.json").read_text(encoding="utf-8-sig"))
    report["status"] = "fail"
...
def test_insufficient_approvals_fails(tmp_path: Path):
    report = json.loads(_report_path("max_biped_v1_release_publication_execution_receipt_pass.json").read_text(encoding="utf-8-sig"))
    report["status"] = "fail"
    report["approval_state"]["approver_ids"] = ["ops.release_manager"]
...
def test_pending_manual_publication_result_requires_pending_decision(tmp_path: Path):
    report = json.loads(_report_path("max_biped_v1_release_publication_execution_receipt_pass.json").read_text(encoding="utf-8-sig"))
    report["status"] = "pending_manual"
    report["execution_receipt"]["publication_result"] = "pending_manual_execution"
    report["approval_state"]["decision"] = "pending_manual_review"
    report["approval_state"]["approver_ids"] = []
    report["approval_state"]["blocked_reason_codes"] = ["waiting_manual_execution"]
    ...
    assert any(item.get("id") == "execution_receipt_pending_manual_review" for item in payload["findings"])


def test_unsafe_publish_command_token_fails(tmp_path: Path):
    report = json.loads(_report_path("max_biped_v1_release_publication_execution_receipt_pass.json").read_text(encoding="utf-8-sig"))
    report["status"] = "fail"
    report["execution_receipt"]["publish_command_display"] = "DisplayOnly: publish_release_bundle --manifest a.json | invoke-expression"
    report["readiness"]["release_readiness_state"] = "blocked_safety_boundary"
...
def test_output_contains_manifest_attachable_qc_payload():
    ...
    assert qc_check.get("check_id") == "release_publication_execution_receipt_v1"
