...
def _validator_script() -> Path:
    return _repo_root() / "tools" / "release-publication-rollback-drill" / "validate_release_publication_rollback_drill_report.py"


def _report_path(name: str) -> Path:
    return _repo_root() / "examples" / "release-publication-rollback-drill" / name
...
def _load_pass() -> dict:
    return json.loads(_report_path("max_biped_v1_release_publication_rollback_drill_pass.json").read_text(encoding="utf-8-sig"))


def test_pass_report_returns_pass_and_exit_zero():
    result = _run_validator(_report_path("max_biped_v1_release_publication_rollback_drill_pass.json"))
...
def test_warn_report_returns_warn_and_exit_zero_with_allow_warn():
    result = _run_validator(_report_path("max_biped_v1_release_publication_rollback_drill_warn.json"), allow_warn=True)
...
def test_warn_report_returns_nonzero_without_allow_warn():
    result = _run_validator(_report_path("max_biped_v1_release_publication_rollback_drill_warn.json"), allow_warn=False)
...
def test_fail_report_returns_fail_and_nonzero():
    result = _run_validator(_report_path("max_biped_v1_release_publication_rollback_drill_fail.json"))
...
def test_missing_required_gates_fails(tmp_path: Path):
    ...
    report["readiness"]["rollback_readiness_state"] = "blocked_missing_evidence"
...
def test_missing_required_artifact_hint_fails(tmp_path: Path):
    report = _load_pass()
    report["status"] = "fail"
    report["rollback_drill"]["evidence_bundle_artifacts"] = [
        "evidence/jobs/job-maxine-release-001/manifest.json",
        "evidence/jobs/job-maxine-release-001/qc/release-publication-request-ledger.json",
        "evidence/jobs/job-maxine-release-001/qc/release-publication-receipt.json",
        "evidence/jobs/job-maxine-release-001/logs/rollback-drill-manual.log",
    ]
    report["readiness"]["rollback_readiness_state"] = "blocked_safety_boundary"
    test_file = tmp_path / "missing-artifact-hint.json"
    test_file.write_text(json.dumps(report, indent=2), encoding="utf-8")
    ...
    assert result.returncode != 0
    assert payload["status"] == "fail"
    assert any(item.get("id") == "required_artifact_missing_post_rollback_validation" for item in payload["findings"])


def test_pending_manual_rollback_requires_pending_decision(tmp_path: Path):
    report = _load_pass()
    report["status"] = "pending_manual"
    report["rollback_drill"]["rollback_result"] = "pending_manual_rollback"
    report["approval_state"]["decision"] = "pending_manual_review"
    report["approval_state"]["approver_ids"] = []
    report["approval_state"]["blocked_reason_codes"] = ["waiting_manual_rollback"]
    report["readiness"]["rollback_readiness_state"] = "blocked_safety_boundary"
    test_file = tmp_path / "pending-manual.json"
    test_file.write_text(json.dumps(report, indent=2), encoding="utf-8")
    ...
    assert any(item.get("id") == "rollback_drill_pending_manual_review" for item in payload["findings"])


def test_unsafe_publish_command_token_fails(tmp_path: Path):
    report = _load_pass()
    report["status"] = "fail"
    report["rollback_drill"]["publish_command_display"] = "DisplayOnly: publish_release_bundle --manifest a.json | invoke-expression"
    report["readiness"]["rollback_readiness_state"] = "blocked_safety_boundary"
...
def test_output_contains_manifest_attachable_qc_payload():
    ...
    assert qc_check.get("check_id") == "release_publication_rollback_drill_v1"
