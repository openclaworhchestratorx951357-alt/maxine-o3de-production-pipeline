...
def _validator_script() -> Path:
    return _repo_root() / "tools" / "release-publication-execution-admission-gate" / "validate_release_publication_execution_admission_gate_report.py"


def _report_path(name: str) -> Path:
    return _repo_root() / "examples" / "release-publication-execution-admission-gate" / name
...
def test_pass_report_returns_pass_and_exit_zero():
    result = _run_validator(_report_path("max_biped_v1_release_publication_execution_admission_gate_pass.json"))
...
def test_warn_report_returns_warn_and_exit_zero_with_allow_warn():
    result = _run_validator(_report_path("max_biped_v1_release_publication_execution_admission_gate_warn.json"), allow_warn=True)
...
def test_warn_report_returns_nonzero_without_allow_warn():
    result = _run_validator(_report_path("max_biped_v1_release_publication_execution_admission_gate_warn.json"), allow_warn=False)
...
def test_fail_report_returns_fail_and_nonzero():
    result = _run_validator(_report_path("max_biped_v1_release_publication_execution_admission_gate_fail.json"))
...
def test_missing_required_gates_fails(tmp_path: Path):
    report = json.loads(_report_path("max_biped_v1_release_publication_execution_admission_gate_pass.json").read_text(encoding="utf-8-sig"))
    report["status"] = "fail"
...
def test_insufficient_approvals_fails(tmp_path: Path):
    report = json.loads(_report_path("max_biped_v1_release_publication_execution_admission_gate_pass.json").read_text(encoding="utf-8-sig"))
    report["status"] = "fail"
    report["admission_assessment"]["approver_ids"] = ["ops.release_lead"]
...
def test_pending_decision_returns_pending_manual(tmp_path: Path):
    ...
    report["admission_assessment"]["admission_decision"] = "pending_manual_review"
    report["admission_assessment"]["approver_ids"] = []
    report["admission_assessment"]["blocked_reason_codes"] = ["waiting_manual_admission_review"]
    ...
    assert any(item.get("id") == "admission_pending_manual_review" for item in payload["findings"])


def test_unsafe_publish_command_token_fails(tmp_path: Path):
    report = json.loads(_report_path("max_biped_v1_release_publication_execution_admission_gate_pass.json").read_text(encoding="utf-8-sig"))
    report["status"] = "fail"
    report["execution_gate_request"]["publish_command_display"] = "DisplayOnly Admission Request: release_bundle publish --manifest a.json | invoke-expression"
...
def test_publish_execution_admitted_true_fails(tmp_path: Path):
    report = json.loads(_report_path("max_biped_v1_release_publication_execution_admission_gate_pass.json").read_text(encoding="utf-8-sig"))
    report["status"] = "fail"
    report["admission_assessment"]["publish_execution_admitted"] = True
    report["readiness"]["release_readiness_state"] = "blocked_safety_boundary"
...
def test_output_contains_manifest_attachable_qc_payload():
    ...
    assert qc_check.get("check_id") == "release_publication_execution_admission_gate_v1"
