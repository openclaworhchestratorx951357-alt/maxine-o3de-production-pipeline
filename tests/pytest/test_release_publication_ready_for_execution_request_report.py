...
def _validator_script() -> Path:
    return (
        _repo_root()
        / "tools"
        / "release-publication-ready-for-execution-request"
        / "validate_release_publication_ready_for_execution_request_report.py"
    )


def _report_path(name: str) -> Path:
    return _repo_root() / "examples" / "release-publication-ready-for-execution-request" / name
...
def _load_pass() -> dict:
    return json.loads(
        _report_path("max_biped_v1_release_publication_ready_for_execution_request_pass.json").read_text(
            encoding="utf-8-sig"
        )
    )
...
def test_pass_report_returns_pass_and_exit_zero():
    result = _run_validator(
        _report_path("max_biped_v1_release_publication_ready_for_execution_request_pass.json")
    )
...
def test_warn_report_returns_warn_and_exit_zero_with_allow_warn():
    result = _run_validator(
        _report_path("max_biped_v1_release_publication_ready_for_execution_request_warn.json"),
        allow_warn=True,
    )
...
def test_fail_report_returns_fail_and_nonzero():
    result = _run_validator(
        _report_path("max_biped_v1_release_publication_ready_for_execution_request_fail.json")
    )
...
def test_missing_required_report_artifact_hash_fails(tmp_path: Path):
    report = _load_pass()
    report["status"] = "fail"
    report["ready_for_execution_request"]["request_verification_status"] = "fail"
    report["ready_for_execution_request"]["request_packet_artifact_hashes"].pop(
        "release_publication_execution_receipt", None
    )
    report["ready_for_execution_request"]["artifact_count"] = len(
        report["ready_for_execution_request"]["request_packet_artifact_hashes"]
    )
    report["ready_for_execution_request"]["missing_report_artifact_ids"] = [
    ...
    report["readiness"]["ready_for_execution_request_readiness_state"] = "blocked_missing_evidence"
...
def test_unsafe_request_packet_path_fails(tmp_path: Path):
    report = _load_pass()
    report["status"] = "fail"
    report["ready_for_execution_request"]["request_verification_status"] = "fail"
    report["ready_for_execution_request"]["request_packet_path"] = "..\\outside\\audit-bundle.json"
    report["approval_state"]["decision"] = "rejected"
    report["approval_state"]["blocked_reason_codes"] = ["unsafe_bundle_path"]
    report["readiness"]["ready_for_execution_request_readiness_state"] = "blocked_safety_boundary"
    ...
    assert result.returncode != 0
    assert payload["status"] == "fail"
    assert any(item.get("id") == "request_packet_path_unsafe" for item in payload["findings"])
...
def test_output_contains_manifest_attachable_qc_payload():
    ...
    assert qc_check.get("check_id") == "release_publication_ready_for_execution_request_v1"
