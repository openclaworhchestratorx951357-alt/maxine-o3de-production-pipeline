...
def _validator_script() -> Path:
    return (
        _repo_root()
        / "tools"
        / "release-publication-evidence-integrity-index"
        / "validate_release_publication_evidence_integrity_index_report.py"
    )


def _report_path(name: str) -> Path:
    return _repo_root() / "examples" / "release-publication-evidence-integrity-index" / name
...
def _load_pass() -> dict:
    return json.loads(
        _report_path("max_biped_v1_release_publication_evidence_integrity_index_pass.json").read_text(
            encoding="utf-8-sig"
        )
    )
...
def test_pass_report_returns_pass_and_exit_zero():
    result = _run_validator(
        _report_path("max_biped_v1_release_publication_evidence_integrity_index_pass.json")
    )
...
def test_warn_report_returns_warn_and_exit_zero_with_allow_warn():
    result = _run_validator(
        _report_path("max_biped_v1_release_publication_evidence_integrity_index_warn.json"),
        allow_warn=True,
    )
...
def test_fail_report_returns_fail_and_nonzero():
    result = _run_validator(
        _report_path("max_biped_v1_release_publication_evidence_integrity_index_fail.json")
    )
...
def test_missing_required_artifact_hash_fails(tmp_path: Path):
    report = _load_pass()
    report["status"] = "fail"
    report["evidence_integrity_index"]["artifact_hash_index"].pop("rollback_drill", None)
    report["evidence_integrity_index"]["artifact_count"] = (
        len(report["evidence_integrity_index"]["artifact_hash_index"])
    )
    report["evidence_integrity_index"]["hash_verification_status"] = "fail"
    report["evidence_integrity_index"]["missing_artifact_ids"] = ["rollback_drill"]
    report["approval_state"]["decision"] = "rejected"
    report["approval_state"]["blocked_reason_codes"] = ["missing_artifact_hash"]
    ...
    report["readiness"]["integrity_readiness_state"] = "blocked_missing_evidence"
    test_file = tmp_path / "missing-artifact-hash.json"
...
def test_unsafe_bundle_path_fails(tmp_path: Path):
    report = _load_pass()
    report["status"] = "fail"
    report["evidence_integrity_index"]["evidence_bundle_path"] = "..\\outside\\bundle.json"
    report["evidence_integrity_index"]["hash_verification_status"] = "fail"
    report["approval_state"]["decision"] = "rejected"
    report["approval_state"]["blocked_reason_codes"] = ["unsafe_bundle_path"]
    report["readiness"]["integrity_readiness_state"] = "blocked_safety_boundary"
    test_file = tmp_path / "unsafe-bundle-path.json"
    test_file.write_text(json.dumps(report, indent=2), encoding="utf-8")
    ...
    assert result.returncode != 0
    assert payload["status"] == "fail"
    assert any(item.get("id") == "evidence_bundle_path_unsafe" for item in payload["findings"])
...
def test_output_contains_manifest_attachable_qc_payload():
    ...
    assert qc_check.get("check_id") == "release_publication_evidence_integrity_index_v1"
