...
def _enable_animation_playback_surface_fixture_env(env: dict) -> None:
...
def _enable_runtime_animation_playback_execution_fixture_env(env: dict) -> None:
...
def _enable_runtime_character_behavior_smoke_fixture_env(env: dict) -> None:
...
def _write_animation_source_validation_files(engine: Path) -> None:
    source_files = {
        ...
        "Gems/EMotionFX/Code/EMotionFX/Source/MotionSystem.h": (
            "MotionInstance* PlayMotion(Motion* motion, PlayBackInfo* info);\n"
            "void Update(float timePassedInSeconds);\n"
            "void UpdateMotionInstances(float timePassedInSeconds);\n"
        ),
        ...
        "Gems/EMotionFX/Code/Source/Integration/Assets/AssetCommon.h": (
            "EMotionFXAssetHandler<DataType>::Register\n"
        ...
        "Code/Framework/AzCore/AzCore/Asset/AssetManager.cpp": (
            "No handler was registered for asset of type %s but it was still in the AssetManager as %s\n"
            "Asset handler for %s is being removed, when assetid %s is still loaded!\n"
            "AssetManager::UnregisterHandler\n"
        ),
...
def _animation_playback_surface_fixture_runner(
    ...
    actor_asset_id: str = "",
    motion_asset_id: str = "",
    product_asset_overrides: dict[str, tuple[str, str]] | None = None,
    playback_markers: bool = False,
    playback_time_before: str = "0.000000",
    playback_time_after: str = "0.125000",
    playback_tick_count: int = 8,
    playback_request_succeeded: bool = True,
    playback_started: bool = True,
    playback_observed: bool = True,
    playback_time_advanced: bool = True,
    playback_active_state_observed: bool = True,
    playback_blocker: str = "",
    extra_stdout_lines: list[str] | None = None,
):
    def runner(argv, **kwargs):
        marker_lines = ["MAXINE_RUNTIME_PRODUCT_LOAD_START count=8 timeout_ticks=120 require_all=true"]
        for index, product in enumerate(products):
            asset_id, asset_type = (product_asset_overrides or {}).get(
                product["product_kind"],
                (
                    f"{{11111111-1111-4111-8111-111111111111}}:{index + 1}",
            ...
            marker_lines.extend(
                [
                    (
                        ...
                        f"asset_id={asset_id} "
                        f"asset_type={asset_type} asset_type_name=runtime_catalog_asset_type"
                    ),
        ...
        if playback_markers:
            marker_lines.extend(
                [
                    (
                        "MAXINE_RUNTIME_ANIMATION_PLAYBACK_SOURCE_VALIDATED "
                        "api=EMotionFX::Integration::SimpleMotionComponentRequestBus::PlayMotion "
                        "observe=EMotionFX::Integration::SimpleMotionComponentRequestBus::GetPlayTime "
                        "motion_instance=EMotionFX::Integration::SimpleMotionComponent::GetMotionInstance "
                        "status=pass"
                    ),
                    (
                        "MAXINE_RUNTIME_ANIMATION_PLAYBACK_REQUEST "
                        "entity_id={44444444-4444-4444-8444-444444444444} "
                        f"motion_asset_id={motion_asset_id} api=SimpleMotionComponentRequestBus::PlayMotion "
                        f"play_time_before={playback_time_before} duration=1.000000 "
                        f"request_succeeded={1 if playback_request_succeeded else 0} status=issued"
                    ),
                    (
                        "MAXINE_RUNTIME_ANIMATION_PLAYBACK_OBSERVE "
                        "entity_id={44444444-4444-4444-8444-444444444444} "
                        f"play_time_before={playback_time_before} play_time_after={playback_time_after} "
                        f"time_advanced={1 if playback_time_advanced else 0} "
                        f"active_state_observed={1 if playback_active_state_observed else 0} "
                        f"started={1 if playback_started else 0} observed={1 if playback_observed else 0} "
                        f"tick_count={playback_tick_count} status={'pass' if playback_observed else 'fail'} "
                        f"blocker={playback_blocker}"
                    ),
                    (
                        "MAXINE_RUNTIME_ANIMATION_PLAYBACK_SUMMARY "
                        f"status={'pass' if playback_observed else 'fail'} requested=1 "
                        f"request_succeeded={1 if playback_request_succeeded else 0} "
                        f"started={1 if playback_started else 0} observed={1 if playback_observed else 0} "
                        f"time_advanced={1 if playback_time_advanced else 0} "
                        f"active_state_observed={1 if playback_active_state_observed else 0} "
                        f"tick_count={playback_tick_count} cleanup=deferred_until_spawn_cleanup "
                        f"blocker={playback_blocker}"
                    ),
                ]
            )
        ...
        return subprocess.CompletedProcess(
            ...
            stdout=(
                ...
                + ("\n" + "\n".join(extra_stdout_lines) if extra_stdout_lines else "")
...
def test_runtime_harness_motion_handler_shutdown_signal_can_be_classified_for_approved_motion_wiring(
    tmp_path: Path,
) -> None:
    env, engine, project, apb = _runtime_env(tmp_path, gates=True)
    _write_animation_component_wiring_source_validation_files(engine)
    ...
    assert report["approved_motion_product_handler_signal_diagnostic_attempted"] is True
    assert report["approved_motion_product_handler_signal_diagnostic_completed"] is True
    assert report["approved_motion_product_handler_signal_source_validation_verified"] is True
    ...
    assert report["approved_motion_product_handler_signal_classification"] == (
        "harmless_shutdown_handler_unregister_after_verified_motion_load"
    )
    assert report["approved_motion_product_handler_signal_classification_verified"] is True
    assert report["approved_motion_product_handler_signal_harmless_under_strict_fixture"] is True
    assert report["approved_motion_product_handler_signal_blocker"] == ""
    assert report["runtime_motion_assignment_id_readback_verified"] is True
    assert report["runtime_motion_assignment_load_verified"] is True
    assert report["runtime_character_product_load_verified"] is True
    assert report["runtime_character_product_load_failed_products"] == []
    assert report["runtime_character_animation_component_wiring_claimed"] is True
    assert report["runtime_character_animation_component_wiring_verified"] is True
...
def test_runtime_harness_motion_handler_signal_for_nonapproved_motion_remains_blocking(
    ...
    other_motion_asset_id = "{AAAAAAAA-BBBB-5CCC-8DDD-EEEEEEEEEEEE}:ddcbe0"
    motion_asset_type = "{00494B8E-7578-4BA2-8B28-272E90680787}"
    ...
    report = runtime_harness.run_runtime_harness(
        ...
        command_runner=_animation_playback_surface_fixture_runner(
            ...
            actor_asset_id="{7E3BE43C-A0C7-512B-9F3E-FA6C2A4DBDAC}:914f19b7",
            motion_asset_id="{794D1588-3C41-5795-8A9A-EEBD6A663A60}:ddcbe0",
            product_asset_overrides={"motion": (other_motion_asset_id, motion_asset_type)},
            extra_stdout_lines=[
                (
                    f"System: No handler was registered for asset of type {motion_asset_type} "
                    f"but it was still in the AssetManager as {other_motion_asset_id}"
                ),
            ],
        ),
        artifact_root=tmp_path / "artifacts",
    ...
    assert report["approved_motion_product_handler_signal_diagnostic_attempted"] is True
    assert report["approved_motion_product_handler_signal_found"] is True
    assert report["approved_motion_product_handler_signal_asset_id"] == other_motion_asset_id
    assert report["approved_motion_product_handler_signal_classification_verified"] is False
    assert report["approved_motion_product_handler_signal_harmless_under_strict_fixture"] is False
    assert report["approved_motion_product_handler_signal_blocker"] == (
...
def test_runtime_harness_motion_handler_signal_diagnostic_records_source_validation(
    tmp_path: Path,
) -> None:
    env, engine, project, apb = _runtime_env(tmp_path, gates=True)
    _write_animation_component_wiring_source_validation_files(engine)
    ...
    report = runtime_harness.run_runtime_harness(
        manifest=runtime_harness.DEFAULT_MANIFEST,
        diagnose_approved_motion_product_handler_unregistered_signal=True,
        strict=True,
        engine_root=engine,
        project=project,
        apb_report=apb,
    ...
    assert report["status"] == "pass"
    assert report["runtime_harness_mode"] == "approved_motion_product_handler_unregistered_signal_diagnostic"
    assert report["runtime_harness_status"] == "approved_motion_product_handler_signal_source_discovery"
    assert report["runtime_execution_attempted"] is False
    assert report["approved_motion_product_handler_signal_diagnostic_attempted"] is True
    assert report["approved_motion_product_handler_signal_diagnostic_completed"] is True
    assert report["approved_motion_product_handler_signal_source_validation_verified"] is True
...
def test_runtime_harness_animation_playback_execution_diagnostic_records_source_validation(
    tmp_path: Path,
) -> None:
    env, engine, project, apb = _runtime_env(tmp_path, gates=False)
    _write_animation_component_wiring_source_validation_files(engine)
    ...
    report = runtime_harness.run_runtime_harness(
        manifest=runtime_harness.DEFAULT_MANIFEST,
        diagnose_runtime_animation_playback_execution_api=True,
        strict=True,
        engine_root=engine,
        project=project,
        apb_report=apb,
    ...
    assert report["status"] == "pass"
    assert report["runtime_harness_mode"] == "runtime_animation_playback_execution_api_diagnostic"
    assert report["runtime_harness_status"] == "runtime_animation_playback_execution_api_source_discovery"
    assert report["runtime_animation_playback_execution_api_diagnostic_attempted"] is True
    assert report["runtime_animation_playback_execution_api_diagnostic_completed"] is True
    assert report["runtime_animation_playback_execution_api_source_validation_verified"] is True
    assert report["runtime_animation_playback_execution_api_found"] is True
    assert report["runtime_animation_playback_execution_api_selected"] == (
...
def test_runtime_harness_animation_playback_execution_fixture_verifies_bounded_playback(
    tmp_path: Path,
) -> None:
    env, engine, project, apb = _runtime_env(tmp_path, gates=True)
    _write_animation_component_wiring_source_validation_files(engine)
    ...
    report = runtime_harness.run_runtime_harness(
        manifest=runtime_harness.DEFAULT_MANIFEST,
        enable_runtime_animation_playback_execution_fixture=True,
        strict_integration=True,
        engine_root=engine,
        project=project,
        apb_report=apb,
    ...
    assert report["status"] == "pass"
    assert report["runtime_harness_mode"] == "runtime_animation_playback_execution_fixture_command"
    assert report["runtime_exit_fixture_runtime_command_uses_animation_playback_execution_probe"] is True
    assert report["runtime_animation_playback_execution_api_diagnostic_attempted"] is True
    assert report["runtime_animation_playback_execution_api_diagnostic_completed"] is True
    assert report["runtime_animation_playback_execution_api_source_validation_verified"] is True
    assert report["runtime_animation_playback_component_wiring_verified_prerequisite"] is True
    assert report["runtime_character_animation_component_wiring_verified"] is True
    ...
    assert report["runtime_character_animation_playback_attempted"] is True
    assert report["runtime_character_animation_playback_request_issued"] is True
    assert report["runtime_character_animation_playback_started"] is True
    assert report["runtime_character_animation_playback_observed"] is True
    assert report["runtime_animation_playback_time_advanced"] is True
    assert report["runtime_character_animation_playback_time_before"] == 0.0
    assert report["runtime_character_animation_playback_time_after"] == 0.125
    assert report["runtime_character_animation_playback_tick_count"] == 8
...
def test_runtime_harness_animation_playback_execution_blocks_without_time_advance(
    tmp_path: Path,
) -> None:
    env, engine, project, apb = _runtime_env(tmp_path, gates=True)
    _write_animation_component_wiring_source_validation_files(engine)
    ...
    report = runtime_harness.run_runtime_harness(
        ...
        command_runner=_animation_playback_surface_fixture_runner(
            ...
            playback_markers=True,
            playback_time_before="0.000000",
            playback_time_after="0.000000",
            playback_tick_count=8,
            playback_observed=False,
            playback_time_advanced=False,
            playback_active_state_observed=False,
            playback_blocker="blocked_by_runtime_motion_playback_time_not_advanced",
        ),
        artifact_root=tmp_path / "runtime-artifacts",
        timeout_seconds=180,
    )
    ...
    assert report["runtime_character_animation_playback_started"] is True
    assert report["runtime_character_animation_playback_observed"] is False
    assert report["runtime_animation_playback_time_advanced"] is False
...
def test_runtime_harness_animation_playback_execution_keeps_poolallocator_blocking(
    tmp_path: Path,
) -> None:
    env, engine, project, apb = _runtime_env(tmp_path, gates=True)
    _write_animation_component_wiring_source_validation_files(engine)
    ...
    report = runtime_harness.run_runtime_harness(
        ...
        command_runner=_animation_playback_surface_fixture_runner(
            ...
            playback_time_after="0.125000",
            playback_tick_count=8,
            extra_stdout_lines=[poolallocator_line],
        ),
        artifact_root=tmp_path / "runtime-artifacts",
        timeout_seconds=180,
    )
...
def test_runtime_harness_animation_playback_execution_fixture_rejects_typeids_only(
    tmp_path: Path,
) -> None:
    env, engine, project, apb = _runtime_env(tmp_path, gates=True)
    _write_animation_component_wiring_source_validation_files(engine)
...
def test_runtime_harness_animation_playback_execution_preserves_specific_marker_blocker_without_request(
    tmp_path: Path,
) -> None:
    env, engine, project, apb = _runtime_env(tmp_path, gates=True)
    _write_animation_component_wiring_source_validation_files(engine)
    ...
    specific_blocker = "blocked_by_runtime_simple_motion_component_missing_for_playback"
        ...
        command_runner=_animation_playback_surface_fixture_runner(
            ...
            extra_stdout_lines=[
                ...
                (
                    "MAXINE_RUNTIME_ANIMATION_PLAYBACK_OBSERVE "
                    "entity_id={44444444-4444-4444-8444-444444444444} "
                    "simple_motion_component_found=0 play_time_before=0.000000 play_time_after=0.000000 "
                    "time_advanced=0 active_state_observed=0 started=0 observed=0 tick_count=0 "
                    f"status=fail blocker={specific_blocker}"
                ),
                (
                    "MAXINE_RUNTIME_ANIMATION_PLAYBACK_SUMMARY status=fail requested=0 request_succeeded=0 "
                    "started=0 observed=0 time_advanced=0 active_state_observed=0 tick_count=0 "
                    "cleanup=deferred_until_spawn_cleanup simple_motion_component_found=0 "
                    f"blocker={specific_blocker}"
                ),
            ],
        ),
        artifact_root=tmp_path / "runtime-artifacts",
    ...
    assert report["status"] == "fail"
    assert report["runtime_animation_playback_request_attempted"] is False
    assert report["runtime_animation_playback_simple_motion_component_found"] is False
    assert report["runtime_animation_playback_execution_api_blocker"] == specific_blocker
    assert report["runtime_animation_playback_execution_api_blocker"] != (
        "blocked_by_runtime_animation_playback_observation_unavailable"
    )
    assert report["runtime_character_animation_verified"] is False
...
def test_runtime_harness_animation_playback_execution_falls_back_without_specific_marker_blocker(
    tmp_path: Path,
) -> None:
    env, engine, project, apb = _runtime_env(tmp_path, gates=True)
    _write_animation_component_wiring_source_validation_files(engine)
...
def test_runtime_harness_animation_playback_execution_preserves_specific_request_failure_blocker(
    tmp_path: Path,
) -> None:
    env, engine, project, apb = _runtime_env(tmp_path, gates=True)
    _write_animation_component_wiring_source_validation_files(engine)
    ...
    specific_blocker = "blocked_by_runtime_animation_product_load_prerequisite"
    ...
    report = runtime_harness.run_runtime_harness(
        ...
        command_runner=_animation_playback_surface_fixture_runner(
            ...
            playback_time_advanced=False,
            playback_active_state_observed=False,
            playback_blocker=specific_blocker,
        ),
        artifact_root=tmp_path / "runtime-artifacts",
        timeout_seconds=180,
    )
...
def test_runtime_harness_animation_playback_execution_preserves_product_load_access_violation_blocker(
    tmp_path: Path,
) -> None:
    env, engine, project, apb = _runtime_env(tmp_path, gates=True)
    _write_animation_component_wiring_source_validation_files(engine)
    ...
    def _runner(**kwargs: object) -> subprocess.CompletedProcess[str]:
        result = _animation_playback_surface_fixture_runner(
            ...
            playback_time_advanced=False,
            playback_active_state_observed=False,
            playback_blocker="blocked_by_runtime_animation_product_load_prerequisite",
        )(**kwargs)
        return subprocess.CompletedProcess(
            result.args,
            3221225477,
...
def test_runtime_harness_animation_playback_execution_modes_bypass_command_pinning(
    tmp_path: Path,
) -> None:
    env, engine, project, apb = _runtime_env(tmp_path, gates=False)
    _write_animation_component_wiring_source_validation_files(engine)
    ...
    def _runner(**_kwargs: object) -> subprocess.CompletedProcess[str]:
        raise AssertionError("playback execution diagnostic must not execute command pinning")
...
def test_runtime_harness_character_behavior_smoke_diagnostic_records_source_validation(
    ...
    report = runtime_harness.run_runtime_harness(
        manifest=runtime_harness.DEFAULT_MANIFEST,
        diagnose_runtime_character_behavior_smoke_gate=True,
        strict=True,
        engine_root=engine,
        project=project,
        apb_report=apb,
    ...
    assert report["status"] == "pass"
    assert report["runtime_harness_mode"] == "runtime_character_behavior_smoke_gate_diagnostic"
    assert report["runtime_character_behavior_smoke_source_validation_verified"] is True
    assert report["runtime_character_behavior_smoke_attempted"] is False
...
def test_runtime_harness_character_behavior_smoke_fixture_passes_with_stable_playback_window(
    tmp_path: Path,
) -> None:
    env, engine, project, apb = _runtime_env(tmp_path, gates=True)
    _write_animation_component_wiring_source_validation_files(engine)
    ...
    report = runtime_harness.run_runtime_harness(
        ...
        command_runner=_animation_playback_surface_fixture_runner(
            ...
            extra_stdout_lines=[
                ...
                (
                    "MAXINE_RUNTIME_CHARACTER_BEHAVIOR_SMOKE_SUMMARY "
                    "status=pass attempted=1 completed=1 claimed=1 verified=1 "
                    "tick_count=12 cleanup=deferred_until_spawn_cleanup "
                    "exit=deferred_until_runtime_exit blocker="
                ),
            ],
        ),
        artifact_root=tmp_path / "runtime-artifacts",
    ...
    assert report["status"] == "pass"
    assert report["runtime_harness_mode"] == "runtime_character_behavior_smoke_fixture_command"
    assert report["runtime_character_behavior_smoke_attempted"] is True
    assert report["runtime_character_behavior_smoke_completed"] is True
    assert report["runtime_character_behavior_smoke_claimed"] is True
    ...
    assert report["runtime_character_behavior_smoke_exit_code_hex"] == "0x00000000"
    assert report["runtime_character_behavior_smoke_selected_log_scan_passed"] is True
    assert report["runtime_character_animation_verified"] is True
    assert report["runtime_character_animation_component_wiring_verified"] is True
...
def test_runtime_harness_character_behavior_smoke_blocks_when_motion_instance_lost(
    tmp_path: Path,
) -> None:
    env, engine, project, apb = _runtime_env(tmp_path, gates=True)
    _write_animation_component_wiring_source_validation_files(engine)
        ...
        command_runner=_animation_playback_surface_fixture_runner(
            ...
            extra_stdout_lines=[
                ...
                (
                    ...
                    "playback_time_before=0.000000 playback_time_after=0.250000 "
                    "tick_count=12 duration_seconds=0.200000 status=fail "
                    "blocker=blocked_by_runtime_character_behavior_smoke_motion_instance_lost"
                ),
                (
                    "MAXINE_RUNTIME_CHARACTER_BEHAVIOR_SMOKE_SUMMARY "
                    "status=fail attempted=1 completed=1 claimed=0 verified=0 tick_count=12 "
                    "cleanup=deferred_until_spawn_cleanup "
                    "blocker=blocked_by_runtime_character_behavior_smoke_motion_instance_lost"
                ),
            ],
        ),
    ...
    assert report["runtime_character_behavior_smoke_completed"] is True
    assert report["runtime_character_behavior_smoke_verified"] is False
    assert report["runtime_character_behavior_smoke_motion_instance_available_after"] is False
    assert report["runtime_character_behavior_smoke_blocker"] == (
...
def test_runtime_harness_character_behavior_smoke_preserves_selected_log_blocker(
    tmp_path: Path,
) -> None:
    env, engine, project, apb = _runtime_env(tmp_path, gates=True)
    _write_animation_component_wiring_source_validation_files(engine)
    ...
    poolallocator_line = "PoolAllocator.cpp:470 'Found page for bucket 13 during allocator teardown'"
        ...
        command_runner=_animation_playback_surface_fixture_runner(
            ...
            extra_stdout_lines=[
                ...
                (
                    ...
                    "motion_instance_available_before=0 motion_instance_available_after=1 "
                    "transform_readback_attempted=1 transform_valid=1 playback_time_monotonic=1 "
                    "playback_time_before=0.000000 playback_time_after=0.125000 "
                    "tick_count=8 duration_seconds=0.133333 status=pass blocker="
                ),
                "MAXINE_RUNTIME_CHARACTER_BEHAVIOR_SMOKE_SUMMARY status=pass attempted=1 completed=1 claimed=1 verified=1 tick_count=8 blocker=",
                poolallocator_line,
            ],
        ),
        artifact_root=tmp_path / "runtime-artifacts",
    ...
    assert report["status"] == "fail"
    assert report["runtime_character_behavior_smoke_selected_log_scan_passed"] is False
    assert report["runtime_character_behavior_smoke_verified"] is False
    assert report["runtime_character_behavior_smoke_blocker"] == (
        "blocked_by_runtime_character_behavior_smoke_selected_log_signal"
    )
    assert (
        report["runtime_exit_fixture_status"]
        == "runtime_exit_fixture_execution_failed_runtime_character_behavior_smoke"
    )
    assert report["runtime_animation_playback_selected_log_blocking_matches"] == [poolallocator_line]
    assert report["runtime_character_proof_claimed"] is False
    assert report["runtime_character_proof_verified"] is False
...
def test_runtime_harness_animation_playback_surface_fixture_blocks_inconclusive_source_validation(
    ...
    assert report["runtime_character_animation_playback_surface_diagnostic_completed"] is True
    assert report["runtime_character_animation_source_validation"]["status"] == (
        "runtime_character_animation_source_validation_inconclusive"
    )
    assert report["runtime_character_animation_playback_surface_blocker"] == (
    ...
    assert report["runtime_harness_status"] != "runtime_execution_pass"
    assert report["runtime_exit_fixture_status"] == (
        "runtime_exit_fixture_execution_failed_character_animation_playback_surface"
    )
    assert report["runtime_character_animation_claimed"] is False
    assert report["runtime_character_animation_verified"] is False
    assert report["runtime_character_proof_claimed"] is False
...
def test_runtime_harness_animation_source_validation_helper_requires_positive_status_and_refs() -> None:
    positive = {
        "runtime_character_animation_source_validation": {
            "status": "runtime_character_animation_source_validation_pass",
            "files": [{"status": "pass", "path": "ActorComponent.h"}],
            "missing": [],
            "runtime_component_surfaces": {
                "actor_component_type_id": "{BDC97E7F-A054-448B-A26F-EA2B5D78E377}",
                "actor_request_bus": "EMotionFX::Integration::ActorComponentRequestBus",
                "anim_graph_component_type_id": "{77624349-D5C4-4902-9F08-665814520999}",
                "anim_graph_request_bus": "EMotionFX::Integration::AnimGraphComponentRequestBus",
                "simple_motion_component_type_id": "{DBE3C105-6FC1-418F-A8B1-D0F29FE8D5BD}",
                "simple_motion_request_bus": "EMotionFX::Integration::SimpleMotionComponentRequestBus",
            },
        },
        "runtime_character_animation_source_refs": runtime_harness._runtime_character_animation_source_refs(
            Path("<engine-root>")
        ),
    }
    inconclusive = {
        **positive,
        "runtime_character_animation_source_validation": {
            **positive["runtime_character_animation_source_validation"],
            "status": "runtime_character_animation_source_validation_inconclusive",
        },
        "runtime_character_animation_playback_surface_blocker": "blocked_by_runtime_animation_source_validation",
    }

    assert runtime_harness._runtime_character_animation_source_validation_passed(positive) is True
    assert runtime_harness._runtime_character_animation_source_validation_passed(inconclusive) is False
    assert runtime_harness._runtime_character_animation_source_validation_passed({}) is False


def test_runtime_harness_validation_rejects_animation_fixture_pass_with_missing_source_validation_fields() -> None:
    report = runtime_harness.fixture_runtime_harness_report()
    report.update(
        {
            "status": "pass",
            ...
            "runtime_execution_verified": True,
            "runtime_exit_fixture_execution_verified": True,
            "runtime_exit_fixture_runtime_command_uses_animation_playback_surface_probe": True,
