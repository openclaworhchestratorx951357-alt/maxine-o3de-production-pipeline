...
def _validator_script() -> Path:
    return (
        _repo_root()
        / "tools"
        / "release-publication-execution-window-ticket"
        / "validate_release_publication_execution_window_ticket_report.py"
    )


def _report_path(name: str) -> Path:
    return _repo_root() / "examples" / "release-publication-execution-window-ticket" / name
...
def _load_pass() -> dict:
    return json.loads(
        _report_path("max_biped_v1_release_publication_execution_window_ticket_pass.json").read_text(
            encoding="utf-8-sig"
        )
    )
...
def test_pass_report_returns_pass_and_exit_zero():
    result = _run_validator(
        _report_path("max_biped_v1_release_publication_execution_window_ticket_pass.json")
    )
...
def test_warn_report_returns_warn_and_exit_zero_with_allow_warn():
    result = _run_validator(
        _report_path("max_biped_v1_release_publication_execution_window_ticket_warn.json"),
        allow_warn=True,
    )
...
def test_fail_report_returns_fail_and_nonzero():
    result = _run_validator(
        _report_path("max_biped_v1_release_publication_execution_window_ticket_fail.json")
    )
...
def test_missing_required_report_artifact_hash_fails(tmp_path: Path):
    report = _load_pass()
    report["status"] = "fail"
    report["execution_window_ticket"]["window_ticket_verification_status"] = "fail"
    report["execution_window_ticket"]["window_ticket_artifact_hashes"].pop(
        "release_publication_execution_receipt", None
    )
    report["execution_window_ticket"]["artifact_count"] = len(
        report["execution_window_ticket"]["window_ticket_artifact_hashes"]
    )
    report["execution_window_ticket"]["missing_window_ticket_artifact_ids"] = [
    ...
    report["readiness"]["execution_window_ticket_readiness_state"] = "blocked_missing_evidence"
    ...
    assert payload["status"] == "fail"
    assert any(
        item.get("id") == "required_window_ticket_artifacts_missing"
        for item in payload["findings"]
    )


def test_unsafe_window_ticket_path_fails(tmp_path: Path):
    report = _load_pass()
    report["status"] = "fail"
    report["execution_window_ticket"]["window_ticket_verification_status"] = "fail"
    report["execution_window_ticket"]["window_ticket_path"] = "..\\outside\\audit-bundle.json"
    report["approval_state"]["decision"] = "rejected"
    report["approval_state"]["blocked_reason_codes"] = ["unsafe_bundle_path"]
    report["readiness"]["execution_window_ticket_readiness_state"] = "blocked_safety_boundary"
    ...
    assert result.returncode != 0
    assert payload["status"] == "fail"
    assert any(item.get("id") == "window_ticket_path_unsafe" for item in payload["findings"])
...
def test_source_execution_authorization_record_reference_must_point_to_authorization_record(tmp_path: Path):
    ...
    report["status"] = "fail"
    report["execution_window_ticket"]["window_ticket_verification_status"] = "fail"
    report["execution_window_ticket"]["source_execution_authorization_record_id"] = (
        "ready-for-execution-request-20260505-0001"
    )
    report["execution_window_ticket"]["source_execution_authorization_record_path"] = (
...
def test_output_contains_manifest_attachable_qc_payload():
    ...
    assert qc_check.get("check_id") == "release_publication_execution_window_ticket_v1"
