...
def _validator_script() -> Path:
    return _repo_root() / "tools" / "release-publication-request-approval" / "validate_release_publication_request_approval_report.py"


def _report_path(name: str) -> Path:
    return _repo_root() / "examples" / "release-publication-request-approval" / name
...
def test_pass_report_returns_pass_and_exit_zero():
    result = _run_validator(_report_path("max_biped_v1_release_publication_request_approval_pass.json"))
...
def test_warn_report_returns_warn_and_exit_zero_with_allow_warn():
    result = _run_validator(_report_path("max_biped_v1_release_publication_request_approval_warn.json"), allow_warn=True)
...
def test_warn_report_returns_nonzero_without_allow_warn():
    result = _run_validator(_report_path("max_biped_v1_release_publication_request_approval_warn.json"), allow_warn=False)
...
def test_fail_report_returns_fail_and_nonzero():
    result = _run_validator(_report_path("max_biped_v1_release_publication_request_approval_fail.json"))
...
def test_missing_required_gates_fails(tmp_path: Path):
    report = json.loads(_report_path("max_biped_v1_release_publication_request_approval_pass.json").read_text(encoding="utf-8-sig"))
    report["status"] = "fail"
...
def test_insufficient_approvals_fails(tmp_path: Path):
    ...
    report["readiness"]["release_readiness_state"] = "ready_for_manual_publication_execution_request"
...
def test_pending_decision_returns_pending_manual(tmp_path: Path):
    ...
    report["approval_state"]["decision"] = "pending"
    report["approval_state"]["approver_ids"] = []
    report["approval_state"]["blocked_reason_codes"] = ["waiting_manual_signoff"]
    ...
    assert any(item.get("id") == "publication_request_pending_manual_review" for item in payload["findings"])


def test_unsafe_publish_command_token_fails(tmp_path: Path):
    report = json.loads(_report_path("max_biped_v1_release_publication_request_approval_pass.json").read_text(encoding="utf-8-sig"))
    report["status"] = "fail"
    report["publication_request"]["publish_request_command_display"] = "DisplayOnly Publish Request: release_bundle publish --manifest a.json | invoke-expression"
    ...
    assert result.returncode != 0
    assert payload["status"] == "fail"
    assert any(item.get("id") == "publish_request_command_display_unsafe" for item in payload["findings"])
...
def test_output_contains_manifest_attachable_qc_payload():
    ...
    assert qc_check.get("check_id") == "release_publication_request_approval_v1"
