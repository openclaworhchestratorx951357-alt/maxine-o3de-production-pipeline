...
def _validator_script() -> Path:
    return _repo_root() / "tools" / "release-promotion-decision" / "validate_release_promotion_decision_report.py"


def _report_path(name: str) -> Path:
    return _repo_root() / "examples" / "release-promotion-decision" / name
...
def test_pass_report_returns_pass_and_exit_zero():
    result = _run_validator(_report_path("max_biped_v1_release_promotion_decision_pass.json"))
...
def test_warn_report_returns_warn_and_exit_zero_with_allow_warn():
    result = _run_validator(_report_path("max_biped_v1_release_promotion_decision_warn.json"), allow_warn=True)
...
def test_warn_report_returns_nonzero_without_allow_warn():
    result = _run_validator(_report_path("max_biped_v1_release_promotion_decision_warn.json"), allow_warn=False)
...
def test_fail_report_returns_fail_and_nonzero():
    result = _run_validator(_report_path("max_biped_v1_release_promotion_decision_fail.json"))
...
def test_missing_required_gates_fails(tmp_path: Path):
    report = json.loads(_report_path("max_biped_v1_release_promotion_decision_pass.json").read_text(encoding="utf-8-sig"))
    report["status"] = "fail"
    report["readiness"]["present_gate_ids"] = ["max_biped_v1_skeleton_contract", "manual_hero_review_v1"]
...
def test_insufficient_approvals_fails(tmp_path: Path):
    report = json.loads(_report_path("max_biped_v1_release_promotion_decision_pass.json").read_text(encoding="utf-8-sig"))
    report["status"] = "fail"
    report["decision"]["approver_ids"] = ["ops.release_lead"]
    report["readiness"]["release_readiness_state"] = "ready_for_manual_release_review"
...
def test_pending_decision_returns_pending_manual(tmp_path: Path):
    ...
    report["decision"]["promotion_decision"] = "pending"
    report["decision"]["approver_ids"] = []
    report["decision"]["blocked_reason_codes"] = ["waiting_manual_signoff"]
    ...
    assert any(item.get("id") == "promotion_pending_manual_review" for item in payload["findings"])
...
def test_output_contains_manifest_attachable_qc_payload():
    ...
    assert qc_check.get("check_id") == "release_promotion_decision_v1"
